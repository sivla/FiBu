...
| Anlagen/Projekte/Service/Manufacturing | 14-16, 21 | Fixed Assets/Warehouse/Manufacturing/Service sind als Readiness bzw. Buch-Sync teilweise belegt; Fixed Assets hat `HGB` und `MACHINES` als Labor-Setup-Prerequisites sichtbar, `FIXEDASSETS-018` zeigt die leere Anlagenkarte als Feld-Preflight, `FIXEDASSETS-020` belegt per `Mehr anzeigen` die Feldpfade fuer `Depreciation Book Code` und `Posting Group`, `FIXEDASSETS-029-EXISTING` zeigt die vorhandene unvollstaendige `FA-CNC-01`-Karte und `FIXEDASSETS-030` entscheidet die Korrektur dieser Karte; Projects ist mit `PROJECTS-001` als Readiness gestartet und mit `PROJECTS-002` im Buch synchronisiert | teilweise | teilweise | teilweise | partial-readiness / fixedassets-correction-approved-next-run | `FIXEDASSETS-031` als naechster Gate-Schritt: bestehende `FA-CNC-01`-Karte korrigieren; Setup/Buchung nur mit Gate |
