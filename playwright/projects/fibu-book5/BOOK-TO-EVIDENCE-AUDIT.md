...
| Anlagen/Projekte/Service/Manufacturing | 14-16, 21 | Fixed Assets/Warehouse/Manufacturing/Service sind als Readiness bzw. Buch-Sync teilweise belegt; Fixed Assets hat `HGB` und `MACHINES` als Labor-Setup-Prerequisites sichtbar, `FIXEDASSETS-018` zeigt die leere Anlagenkarte als Feld-Preflight und `FIXEDASSETS-020` belegt per `Mehr anzeigen` die Feldpfade fuer `Depreciation Book Code` und `Posting Group`; Projects ist mit `PROJECTS-001` als Readiness gestartet und mit `PROJECTS-002` im Buch synchronisiert | teilweise | teilweise | teilweise | partial-readiness / fixedassets-field-paths-visible-labor | Fixed-Assets-Setup-Fit-Entscheidung fuer `FA-CNC-01` als naechster Gate-Schritt; Setup/Buchung nur mit Gate |
