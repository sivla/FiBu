...
| `universaarl-training-draft` | Echte `playthru`-/`UNIVERSAARL-DE`-Beobachtung liegt vor und darf fuer Training/Handbuch-Entwurf genutzt werden; UAT, Prozessfreigabe, Preview und Posting sind nicht bewiesen. |
...
| `UAT-MD-001` | Kap. 10 - Debitoren | `process-test` | `FOUNDATION-READINESS-DECISION.md` erlaubt Training/Handbuch aus vorhandener Evidence; neue Debitoranlage bleibt ohne eigenes Write-Gate gesperrt | `U-CUST-100` / `Saarland Maschinenbau AG`; realistische fiktive Universaarl-Kundendaten, keine vertraulichen echten Kundendaten | Key User koennen die Debitorenkarte, `Fakturierung` und `Zahlungen` lesen und Setup-Grenzen erkennen; O2C, Preview und Posting bleiben blockiert. | `PWS-MD-001`, `PWS-MD-004C`, `CUSTOMER-U-CUST-100-SETUP-REOPEN-PROOF`; keine Mockup-Screenshots | `universaarl-training-draft` | keine neue Anlage; naechste sinnvolle Route ist VAT-/Posting-Boundary oder Debitoren-Handbuchtext |
...
| `UAT-MD-003` | Kap. 10-13 - Artikel | `process-test` | `FOUNDATION-READINESS-DECISION.md` erlaubt Training/Handbuch aus vorhandener Evidence; neue Artikel-/Serviceanlage bleibt ohne eigenes Write-Gate gesperrt | `U-ITEM-HW100` / `Steuerbox Standard U100`; realistische fiktive Universaarl-Produktdaten, keine vertraulichen echten Produktivdaten | Key User koennen Artikelkarte, Basiseinheit, Preise, Buchungsgruppen- und USt-Signale lesen; O2C, P2P, Inventory Valuation, Preview und Posting bleiben blockiert. | `PWS-MD-003`, `ITEM-SERVICE-MASTERDATA-READFIRST`, `ITEM-SERVICE-U-ITEM-HW100-CARD-EDITOR-RECOVERY`, `ITEM-SERVICE-U-ITEM-HW100-PRICE-COST-FIELD-ROUTE`; keine Mockup-Screenshots | `universaarl-training-draft` | keine neue Anlage; naechste sinnvolle Route ist VAT-/Posting-/Inventory-Boundary oder Artikel-Handbuchtext |
