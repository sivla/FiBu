...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-001` belegt offene Debitoren-/Kreditorenposten als Startpunkt; Bankkonto selbst noch nicht geprueft | ja | ja fuer offene Posten, nein fuer Bankkonto | `partial` | P4 | `PAYMENTS-002`: Bankkonto-Readiness pruefen, bevor Zahlung oder Bankabstimmung gebucht wird. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; Zahlungs-/Cash-Receipt-Journal ist noch nicht geprueft; `PAYMENTS-001` zeigt nur die offenen Ausgangsposten | teilweise | ja fuer Item Journal und offene Posten, nein fuer Zahlungsjournale | `partial` | P4 | `PAYMENTS-002`: Payment Journal/Cash Receipt Journal und Apply-Entries-Pfad read-only/controlled pruefen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt read-only Debitorenposten `PS-INV103297` und Kreditorenposten `108219`; Restbetrag/Open-Logik und Payment-/Apply-Aktionen sichtbar; Zahlung, Ausgleich, Zahlungsjournal und Bankabstimmung fehlen | ja als Prozessfall | ja, `payments-001` | `partial` | P4 | `PAYMENTS-002`: Bankkonto, Zahlungs-/Cash-Receipt-Journal, Zahlungsbedingungen und Apply-Entries-Pfad pruefen; erst danach ggf. eine einzelne Laborzahlung. |
...
2. P2P-Laborbuchung ist erledigt: `106049` -> `108219`. Nicht erneut buchen; `PAYMENTS-001` hat offene Posten als Startpunkt belegt. Naechster P2P-/O2C-naher Schritt ist `PAYMENTS-002` fuer Bank-/Journal-/Apply-Readiness.
...
Lies CURRENT-STATE.md, LAB-FIT-STATUS.md, BOOK-CLICK-GUIDE-COVERAGE.md, evidence/payments-001/README.md, evidence/payments-001/PAYMENTS-001-result.json, evidence/uat-o2c-001/O2C-LAB-FINAL-SYNC.md und evidence/p2p-001/P2P-LAB-POSTING.md.
Starte `PAYMENTS-002` als kontrollierten Readiness-Lauf: pruefe Bankkonto, Payment Journal/Cash Receipt Journal, Zahlungsbedingungen und Apply-Entries-Pfad fuer die offenen Posten `PS-INV103297` und `108219`, ohne Zahlung oder Ausgleich zu buchen. Dokumentiere, welcher Pfad fuer Zahlungseingang und Zahlungsausgang sicher ist und welche Laborgrenzen bleiben. Keine neue O2C-/P2P-/Inventory-Buchung, keine Warehouse-Aktivierung, kein deutscher Finalnachweis.
```
