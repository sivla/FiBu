...
const specPath = 'playwright/projects/fibu-book5/tests/customer-u-cust-100-dependency-readfirst.spec.ts';
const casePath = '.agent/state/cases/customer-u-cust-100-dependency-readfirst.json';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const ACTIVE_CASE = 'CUSTOMER-U-CUST-100-DEPENDENCY-READFIRST';
...
function activeCaseStatus() {
  if (!existsSync(casePath)) {
    return {
      ...
      nextStep: 'Create customer-u-cust-100-dependency-readfirst.json before running.'
    };
  }
  let parsed;
  try {
  ...
  const ready =
    ...
    parsed.effectiveBcActionsAllowed === false;
  return {
    ready,
    blocker: ready ? '' : 'case-file-does-not-match-readfirst-gate',
    nextStep: ready
      ? 'Run CUSTOMER-U-CUST-100-DEPENDENCY-READFIRST only with --live-approved.'
      : 'Align case file to playthru / UNIVERSAARL-DE read-first/no-write before running.'
  };
}

if (help) {
  console.log(`CUSTOMER-U-CUST-100-DEPENDENCY-READFIRST guarded runner

Usage:
  node scripts/agent/run-customer-u-cust-100-dependency-readfirst.mjs --check
  node scripts/agent/run-customer-u-cust-100-dependency-readfirst.mjs --list
  node scripts/agent/run-customer-u-cust-100-dependency-readfirst.mjs --live-approved

This runner inspects existing customer U-CUST-100 read-only. It must not edit, save, post, preview, create documents or use API shortcuts.`);
  process.exit(0);
}
...
const blockedBy = [
  ...
  activeCaseMatches ? '' : 'active-case-is-not-customer-u-cust-100-dependency-readfirst',
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-u-cust-100-dependency-readfirst-check',
        caseId: ACTIVE_CASE,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-u-cust-100-dependency-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run customer dependency proof before case gate, live gate, usable auth and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      CUSTOMER_U_CUST_100_DEP_RUNNER_GUARD_CHECKED: '1',
      CUSTOMER_U_CUST_100_DEP_LIVE_APPROVED: '1',
      CUSTOMER_U_CUST_100_DEP_BC_TARGET_URL: targetUrl
    }
  })
);
