...
- Anlagen: `FIXEDASSETS-001`/`FIXEDASSETS-002` sind Labor-Readiness; `FIXEDASSETS-003` oeffnet direkte Zielseiten read-only; `FIXEDASSETS-004` prueft die Zielwerte gefiltert. Praktisch sichtbar sind Anlagenliste `5601`, AfA-Buecher `5611`, Einkaufsrechnungen `9308` und Anlagenposten `5604`; `Purchase Invoices` bleibt als Zugangspfad erreichbar. Nicht sichtbar sind `FA-CNC-01`, `HGB`, `MACHINES` und `K30000`. `FA Posting Groups` ueber direkte Page-ID `5606` ist rejected; Tell-Me zeigt zwar den Treffer, aber der Klickpfad ist noch nicht belastbar. Der alte Testdatenbetrag `250.000` wurde auf das Buchziel `120.000 EUR` harmonisiert. Offen bleiben UI-Setup-Fit, Aktivierung, AfA-Buchung, Anlagenpostenspur fuer `FA-CNC-01` und deutscher Anlagen-Endstand.
