...
const specPath = 'playwright/projects/fibu-book5/tests/pws-md-003-item-service-context-readonly.spec.ts';
...
const REAL_CUSTOMER_PROJECT_BOUNDARY = {
  ...
  acceptedDataMode:
    'customer-project-like Universaarl item/service records that are fictional or anonymized, have business purpose, owner, dependencies, UAT/training use and BC setup readiness, and are proven through real playthru UI evidence before they become book or process truth'
};
...
if (help) {
  console.log(`PWS-MD-003 guarded runner

Usage:
  node scripts/agent/run-pws-md-003-item-service-context-readonly.mjs --check
  node scripts/agent/run-pws-md-003-item-service-context-readonly.mjs --list
  node scripts/agent/run-pws-md-003-item-service-context-readonly.mjs --live-approved

This runner never writes item or service data. Live execution requires:
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-003-item-service-context-readonly-check',
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-003-item-service-context-readonly-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep:
          'Do not run PWS-MD-003 live before TARGET-075 evidence, Foundation Readiness Decision, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_MD_003_RUNNER_GUARD_CHECKED: '1',
      PWS_MD_003_LIVE_APPROVED: '1',
      PWS_MD_003_BC_TARGET_URL: targetUrl
    }
  })
);
