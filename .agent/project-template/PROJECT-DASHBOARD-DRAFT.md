...
| Aktiver Case | `CUSTOMER-VAT-DIMENSION-BOUNDARY-DECISION` |
...
| Master Data | customer-setup-values-proven-boundary-next | Debitor `U-CUST-100 / Saarland Maschinenbau AG` ist auf echter `playthru`-Debitorenkarte sichtbar. `CUSTOMER-U-CUST-100-SETUP-REOPEN-PROOF` hat `INLAND` als Debitorenbuchungsgruppe, `INLAND` als Geschaeftsbuchungsgruppe und `NET30` als `Zlg.-Bedingungscode` mit separaten sichtbaren Reopen-Screenshots bewiesen. Naechster Schritt ist `CUSTOMER-VAT-DIMENSION-BOUNDARY-DECISION` lokal/read-only: VAT Business Posting Group, Dimensionen und Zahlungsform bleiben noch bewusst unbewiesen; kein Beleg, kein Preview, kein Posting. |
...
6. `CUSTOMER-U-CUST-100-SETUP-REOPEN-PROOF` ist beobachtet: `U-CUST-100 / Saarland Maschinenbau AG` zeigt `INLAND / INLAND / NET30` in echten `playthru / UNIVERSAARL-DE`-Screenshots. Naechster Schritt ist `CUSTOMER-VAT-DIMENSION-BOUNDARY-DECISION` lokal/read-only, damit O2C nicht vor VAT-/Dimensions-/Payment-Method-Boundary-Entscheidung startet. Kein Beleg, kein Preview, kein Posting.
