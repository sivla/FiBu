...
| Lagerbestand | Item Journals | 40 | `Post`, Batch Name, Posting Date, Entry Type, Document No., Item No., Location Code, Quantity, Unit Amount, Amount, Unit Cost, `Line` -> `Dimensions`, Zeilenmenue `Weitere Optionen anzeigen` -> `Zeile loeschen` | positiven Trainings-/Opening-Balance-Zugang `RM-M100 +2` kontrolliert vorbereiten und vor Buchung pruefen | `INVENTORY-005`, `INVENTORY-006` | `playwright/projects/fibu-book5/img/inventory-005-010-item-journal-direct.png`; `playwright/projects/fibu-book5/img/inventory-006-010-target-journal-line-before-post.png`; `playwright/projects/fibu-book5/evidence/inventory-005/`; `playwright/projects/fibu-book5/evidence/inventory-006/` | verstanden als Labor-Draft: Zielzeile und `PRODUCTLINE=MACHINE` sichtbar, `Post` sichtbar, keine stabile Preview-Wirkung, keine Buchung, Cleanup erfolgreich | Kapitel 13/23 |
...
| Reihenfolge | Ziel |
|---:|---|
| 1 | `INVENTORY-007`: Journal Check/Preview-Alternative fuer die vorbereitete `RM-M100 +2`-Zeile klaeren; erst danach genau eine positive Laborbuchung und Postenspur/Lagerbewertung sichern |
