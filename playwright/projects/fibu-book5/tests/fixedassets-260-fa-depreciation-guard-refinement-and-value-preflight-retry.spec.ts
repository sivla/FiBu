...
const TEST_ID = 'fixedassets-260';
const CASE_ID = 'FIXEDASSETS-260-FA-DEPRECIATION-GUARD-REFINEMENT-AND-VALUE-PREFLIGHT-RETRY';
...
test('FIXEDASSETS-260 refines guard and fills Calculate Depreciation target values without OK', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-guard-refinement-and-target-value-preflight-no-ok',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 30.06.2026, FADEP-260-NO-OK and FA-CNC-01 were visibly proven on the request page without OK.']
        : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-260-fa-depreciation-guard-refinement-and-value-preflight-retry.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-260/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-260/FIXEDASSETS-260-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-260/FIXEDASSETS-260-TARGET-VALUE-PREFLIGHT-NO-OK.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-260/010-target-value-preflight.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-260/010-target-value-preflight-text.txt',
    ],
    statePatch: {
      current: {
        activeCase: allValuesProved
          ? 'FIXEDASSETS-261-FA-DEPRECIATION-OK-EXECUTION-GATE-REVIEW'
          : 'FIXEDASSETS-261-FA-DEPRECIATION-TARGET-VALUE-PREFLIGHT-BLOCKER-REVIEW',
        active_case_file: allValuesProved
          ? '.agent/state/cases/fixedassets-261-fa-depreciation-ok-execution-gate-review.json'
          : '.agent/state/cases/fixedassets-261-fa-depreciation-target-value-preflight-blocker-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-260-fa-depreciation-guard-refinement-and-value-preflight-retry.json',
        nextStep: allValuesProved
          ? 'FIXEDASSETS-261: locally review whether a controlled OK execution is justified; do not execute OK yet.'
      ...
      coverage: {
        ...
        depreciationReadiness: allValuesProved
          ? 'FA-260 proved HGB, 30.06.2026, FADEP-260-NO-OK and FA-CNC-01 on the Calculate Depreciation request page without OK. OK remains locked pending local review.'
          : 'FA-260 could not prove all target request-page values without OK. OK remains locked pending blocker review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
    ...
    reason: allValuesProved
      ? 'Target request-page values were proven without OK after guard refinement. Local review required before any execution.'
      : 'At least one target request-page value was not proven after guard refinement. OK remains locked.',
    observed: {
      startedAt,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-260-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-260-TARGET-VALUE-PREFLIGHT-NO-OK.md'),
    [
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-260 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-260-result.json` | JSON-Ergebnis | Guard-verfeinerter Zielwert-Preflight ohne OK | keine AfA-Ausfuehrung | labor |',
      '| `FIXEDASSETS-260-TARGET-VALUE-PREFLIGHT-NO-OK.md` | Lernzusammenfassung | Zielwerte, Grenzen und naechsten Gate-Schritt | keine Journalzeile | no-ok |',
