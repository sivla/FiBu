...
const CASE_ID = 'TARGET-026F-CHART-OF-ACCOUNTS-CONTROLLED-SETUP-GATE';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'target-026f-chart-of-accounts-controlled-setup-gate';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-026F-result.json');
...
const account = {
  ...
  purpose: 'erstes geplantes Basiskonto fuer spaetere Bank-/Zahlungsstrecken'
};
...
async function correctIncomeBalanceOnAccountCard(page: Page, row: Locator) {
  ...
  if (!/Bank Saarland|1200/i.test(bodyText)) {
  ...
  const text = await safeText(page);
  return /1200[\s\S]{0,500}Bilanz/i.test(text) || /Bank Saarland[\s\S]{0,500}Bilanz/i.test(text)
...
async function setIncomeBalanceViaNameFieldTab(page: Page, frame: Frame) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const text = await safeText(page);
    if (/1200[\s\S]{0,1500}Bilanz/i.test(text) || /Bank Saarland[\s\S]{0,1500}Bilanz/i.test(text)) {
...
async function captureState(page: Page, filePrefix: string, step: string, extra: Record<string, unknown> = {}) {
  const compact = await compactPageText(page, {
    include: [
      /Chart of Accounts|Kontenplan|G\/L Account|Sachkonto|No\.|Nr\.|Name|Balance Sheet|Bilanz|Income Statement|GuV|1200|Bank Saarland|Neu|New|Liste bearbeiten|Edit List/i
    ],
    maxLines: 160,
    maxLineLength: 220
  });
  ...
  const snapshot = {
    ...
    visible: {
      ...
      accountName: literalPattern(account.name).test(text),
      balanceSheet: /1200[\s\S]{0,500}(Bilanz|Balance Sheet)/i.test(text) || /Bank Saarland[\s\S]{0,500}(Bilanz|Balance Sheet)/i.test(text),
      incomeStatement: /1200[\s\S]{0,500}(GuV|Income Statement)/i.test(text) || /Bank Saarland[\s\S]{0,500}(GuV|Income Statement)/i.test(text),
...
async function ensureAccount(page: Page): Promise<{ status: StepStatus; reason: string; steps: Array<Record<string, unknown>> }> {
  ...
  if (!filled) return { status: 'blocked', reason: 'Keine editierbaren Nr./Name-Felder fuer das geplante Konto gefunden.', steps };
  ...
  if (literalPattern(account.no).test(afterText) && literalPattern(account.name).test(afterText) && /1200[\s\S]{0,500}Bilanz/i.test(afterText)) {
...
test('TARGET-026F creates or blocks first planned G/L account with reopen proof', async ({ page }) => {
  await page.setViewportSize({ width: 2200, height: 1300 });
  await fs.mkdir(EVIDENCE_DIR, { recursive: true });

  await openChartOfAccounts(page);
  const before = await captureState(page, 'target-026f-010-before-chart-of-accounts', 'Before controlled setup attempt.', {
    targetAccount: account
  });

  const accountResult = await ensureAccount(page);
  const afterAttempt = await captureState(page, 'target-026f-020-after-account-attempt', 'After controlled account setup attempt.', {
    targetAccount: account,
    accountResult
  });
  await openChartOfAccounts(page);
  const afterReopen = await captureState(page, 'target-026f-030-after-reopen-proof', 'After reopening Chart of Accounts for persistence proof.', {
    targetAccount: account,
    accountResult
  });
  ...
  const nextStepDecisionCard = {
    ...
    lookaheadReviewed: [
      {
        caseId: 'TARGET-026G-CHART-OF-ACCOUNTS-REMAINING-ACCOUNTS-FIT',
        status: accountVisible ? 'ready-next' : 'needs-ui-discovery-first',
        reason: accountVisible
          ? 'The first account route has reopen proof and can be reused for remaining planned accounts.'
    ...
    selectedNextCase: accountVisible
      ? 'TARGET-026G-CHART-OF-ACCOUNTS-REMAINING-ACCOUNTS-FIT'
      : 'TARGET-026G-CHART-OF-ACCOUNTS-ROUTE-RECOVERY',
    whySelectedNextCaseIsBest: accountVisible
      ? 'The fastest useful follow-up is to complete the remaining planned foundation accounts on the proven route.'
  ...
  const result = {
    ...
    changedFiles: [
      'playwright/projects/fibu-book5/evidence/target-026f-chart-of-accounts-controlled-setup-gate/'
    ],
    evidenceRefs: [
      'target-026f-010-before-chart-of-accounts.png',
      'target-026f-020-after-account-attempt.png',
      'target-026f-030-after-reopen-proof.png',
      'TARGET-026F-result.json'
    ],
    warnings: [],
    blockedBy,
    ...
    statePatch:
      resultStatus === 'observed'
        ? {
            current: {
              activeCase: 'TARGET-026G-CHART-OF-ACCOUNTS-REMAINING-ACCOUNTS-FIT',
              nextCase: 'TARGET-026G-CHART-OF-ACCOUNTS-REMAINING-ACCOUNTS-FIT',
              activeArea: 'universaarl-chart-of-accounts-remaining-accounts-fit'
            },
            lastRunSummary: {
              caseId: CASE_ID,
    ...
    reason:
      resultStatus === 'observed'
        ? 'First planned Universaarl foundation account is visible after controlled setup and reopen proof.'
