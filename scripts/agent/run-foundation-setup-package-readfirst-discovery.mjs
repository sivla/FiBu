...
const specPath = 'playwright/projects/fibu-book5/tests/foundation-setup-package-readfirst-discovery.spec.ts';
const currentPath = '.agent/state/current.json';
const casePath = '.agent/state/cases/foundation-setup-package-readfirst-discovery.json';
...
function localCaseStatus() {
  ...
  if (activeCase !== CASE_ID) {
    return {
      ready: false,
      blocker: 'foundation-package-readfirst-not-active-case',
  ...
  if (caseFile.effectiveBcActionsAllowed !== false) {
    return {
      ...
      nextStep: 'This discovery must remain read-first/no-write.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'Run the read-first discovery only with --live-approved.'
  };
}

if (help) {
  console.log(`FOUNDATION package read-first discovery runner

Usage:
  node scripts/agent/run-foundation-setup-package-readfirst-discovery.mjs --check
  node scripts/agent/run-foundation-setup-package-readfirst-discovery.mjs --list
  node scripts/agent/run-foundation-setup-package-readfirst-discovery.mjs --live-approved

This runner opens only the Configuration Packages context in playthru / UNIVERSAARL-DE.
It must not create, import, export, validate or apply packages and must not write setup values.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-setup-package-readfirst-discovery-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-setup-package-readfirst-discovery-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run live discovery before case selection, fresh auth, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      FOUNDATION_PACKAGE_READFIRST_RUNNER_GUARD_CHECKED: '1',
      FOUNDATION_PACKAGE_READFIRST_LIVE_APPROVED: '1',
      FOUNDATION_PACKAGE_READFIRST_BC_TARGET_URL: targetUrl
    }
  })
);
