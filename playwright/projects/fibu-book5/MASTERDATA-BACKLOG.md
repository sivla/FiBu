...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an, aber API-basiert und damit nicht als Buch-Klickpfad; `PAYMENTS-004` bestaetigt live Sichtbarkeit; `PAYMENTS-005` nutzt `BANK-RM-01` in einer UI-Draft-Zeile | ja | ja, `payments-002`, `payments-003`, `payments-004`, `payments-005` | `partial` | P4 | Fuer finale Buchanleitung Bankkontoanlage als UI-Klickpfad nachziehen oder `BANK-RM-01` als vorbereitete Voraussetzung behandeln; Zahlungsbuchung weiter gesperrt. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only; `PAYMENTS-004` bestaetigt Pflichtfelder; `PAYMENTS-005` bereitet eine Cash-Receipt-Journal-Zeile per UI vor und loescht sie wieder, aber Journal Check meldet noch ein Amount-Issue | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003`, `payments-004`, `payments-005` | `partial` | P4 | UI-Draft Amount-Validierung klaeren, bis Journal Check keine Issues meldet; noch nicht buchen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder; `PAYMENTS-005` belegt UI-Draft mit Debitor, Betrag, Gegenkonto und Rechnungsbezug, aber Journal Check meldet `1 Issue`; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004`, `payments-005` | `partial` | P4 | `PAYMENTS-006`: UI-Draft korrigieren, bis Journal Check keine Issues meldet; erst danach ggf. eine Laborzahlung explizit freigeben. |
