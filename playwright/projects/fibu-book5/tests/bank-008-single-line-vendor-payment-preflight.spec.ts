...
function compactPageText(text: string) {
  const interesting =
    /BANK008|108204|107196|20000|First Up Consultants|BANK-RM-01|Payment Journal|Vendor Ledger Entries|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Vendor Name|Amount|Remaining Amount|Bal\. Account|Applies-to|Apply Entries|Journal Check|Refresh|Post|Payment|Vendor|Open|Application|muss|must|Issues/i;
...
async function openVendorLedger(page: Page) {
  ...
  await writeTextEvidence(bankEvidencePath('010-vendor-ledger-document-108204-page-text.txt'), compactPageText(text));
  await screenshot(page, 'bank-008-010-vendor-ledger-document-108204.png', {
    projectName: project.name,
    testId,
    status: text.includes(targetDocumentNo) && text.includes(vendorName) ? 'labor' : 'rejected',
    purpose: 'BANK-008 Zielposten vor Payment-Journal-Preflight: Vendor Ledger Entry 108204 / First Up Consultants.',
    knownLimitations: [
  ...
  return {
    ...
    amountVisible: /-2\.151,46|-2151,46|2\.151,46/i.test(text),
    openSignalVisible: /\bOpen\b|Offen|Remaining Amount|Restbetrag/i.test(text),
    textEvidenceFile: '010-vendor-ledger-document-108204-page-text.txt',
    buttons
  };
}
...
async function prepareDraft(page: Page) {
  ...
  await fillControl(initialControls[8], 'BANK-008 preflight vendor payment 108204');
...
async function captureApplyEntriesPreflight(page: Page) {
  ...
  const text = normalizeText(await pageText(page));
  const buttons = (await visibleButtonNames(page)).map(sanitizeEvidenceText);
  const opened = clicked && /Apply.*Entries|Apply Vendor Entries|Posten ausgleichen|108204|20000|Remaining Amount/i.test(text);
  ...
  await writeJsonEvidence(bankEvidencePath('030-apply-entries-preflight-controls.json'), {
    ...
    clicked,
    opened,
    documentVisible: /108204|20000|First Up Consultants/i.test(text),
    dangerousActionsVisible: /Set Applies-to ID|Post Application/i.test(`${text}\n${buttons.join('\n')}`),
    dangerousActionsClicked: false,
    buttons
  });
  if (opened) {
    await screenshot(page, 'bank-008-030-apply-entries-preflight.png', {
      projectName: project.name,
      testId,
      status: 'labor',
      purpose:
        'BANK-008 Apply Entries vor moeglicher Kreditorenzahlung read-only pruefen; keine Set-/Post-Application-Aktion.',
      knownLimitations: [
...
test('BANK-008 Single-Line Vendor Payment Journal Preflight ohne Buchung', async ({ page }) => {
  ...
  await screenshot(page, 'bank-008-020-payment-journal-preflight.png', {
    projectName: project.name,
    testId,
    status: draft.journalCheck.zeroIssuesTotalVisible ? 'labor' : 'rejected',
    purpose:
      'BANK-008 Payment Journal Preflight: Vendor 20000, Document 108204, BANK-RM-01, Amount und Journal Check.',
    knownLimitations: [
  ...
  const result = {
    ...
    caseId: 'BANK-008-SINGLE-LINE-VENDOR-PAYMENT-PREFLIGHT',
    ...
    cleanup,
    proved: [
      'Vendor Ledger Entry 108204 / First Up Consultants / Vendor 20000 was visible before Payment Journal preflight.',
      'A controlled Payment Journal line BANK008-108204 was attempted for Vendor 20000, BANK-RM-01 and Applies-to Doc. No. 108204.',
    ...
    changedFiles: [
      'playwright/projects/fibu-book5/evidence/bank-008/BANK-008-result.json',
      'playwright/projects/fibu-book5/evidence/bank-008/BANK-008-SINGLE-LINE-VENDOR-PAYMENT-PREFLIGHT.md',
      'playwright/projects/fibu-book5/evidence/bank-008/README.md'
    ],
    evidenceRefs: ['playwright/projects/fibu-book5/evidence/bank-008/'],
    ...
    nextStep: preflightPassed
      ...
      : 'BANK-009: fix the specific Payment Journal preflight blocker before any payment posting.'
  };

  await writeJsonEvidence(bankEvidencePath('BANK-008-result.json'), result);
  await writeTextEvidence(bankEvidencePath('BANK-008-SINGLE-LINE-VENDOR-PAYMENT-PREFLIGHT.md'), renderMarkdown(result));
  await writeTextEvidence(
    bankEvidencePath('README.md'),
    [
      '# BANK-008 Evidence Index',
      '',
      'Status: CRONUS-USA-Labor, Payment-Journal-Preflight, keine Zahlung, Cleanup, kein deutscher Finalnachweis.',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `010-vendor-ledger-document-108204-page-text.txt` | kompakter UI-Seitentext | Zielposten `108204` / Vendor `20000` / First Up Consultants | keine Zahlung | labor, preflight |',
      ...
      '| `BANK-008-result.json` | JSON-Ergebnis | strukturierter Preflight-/Cleanup-Befund | keine Postenspur aus neuer Zahlung | labor |',
      '| `BANK-008-SINGLE-LINE-VENDOR-PAYMENT-PREFLIGHT.md` | Lernzusammenfassung | Einzelzeilenroute und Buchungsgrenze | keinen deutschen Finalnachweis | labor |',
      ''
    ].join('\n')
  );

  expect(cleanup.cleaned, 'BANK-008 Draft-Zeile muss bereinigt werden.').toBe(true);
});
