...
- Anlagen: `FIXEDASSETS-001`/`FIXEDASSETS-002` sind Labor-Readiness; `FIXEDASSETS-003` oeffnet danach direkte Zielseiten read-only. Praktisch sichtbar sind Anlagenliste `5601`, AfA-Buecher `5611`, Einkaufsrechnungen `9308` und Anlagenposten `5604`; `FA Posting Groups` ueber direkte Page-ID `5606` ist rejected und braucht einen alternativen UI-/Tell-Me-Pfad. Der alte Testdatenbetrag `250.000` wurde auf das Buchziel `120.000 EUR` harmonisiert. Nicht geprueft sind Anlagenkarte `FA-CNC-01`, AfA-Buch `HGB` als Zielwert, FA Posting Group `MACHINES`, Aktivierung, AfA-Buchung, Anlagenpostenspur fuer `FA-CNC-01` und deutscher Anlagen-Endstand.
