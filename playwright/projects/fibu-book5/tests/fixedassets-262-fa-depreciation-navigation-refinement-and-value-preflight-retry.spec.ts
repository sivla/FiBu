...
const TEST_ID = 'fixedassets-262';
const CASE_ID = 'FIXEDASSETS-262-FA-DEPRECIATION-NAVIGATION-REFINEMENT-AND-VALUE-PREFLIGHT-RETRY';
const EXPECTED_INSTANCE = 'MCP_1_20260210';
const EXPECTED_COMPANY = 'RM-DEMO';
const SEARCH_TERM = 'Calculate Depreciation';
const TARGETS = {
  depreciationBook: 'HGB',
  postingDate: '30.06.2026',
  documentNo: 'FADEP-262-NO-OK',
...
test('FIXEDASSETS-262 refines navigation and fills Calculate Depreciation target values without OK', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-navigation-refinement-and-target-value-preflight-no-ok',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 30.06.2026, FADEP-262-NO-OK and FA-CNC-01 were visibly proven on the request page without OK.']
        : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-262-fa-depreciation-navigation-refinement-and-value-preflight-retry.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-262/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-262/FIXEDASSETS-262-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-262/FIXEDASSETS-262-TARGET-VALUE-PREFLIGHT-NO-OK.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-262/010-target-value-preflight.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-262/010-target-value-preflight-text.txt',
    ],
    statePatch: {
      current: {
        activeCase: allValuesProved
          ? 'FIXEDASSETS-263-FA-DEPRECIATION-OK-EXECUTION-GATE-REVIEW'
          : 'FIXEDASSETS-263-FA-DEPRECIATION-TARGET-VALUE-PREFLIGHT-BLOCKER-REVIEW',
        active_case_file: allValuesProved
          ? '.agent/state/cases/fixedassets-263-fa-depreciation-ok-execution-gate-review.json'
          : '.agent/state/cases/fixedassets-263-fa-depreciation-target-value-preflight-blocker-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-262-fa-depreciation-navigation-refinement-and-value-preflight-retry.json',
        nextStep: allValuesProved
          ? 'FIXEDASSETS-263: locally review whether a controlled OK execution is justified; do not execute OK yet.'
          : 'FIXEDASSETS-263: locally review the no-OK target-value preflight blocker before any further value write.',
      },
      coverage: {
        ...
        depreciationReadiness: allValuesProved
          ? 'FA-262 proved HGB, 30.06.2026, FADEP-262-NO-OK and FA-CNC-01 on the Calculate Depreciation request page without OK. OK remains locked pending local review.'
          : 'FA-262 could not prove all target request-page values without OK. OK remains locked pending blocker review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
    ...
    reason: allValuesProved
      ? 'Target request-page values were proven without OK after navigation refinement. Local review required before any execution.'
      : 'At least one target request-page value was not proven after navigation refinement. OK remains locked.',
    observed: {
      startedAt,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-262-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-262-TARGET-VALUE-PREFLIGHT-NO-OK.md'),
    [
      '# FIXEDASSETS-262 Navigation-Refinement und Zielwert-Preflight ohne OK',
      '',
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-262 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-262-result.json` | JSON-Ergebnis | Navigations-verfeinerter Zielwert-Preflight ohne OK | keine AfA-Ausfuehrung | labor |',
      '| `FIXEDASSETS-262-TARGET-VALUE-PREFLIGHT-NO-OK.md` | Lernzusammenfassung | Zielwerte, Grenzen und naechsten Gate-Schritt | keine Journalzeile | no-ok |',
