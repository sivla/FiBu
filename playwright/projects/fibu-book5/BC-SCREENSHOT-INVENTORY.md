...
Screenshot-QA zu TARGET-027D/D2/D3: Der Lauf hat eine echte Setup-Wirkung erzeugt, aber keinen fachlichen Erfolg. Die Seite 472 wurde korrekt erreicht und eine partielle `INLAND`/`VAT19`-Zeile ist nach Reopen sichtbar. Gleichzeitig zeigen die Bilder, dass die Werte durch die Tab-Route verrutscht sind: `19` steht als Beschreibung, `MwSt. %` bleibt nicht als `19` bewiesen, und die Konten `3806`/`1406` sind nicht sichtbar zugeordnet. Diese Evidence ist deshalb Blocker- und Lernmaterial, kein Buchbeweis fuer deutsche Umsatzsteuer. Der naechste Screenshot muss Page Inspection, Feld-/Control-Mapping oder eine saubere Korrektur-/Cleanup-Route zeigen.
