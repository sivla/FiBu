...
| Lagerplaetze/Bins | 13 | Bins fuer gesteuertes Warehouse | nicht aufgebaut; `WAREHOUSE-001` zeigt `FRA-ZL` read-only als Lagerort und Warehouse-Einstiege, aber keine aktivierte Bin-/Warehouse-Logik | nein | ja, `warehouse-001` | `planned-only / readiness` | P1 | Ohne Gate nur `WAREHOUSE-002` Buch-/Evidence-Sync; Bins und Warehouse-Aktivierung erst mit Gate. |
