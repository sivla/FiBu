...
const testId = 'bank-009';
const pageIdPaymentJournal = 256;
const paymentDocumentNo = 'BANK009-108204';
const postedPurchaseInvoiceNo = '108204';
const vendorNo = '20000';
const bankAccountNo = 'BANK-RM-01';
const paymentAmount = '2.151,46';
...
function compactPageText(text: string) {
  const interesting =
    /BANK009|108204|107196|20000|First Up|BANK-RM-01|Payment Journal|Vendor Ledger Entries|Detailed Vendor|Bank Account Ledger|G\/L Entries|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Amount|Amount \(\$\)|Bal\. Account|Applies-to|Apply Entries|Journal Check|Refresh|Post|Payment|Vendor|Remaining Amount|Open|Closed|Application|successful|posted|muss|must/i;
...
async function prepareDraft(page: Page) {
  ...
  await fillControl(initialControls[8], 'BANK-009 lab vendor payment 108204');
  ...
  return {
    ...
    amountVisible: values.some((value) => /2[.,]151[.,]46|2151[.,]46/i.test(value)) || /2[.,]151[.,]46|2151[.,]46/i.test(text),
...
async function openFilteredPageAndCapture(page: Page, target: TraceTarget) {
  ...
  await screenshot(page, target.imageFileName, {
    ...
    purpose: `BANK-009 UI-Postenspur fuer Kreditorenzahlung ${paymentDocumentNo}: ${target.id}.`,
    knownLimitations: [
  ...
  return {
    ...
    amountVisible: /2[.,]151[.,]46|2151[.,]46/i.test(text),
...
async function checkVendorEntryStillOpen(page: Page) {
  const target: TraceTarget = {
    ...
    imageFileName: 'bank-009-010-vendor-ledger-before-payment.png',
...
async function checkPaymentDocumentNotAlreadyPosted(page: Page) {
  const target: TraceTarget = {
    ...
    imageFileName: 'bank-009-005-existing-payment-document-guard.png',
    labelPattern: /Vendor Ledger Entries|Vendor Ledger Entry|Kreditorenposten/i
  };
  const trace = await openFilteredPageAndCapture(page, target);
  return {
...
async function captureApplyEntriesPreflight(page: Page) {
  ...
  const buttons = (await visibleButtonNames(page)).map(sanitizeEvidenceText);
  const opened =
    clicked && /Apply.*Entries|Apply Vendor Entries|Posten ausgleichen|108204|20000|Remaining Amount/i.test(text);
  ...
  await writeJsonEvidence(bankEvidencePath('030-apply-entries-preflight-controls.json'), {
    ...
    clicked,
    opened,
    documentVisible: /108204|20000/i.test(text),
    dangerousActionsVisible: /Set Applies-to ID|Post Application/i.test(`${text}\n${buttons.join('\n')}`),
    dangerousActionsClicked: false,
    buttons
  });
  if (opened) {
    await screenshot(page, 'bank-009-030-apply-entries-preflight.png', {
      projectName: project.name,
      testId,
      status: 'labor',
      purpose:
        'BANK-009 Apply Entries vor Kreditorenzahlung read-only pruefen; Applies-to ist ueber Journalfelder gesetzt, keine separate Apply-Aktion.',
      knownLimitations: [
...
async function postDraftOnce(page: Page) {
  ...
  await screenshot(page, 'bank-009-040-post-confirm-dialog.png', {
    projectName: project.name,
    testId,
    status: confirmationDialogVisible ? 'labor' : 'rejected',
    purpose: `BANK-009 Buchungsdialog vor der genau einmaligen Labor-Kreditorenzahlung ${paymentDocumentNo}.`,
    knownLimitations: [
  ...
  await screenshot(page, 'bank-009-050-post-result.png', {
    projectName: project.name,
    testId,
    status: confirmed ? 'labor' : 'rejected',
    purpose: `BANK-009 Zustand nach Bestaetigung der Labor-Kreditorenzahlung ${paymentDocumentNo}.`,
    knownLimitations: [
...
function renderMarkdown(result: Record<string, any>) {
  return [
    ...
    `| 20000 sichtbar | ${result.preflight.vendorVisible ? 'ja' : 'nein'} |`,
    `| BANK-RM-01 sichtbar | ${result.preflight.bankVisible ? 'ja' : 'nein'} |`,
    `| 108204 als Applies-to Doc. No. sichtbar | ${result.preflight.invoiceReferenceVisible ? 'ja' : 'nein'} |`,
    `| Amount 2.151,46 sichtbar | ${result.preflight.amountVisible ? 'ja' : 'nein'} |`,
...
test('BANK-009 Single-Line Vendor Payment buchen und Postenspur sichern', async ({ page }) => {
  ...
  expect(
    ...
    'BANK009-108204 darf vor diesem Lauf noch nicht als gebuchter Kreditorenposten sichtbar sein.'
  ).toBe(true);
  ...
  expect(vendorBefore.stillOpenEnoughForPayment, '108204 muss vor BANK-009 noch als offener Kreditorenposten sichtbar sein.').toBe(true);
  ...
  await screenshot(page, 'bank-009-020-payment-journal-preflight.png', {
    projectName: project.name,
    testId,
    status: draft.journalCheck.zeroIssuesTotalVisible ? 'labor' : 'rejected',
    purpose:
      'BANK-009 Payment Journal vor Labor-Kreditorenzahlung: 20000, 108204, BANK-RM-01, Amount und Journal Check.',
    knownLimitations: [
  ...
  if (!preflightPassed) {
    ...
    const blocker = {
      ...
      cleanup,
      posted: false,
      nextStep: 'BANK-009 Payment-Journal-Preflight gezielt korrigieren; bis dahin keine Kreditorenzahlung buchen.'
    };
    await writeJsonEvidence(bankEvidencePath('BANK-009-result.json'), blocker);
    await writeTextEvidence(bankEvidencePath('BANK-009-VENDOR-PAYMENT-BLOCKER.md'), renderMarkdown({
      environment: 'MCP_1_20260210',
      company: project.defaultCompany,
      paymentDocumentNo,
    ...
    throw new Error('BANK-009 Preflight nicht vollstaendig; keine Kreditorenzahlung gebucht.');
  }
  ...
  expect(posting.confirmed, 'BANK-009 muss genau einmal bestaetigt werden.').toBe(true);
  ...
  const traceTargets: TraceTarget[] = [
    {
      ...
      imageFileName: 'bank-009-060-vendor-ledger-invoice-after-payment.png',
      labelPattern: /Vendor Ledger Entries|Vendor Ledger Entry|Kreditorenposten/i
    },
    {
      ...
      imageFileName: 'bank-009-061-vendor-ledger-payment.png',
      labelPattern: /Vendor Ledger Entries|Vendor Ledger Entry|Kreditorenposten/i
    },
    {
      ...
      imageFileName: 'bank-009-062-detailed-vendor-ledger-payment.png',
    ...
    },
    {
      ...
      imageFileName: 'bank-009-063-bank-account-ledger-payment.png',
    ...
    },
    {
      ...
      imageFileName: 'bank-009-064-gl-entries-payment.png',
  ...
  const result = {
    ...
    proves: [
      ...
      '20000, 108204, BANK-RM-01, Betrag und Journal Check wurden vor der Buchung sichtbar geprueft.',
  ...
  await writeJsonEvidence(bankEvidencePath('BANK-009-result.json'), result);
  await writeTextEvidence(bankEvidencePath('BANK-009-VENDOR-PAYMENT.md'), renderMarkdown(result));
