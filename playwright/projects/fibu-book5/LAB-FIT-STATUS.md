...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort; `FRA-ZL` + `RESALE` wurde mit `Inventory Account = 14140` als CRONUS-Laborfit eingerichtet; Preview zeigt `Item Ledger Entry = 1` und `Value Entry = 1`; nach Laborbuchung ist ein Wertposten sichtbar | `masterdata-004`, `masterdata-005`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001` | Inventory-Posting-Setup-Luecke ist im Labor geschlossen; Wertposten zur Rechnung sichtbar; direkte Artikelpostenliste blieb mit Filter `Order No. = S-ORD101068` leer | Artikelposten und Wertposten in deutscher Zielcompany | Artikelposten read-only ueber `Find entries...` oder gebuchte Lieferung finden |
...
1. Auf gebuchter Rechnung `PS-INV103297` per `Find entries...` read-only die direkte Artikelpostenspur und Dimensionswirkung suchen.
2. Postenspur aus `UAT-O2C-001` als Anfaenger-Lernbild erklaeren: gebuchte Verkaufsrechnung, Debitorenposten, Sachposten, Wertposten und fehlender direkter Artikelpostenfilter.
