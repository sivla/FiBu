...
const CASE_ID = 'FIXEDASSETS-208-HGB-ACQ-COST-GL-INTEGRATION-SETUP-FIT';
const NEXT_CASE_ID = 'FIXEDASSETS-209-HGB-ACQ-COST-GL-INTEGRATION-SETUP-FIT-REVIEW';
const TEST_ID = 'fixedassets-208';
const EXPECTED_INSTANCE = 'MCP_1_20260210';
const EXPECTED_COMPANY = project.defaultCompany;
const TARGET_FIELD = 'G/L Integration - Acq. Cost';
...
async function findHgbCardFrame(page: Page) {
  for (const frame of page.frames()) {
    ...
    if (/\bHGB\b/i.test(text) && /Depreciation Book Card|G\/L Integration|Acq\. Cost/i.test(text)) {
      return frame;
    }
  }
...
async function findAcqCostCheckboxCandidate(page: Page) {
...
async function clickSelectedCheckbox(page: Page, checkboxIndex: number) {
      ...
      target.type === 'checkbox'
        ...
        : target.getAttribute('aria-checked') === 'true'
          ...
          : target.getAttribute('aria-checked') === 'false'
            ? false
            : null;
    target.setAttribute('data-fa208-target-checkbox', 'true');
  ...
  const locator = frame.locator('[data-fa208-target-checkbox="true"]');
...
async function writeResult(partial: Record<string, any>) {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-208-hgb-acq-cost-gl-integration-setup-fit-result',
    caseId: CASE_ID,
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-208-hgb-acq-cost-gl-integration-setup-fit.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-208/',
      '.agent/state/cases/fixedassets-208-hgb-acq-cost-gl-integration-setup-fit.json',
      '.agent/state/cases/fixedassets-209-hgb-acq-cost-gl-integration-setup-fit-review.json',
      '.agent/state/current.json',
      '.agent/state/coverage_state.json',
      '.agent/state/last_run_summary.json',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-208/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-208/FIXEDASSETS-208-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-208/FIXEDASSETS-208-learning.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-208/010-hgb-acq-cost-gl-integration-before-after.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-208/020-focused-card-text.txt',
    ],
    ...
    statePatch: {
      current: {
        ...
        active_case_file: '.agent/state/cases/fixedassets-209-hgb-acq-cost-gl-integration-setup-fit-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-208-hgb-acq-cost-gl-integration-setup-fit.json',
        requiresStrongModel: true,
        nextStep: 'FIXEDASSETS-209: locally review FA-208 setup-fit evidence before any Preview Posting-only retry.',
      },
      activeCase: {
        status: partial.resultStatus,
        lastResult: {
          status: partial.resultStatus,
          resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-208/FIXEDASSETS-208-result.json',
          summary: partial.summary,
        },
        nextSafeAction: 'FIXEDASSETS-209: local review; no Preview Posting or Post in FA-208.',
      },
    },
    ...partial,
  };

  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-208-result.json'), result);
  await writeTextEvidence(faEvidencePath('FIXEDASSETS-208-learning.md'), renderLearning(result));
  await writeTextEvidence(
    faEvidencePath('README.md'),
    [
      '# FIXEDASSETS-208 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-208-result.json` | JSON | Setup-Fit-Ergebnis, Safety-Flags, State-Patch-Plan | keine Preview-Zeilen, keine Buchung | `review-required` |',
      '| `010-hgb-acq-cost-gl-integration-before-after.json` | JSON | Vorher/Nachher-Kandidat fuer genau ein Checkbox-Feld | keine vollstaendige Tabellenlogik | `single-field-fit` |',
      '| `020-focused-card-text.txt` | Text | kompakter Kartenkontext | kein Rohdump | `compact` |',
      '| `FIXEDASSETS-208-learning.md` | Markdown | Lernwert und Grenzen | keinen deutschen Finalnachweis | `labor` |',
      '',
...
test('FIXEDASSETS-208 sets HGB G/L Integration - Acq. Cost on if uniquely accessible', async ({ page }) => {
  const startedAt = new Date().toISOString();
  const cardOpen = await openHgbCard(page);
  const context = await assertSandboxContext(page);
  const before = await findAcqCostCheckboxCandidate(page);
  const editMode = await ensureEditMode(page);
  const beforeAfterEditMode = await findAcqCostCheckboxCandidate(page);
  await writeTextEvidence(
    faEvidencePath('020-focused-card-text.txt'),
    await compactPageText(page, {
      include: [/Depreciation Book Card|HGB|G\/L Integration|Acq\. Cost|Acquisition|Error|Fehler/i],
      maxLines: 140,
      maxLineLength: 260,
    }),
  );
  ...
  if (beforeAfterEditMode.acceptedCount !== 1 || !beforeCandidate || typeof beforeChecked !== 'boolean') {
    ...
    summary = 'FA-208 blocked: target checkbox was not uniquely and safely identifiable.';
  } else if (beforeChecked === true) {
    resultStatus = 'already-fit';
    summary = 'FA-208 observed HGB G/L Integration - Acq. Cost already on; no setup change was needed.';
  } else {
    ...
    if (danger.postingDialogVisible || danger.dangerousConfirmVisible) {
      ...
      summary = 'FA-208 blocked after click attempt because a risky dialog/signal appeared.';
      ...
      summary = 'FA-208 blocked: target checkbox was visible but not clickable.';
    } else {
      resultStatus = 'setup-fit-applied';
      summary = 'FA-208 set HGB G/L Integration - Acq. Cost to on through one UI-first checkbox action; no Preview Posting or Post occurred.';
    }
  }

  await page.keyboard.press('Tab').catch(() => undefined);
  await page.waitForTimeout(1600);
  const after = await findAcqCostCheckboxCandidate(page);
  ...
  if (['setup-fit-applied', 'already-fit'].includes(resultStatus) && afterChecked !== true) {
    ...
    summary = 'FA-208 did not prove the target checkbox checked after the action.';
  }
  ...
  await writeJsonEvidence(faEvidencePath('010-hgb-acq-cost-gl-integration-before-after.json'), evidence);
  ...
  const result = await writeResult({
    ...
    bookImpact: ['already-fit', 'setup-fit-applied'].includes(resultStatus)
      ? 'Kapitel 21 kann den HGB-G/L-Integration-Acq.-Cost-Fit als CRONUS-USA-Labor-Setupvoraussetzung dokumentieren; Preview/Post bleiben separate Nachweise.'
      : 'Kapitel 21 darf den HGB-G/L-Integration-Acq.-Cost-Fit noch nicht behaupten; der Checkbox-Pfad bleibt Lern-/Blockerfall.',
    nextStep: 'FIXEDASSETS-209: locally review FA-208 setup-fit evidence before any Preview Posting-only retry.',
  });
