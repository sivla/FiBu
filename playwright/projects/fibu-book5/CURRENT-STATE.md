...
- `INTEGRATIONS-001` ist als Buch-/Readiness-Sync ohne BC-Lauf dokumentiert: Kapitel 29 hat jetzt eine Statusbox zum Labor-Nachweis in `RM-DEMO`. Das Buch trennt Standardnachweis, Fit-Gap, Extension/AppSource, API/Web Services, Power Platform, Power BI, UAT, Rollback und Betrieb. Der Lauf hat keine Extension installiert, keine API-/Connector-Einrichtung, kein Power-Platform-/Power-BI-Setup, keinen produktiven Integrationslauf und keine Setup-/Stammdaten-Aenderung erzeugt. Naechster Schritt ohne Gate ist `OPERATIONS-001-READINESS`.
