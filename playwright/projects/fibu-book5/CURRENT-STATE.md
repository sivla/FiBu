...
| Bank-/Journal-Readiness | `PAYMENTS-002` prueft Bankkonten, Cash Receipt Journal, Payment Journal und Apply-Entries-Pfade kontrolliert ohne Buchung. `PAYMENTS-003` legt `BANK-RM-01` idempotent als CRONUS-USA-Laborbankkonto an und zeigt es in Bank Accounts. `PAYMENTS-004` bestaetigt live `BANK-RM-01` und zeigt im Cash Receipt Journal Posting Date, Document Type/No., Account Type/No., Amount, Bal. Account, Apply Entries und Journal Check. `Preview Posting` ist dort nicht sichtbar. Es gibt weiterhin keine Zahlungsjournalzeile, keine Zahlung, keinen Ausgleich und keine Bankabstimmung |
...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. Sinnvollster naechster Lauf ist `PAYMENTS-005` als kontrollierter, bereinigbarer Cash-Receipt-Journal-Draft: eine eindeutig markierte Entwurfszeile fuer Debitor `D10000`, offene Rechnung `PS-INV103297`, Betrag und Gegenkonto `BANK-RM-01` vorbereiten, Journal Check/Ausgleichsbezug soweit sichtbar pruefen und den Entwurf wieder loeschen. Weiterhin keine Zahlung buchen, keinen Ausgleich setzen und keine Bankabstimmung starten. Begruendung: `PAYMENTS-001` beweist die offenen Ausgangsposten, `PAYMENTS-002` beweist Journal-/Apply-Pfade, `PAYMENTS-003` beweist das Laborbankkonto `BANK-RM-01`, und `PAYMENTS-004` beweist die Feld-/Preflight-Readiness im Cash Receipt Journal; offen ist jetzt der konkrete Entwurfs- und Ausgleichsfit.
...
8. `PAYMENTS-001` beweist offene Debitoren-/Kreditorenposten als Startpunkt fuer OP-Ausgleich. `PAYMENTS-002` beweist Bankkonten-, Cash-Receipt-Journal-, Payment-Journal- und Apply-Entries-Readiness. `PAYMENTS-003` beweist `BANK-RM-01` als neues CRONUS-USA-Laborbankkonto. `PAYMENTS-004` beweist Cash Receipt Journal Pflichtfelder, Gegenkonto-/Ausgleichshinweise und Journal Check read-only. Offen bleiben Zahlungsjournalzeile, Betrag/Betragsrichtung, Ausgleich, Bank Account Posting Group/Sachkonto-Fit, Zahlung und Bankabstimmung.
