...
4. Inventory einfach vertiefen: `INVENTORY-002` ist als read-only Zahlenbericht erledigt, `INVENTORY-003` erklaert den negativen `RM-M100`-Wert, `INVENTORY-004` legt den Zielbestandsplan fest, `INVENTORY-005` bestaetigt `Item Journals` als Einstieg, `INVENTORY-006` bestaetigt den Ziel-Draft mit Dimension und `INVENTORY-007` bestaetigt Journal Check ohne Issues; naechster Schritt ist nach bewusstem Laborentscheid die kontrollierte positive `RM-M100`-Bestandsbewegung.
...
Fuehre `INVENTORY-008` als kontrollierte Laborbuchung aus: Page `40`/`Item Journals`, Zielzeile `RM-M100 +2` in `FRA-ZL`, Unit Cost `42.000,00` und `Journal Check` mit `1 Lines checked`/keinen Issues sind belegt. Nach bewusstem Projektentscheid genau einmal buchen und danach Artikelposten, Wertposten, Sachposten sowie `Inventory Valuation` dokumentieren. Keine neue O2C-/P2P-Buchung, keine Warehouse-Aktivierung, kein deutscher Finalnachweis.
```
