...
function foundationDecisionReady() {
  ...
  return !/template\/no-evidence|pending-target075-evidence/i.test(text) && text.includes('PWS-MD-002');
}
...
const blockedBy = [
  foundationReady ? '' : 'foundation-readiness-decision-missing-or-not-finalized-for-pws-md-002',
    ...
    JSON.stringify(
      {
        ...
        nextStep: foundationReady
          ? 'When live gate is open, run PWS-MD-002 only with --live-approved as read-first/no-write.'
          : 'Run TARGET-075, write FOUNDATION-READINESS-DECISION.md and include PWS-MD-002 before vendor context read-first proof.'
      },
      null,
      2
    )
...
if (authStatus.canUseStoredAuth !== true) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-002-auth-state-guard',
        canRun: false,
        liveActionsExecuted: false,
        businessCentralOpened: false,
        playwrightLiveRunExecuted: false,
        blockedBy: authStatus.blockedBy ?? ['stored-auth-not-usable'],
        nextStep: authStatus.nextStep ?? 'Refresh Playwright auth before running PWS-MD-002.'
      },
      null,
      2
    )
