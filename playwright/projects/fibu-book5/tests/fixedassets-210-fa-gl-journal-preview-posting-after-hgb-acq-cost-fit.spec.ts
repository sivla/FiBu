...
const CASE_ID = 'FIXEDASSETS-210-FA-GL-JOURNAL-PREVIEW-POSTING-AFTER-HGB-ACQ-COST-FIT';
const NEXT_CASE_ID = 'FIXEDASSETS-211-FA-GL-JOURNAL-PREVIEW-POSTING-AFTER-HGB-FIT-REVIEW';
const TEST_ID = 'fixedassets-210';
const EXPECTED_INSTANCE = 'MCP_1_20260210';
const EXPECTED_COMPANY = project.defaultCompany;
const FA_GL_JOURNAL_PAGE_ID = 5628;

const HGB_FIT_EVIDENCE = 'playwright/projects/fibu-book5/evidence/fixedassets-209/FIXEDASSETS-209-result.json';
...
function statePatch(status: 'observed' | 'blocked', summary: string) {
  return {
    current: {
      ...
      active_case_file: '.agent/state/cases/fixedassets-211-fa-gl-journal-preview-posting-after-hgb-fit-review.json',
      lastReferenceCase: CASE_ID,
      lastReferenceCaseFile: '.agent/state/cases/fixedassets-210-fa-gl-journal-preview-posting-after-hgb-acq-cost-fit.json',
      requiresStrongModel: true,
      nextStep: 'FIXEDASSETS-211: locally review FA-210 Preview Posting result before any Post, setup change or further journal edit.',
    },
    activeCase: {
      status: status === 'observed' ? 'observed-preview-posting-after-hgb-fit' : 'blocked-preview-posting-after-hgb-fit',
      lastResult: {
        status,
        resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-210/FIXEDASSETS-210-result.json',
        summary,
      },
      nextSafeAction: 'Run FIXEDASSETS-211 local review; do not post.',
    },
  };
}

test('FIXEDASSETS-210 retries Preview Posting after HGB Acq. Cost G/L Integration setup fit', async ({ page }) => {
  ...
  const hgbFitEvidence = JSON.parse(fs.readFileSync(HGB_FIT_EVIDENCE, 'utf8'));
  const hgbFitAccepted =
    hgbFitEvidence?.resultStatus === 'accepted-labor-setup-fit' &&
    hgbFitEvidence?.decision?.nextCase === CASE_ID;

  try {
    if (!hgbFitAccepted) {
      blockedBy.push('FA-209 did not accept HGB Acq. Cost G/L Integration setup fit for this case.');
    }
    ...
    if (!context.environmentInUrl || (!context.companyInUrl && !context.companyInText) || context.wrongEnvironmentVisible) {
      ...
      if (blockedBy.length === 0 && hgbFitAccepted) {
  ...
  const summary =
    status === 'observed'
      ? `FA-210 clicked only the exact Preview Posting menuitem after proving FA Posting Type = Acquisition Cost; outcome=${outcome}; prior FA Posting Type blocker gone=${priorFaPostingTypeBlockerGone}.`
      : `FA-210 stayed safe but did not complete the guarded Preview Posting retry: ${blockedBy.join(' | ')}`;
  const patch = statePatch(status, summary);

  await writeJsonEvidence(faEvidencePath('010-preflight-after-hgb-fit.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-210-preflight-fa-posting-type',
    caseId: CASE_ID,
    context,
    frameUrl,
    target: TARGET,
    hgbFitEvidence: {
      resultStatus: hgbFitEvidence?.resultStatus,
      accepted: hgbFitEvidence?.decision?.accepted,
      nextCase: hgbFitEvidence?.decision?.nextCase,
    },
    hgbFitAccepted,
    snapshotBefore,
    helperBefore,
  ...
  await writeJsonEvidence(faEvidencePath('020-preview-posting-after-hgb-fit.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-210-preview-posting-after-correction',
    caseId: CASE_ID,
    buttonDiscovery,
    clickedSplitButton,
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-210-preview-posting-after-hgb-fit-result',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(hgbFitAccepted ? ['FA-209 accepted HGB G/L Integration - Acq. Cost as fit before Preview Posting.'] : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-210-fa-gl-journal-preview-posting-after-hgb-acq-cost-fit.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-210/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/010-preflight-after-hgb-fit.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/020-preview-posting-after-hgb-fit.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/030-preview-posting-result-text.txt',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/FIXEDASSETS-210-learning.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-210/FIXEDASSETS-210-result.json',
    ],
    warnings: [
      'Preview Posting-only. Do not treat as posting permission.',
      'Posting remains locked until FIXEDASSETS-211 local review.',
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-210-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-210-learning.md'),
    [
  ...
  expect(result.flags.hgbFitAccepted).toBe(true);
