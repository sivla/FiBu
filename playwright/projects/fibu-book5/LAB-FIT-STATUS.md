...
| Stammdaten-Audit | passt | Customers, Items, Locations, Dimensions, General/VAT/Inventory Posting Setup gesichtet | `evidence/masterdata-001/`, `img/masterdata-001-*` | Ist-Stand als Laborbasis bekannt | Audit in deutscher Zielcompany wiederholen | bei neuen Modulen Audit erweitern |
...
| Posting Groups | labor | Debitor-/Artikel-Posting-Fit fuer CRONUS-Techniklauf hergestellt | `masterdata-006`, `UAT-O2C-001`, `MICROSOFT-DOC-VALIDATION.md` | Auftrag ist technisch anlegbar; Kontenfindung noch nicht per Preview/Posten bewiesen | deutsche Posting Groups, General Posting Setup und Inventory Posting Setup | `Preview Posting` als nicht buchenden Laborlauf untersuchen |
...
| Order-to-Cash | labor | Sales Orders gesucht, Auftrag per API erzeugt, Kopf/Zeile fotografiert, Dimensionen geprueft, Cleanup nachgewiesen | `img/uat-o2c-001-*`, `evidence/uat-o2c-001/` | Klickpfad, EUR, Menge, Lagerort, Preis und Dimension funktionieren; Steuer bleibt 0 % | deutscher O2C mit 19 % USt, Buchungsvorschau, Buchung und Postenspur | `Preview Posting` erforschen und als Lernfall dokumentieren |
...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort | `masterdata-004`, `masterdata-005`, `UAT-O2C-001` | keine Buchung, daher keine Artikelposten/Wertposten | Artikelposten und Wertposten nach Lieferung/Fakturierung | erst nach Steuer-/Preview-Entscheidung |
...
1. `Preview Posting` fuer `UAT-O2C-001` als nicht buchenden Laborlauf untersuchen.
2. Dabei dokumentieren, ob BC wegen Sales Tax, Lagerbestand, Posting Groups oder Dimensionen warnt/blockiert.
3. Danach entscheiden: Steuer-/VAT-Setup weiter vertiefen oder mit P2P-Stammdaten beginnen.
