...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an, aber API-basiert und damit nicht als Buch-Klickpfad; `PAYMENTS-004` bestaetigt live Sichtbarkeit; `PAYMENTS-005` nutzt `BANK-RM-01` in einer UI-Draft-Zeile; `PAYMENTS-006` zeigt den fehlenden `Bank Account Posting Group`-Fit; `PAYMENTS-007` fittet `BANK-RM-01` per UI auf `CHECKING`; `PAYMENTS-008` zeigt den Draft nach Refresh ohne Journal-Check-Issues; `PAYMENTS-009` zeigt Apply Entries read-only aus dem Draft; `PAYMENTS-010` zeigt Post-Dialog mit `Ja`/`Nein` und Abbruch | ja | ja, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007`, `payments-008`, `payments-009`, `payments-010` | `partial` | P4 | Bankkonto-Posting-Fit, Journal Check, Apply-Readiness und Buchungsdialog-Risiko sind im Labor erledigt; Zahlung/Ausgleich/Bankwirkung bleiben gesperrt bis ausdrueckliche Buchungsfreigabe. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder; `PAYMENTS-005` belegt UI-Draft; `PAYMENTS-006` zeigt Amount-Format und Bank-Posting-Group-Blocker; `PAYMENTS-007` fittet `BANK-RM-01 = CHECKING`; `PAYMENTS-008` zeigt einen nicht buchenden Draft mit `Journal Check = 0 Issues`; `PAYMENTS-009` zeigt Apply Entries read-only aus dem Draft und keine direkte Preview-Posting-Aktion; `PAYMENTS-010` zeigt den Post-Bestaetigungsdialog mit `Ja`/`Nein` und Abbruch; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007`, `payments-008`, `payments-009`, `payments-010` | `partial` | P4 | `PAYMENTS-011` nur mit ausdruecklicher Freigabe als kontrollierte Laborzahlung; sonst Payments-Buchtext/Evidence-Pack abrunden. |
...
1. Aktuellen O2C-Laborbeleg nicht erneut buchen; `REPORTING-002` bis `REPORTING-005` sind erledigt als Sichtbarkeits-/Negativbefunde. Naechster Reporting-Schritt ist ein alternativer UI-Einstieg ueber Berichtssuche/`Analysis by Dimensions` oder Data Analysis Mode auf `G/L Entries`; ein gezielter Analysis-View-Setup-Fit fuer `PRODUCTLINE`/`CHANNEL` braucht eigene Freigabe.
2. P2P-Laborbuchung ist erledigt: `106049` -> `108219`. Nicht erneut buchen; `PAYMENTS-001` hat offene Posten als Startpunkt belegt, `PAYMENTS-002` hat Journal-/Apply-Readiness belegt, `PAYMENTS-003` hat `BANK-RM-01` als Laborbankkonto angelegt und `PAYMENTS-004` hat Cash Receipt Journal Pflichtfelder/Journal Check read-only bestaetigt. `PAYMENTS-005` hat die bereinigbare UI-Draft-Zeile erzeugt; `PAYMENTS-006` klaert das Amount-Format und legt den Bank-Posting-Group-Blocker offen; `PAYMENTS-007` fittet das Bankkonto; `PAYMENTS-008` zeigt `Journal Check = 0 Issues`; `PAYMENTS-009` zeigt Apply Entries read-only und keine direkte Preview-Posting-Aktion; `PAYMENTS-010` zeigt den Post-Dialog und Abbruch. Naechster P2P-/O2C-naher Schritt ist nur nach ausdruecklicher Freigabe eine kontrollierte Laborzahlung, sonst Doku/Evidence abrunden.
...
| Welche Reporting-Auswertung ist der erste Buchnachweis: `Income Statement`, `Revenue`, Analysis View, `Analysis by Dimensions` oder G/L Entries Data Analysis? | Bestimmt Screenshots und Anfaengererklaerung. `REPORTING-005` hat den einfachen `Dimensions - Detail`-Tell-Me-Pfad nicht sichtbar erreicht. | offen |
