...
| TARGET-036B Debitor/Kreditor/Artikel `Neu` | Kein separater Template-Auswahldialog; `Neu` oeffnet direkt Karte mit Aktion `Vorlage anwenden` | mittel, weil eine spaetere Vorlage oder Pflichtfeldbearbeitung fachlich wirksam sein kann | Karte nur lesen, Screenshot erstellen, ohne Feldwerte und ohne `Vorlage anwenden` zurueck zur Liste | `evidence/target-036b-customer-vendor-item-template-dialog-discovery/` | Keine Vorlage angewendet, kein Debitor/Kreditor/Artikel gespeichert, keine Posting-/VAT-/Number-Series-Reife bewiesen |
