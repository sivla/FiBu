...
| Entscheidung | Kapitel 40 trennt Quellen, Labor-Evidence und deutschen Finalnachweis. `Q10` Shopify bleibt nur Out-of-Scope-Marker. `PAYMENTS-012` ist inzwischen erledigt; naechster sicherer Block ist `REPORTING-011-ANALYSIS-VIEW-FIT`. |
...
| Entscheidung | Kapitel 39 trennt Templates, Handover und echte Evidence. `SOURCES-001` hat den Folgeblock inzwischen erledigt; `PAYMENTS-012` ist inzwischen erledigt; naechster sicherer Block ist `REPORTING-011-ANALYSIS-VIEW-FIT`. |
...
| Entscheidung | Kapitel 38 trennt jetzt Index, Zielpfad, QA-Rahmen, echte Klickpfad-Evidence, Labor-Nachweis und offenen deutschen Finalnachweis. `SOURCES-001` hat Kapitel 40 inzwischen synchronisiert; `PAYMENTS-012` ist inzwischen erledigt; naechster sicherer Block ist `REPORTING-011-ANALYSIS-VIEW-FIT`. |
...
| Status | offen |
| Projekt | fibu-book5 |
| Testfall | `REPORTING-002`, `REPORTING-003`, `REPORTING-004`, `REPORTING-005`, `REPORTING-006`, `REPORTING-007`, `REPORTING-008`, `REPORTING-009`, `GOVERNANCE-006` |
...
| Entscheidung | Buch ergaenzen: Postendimension und Reportingauswertung sind zwei Nachweisebenen. Fuer den finalen Reportingbeweis braucht es jetzt einen kontrollierten Analysis-View-Fit fuer `PRODUCTLINE`/`CHANNEL`; vorhandene Revenue-Views, gefilterte `G/L Entries` und reine Tell-Me-Treffer duerfen nicht ungeprueft als Produktlinienauswertung ausgegeben werden. `GOVERNANCE-006` hat diesen Hebel fuer genau den naechsten Lauf freigegeben; der Lauf muss danach auf `done-labor`, `rejected` oder `locked` zuruecksetzen. |
