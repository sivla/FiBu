...
| Locations / Lagerorte | 15 | Inventory/Warehouse | Lagerorte und Warehouse-Schalter unterscheiden | Code, Name; Warehouse-Schalter wie Require Receive/Shipment und Bin Mandatory bleiben eigene Gates | Neu/New, Edit, Lagerort-Menue | Stammdatenwirkung; Warehouse-/Postingwirkung erst nach eigener Einrichtung | TARGET-035, TARGET-036A | `tested-effective-action`, `reopen-proof`, `not-warehouse-ready` | TARGET-036B |
...
| `RO-W1-POSTING-GROUPS` | Posting Groups / Posting Setup | 4 | Buchungsgruppen- und Matrixlisten fuer Kontenfindung vorbereiten | `matrix-route-parked-blocked-not-proven`; `INLAND` und `WAREN` sind nach Reopen sichtbar, Page 314 zeigt die Zielspalten fuer `4400` und `5400`, aber `Liste bearbeiten` + `Neu` sowie `Strg+Einfuegen` erzeugten keine vertrauenswuerdige Eingabezeile. TARGET-032E zeigt zusaetzlich: `Weitere Optionen` liefert keinen Kartenpfad, `Ctrl+Alt+F1` ist hier kein stabiler Page-314-Beweis, und Page 315 ist `MwSt.-Posten`, nicht die Buchungsmatrix-Karte. TARGET-032F parkt diese Route; TARGET-032G bestaetigt, dass Artikel-WAREN sie nicht entsperrt. TARGET-032H zeigt Page 314 read-only mit `Neu`, `Liste bearbeiten`, `Kopieren...`, `Weitere Optionen`, Spaltenmenues und Reopen-Proof; der Routenkandidat ist schwach und braucht TARGET-032I statt direktem Schreiben. Keine Posting-Readiness. |
