...
const CASE_ID = 'TARGET-027D13C-VAT-MATRIX-ROW-INLINE-MENU-CLEANUP-ROUTE';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const EVIDENCE_ID = 'target-027d13c-vat-matrix-row-inline-menu-cleanup-route';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-027D13C-result.json');
...
async function findRowInlineMenuButton(page: Page, row: UiEntry | null) {
  if (!row) return null;
  const rowCenterY = row.y + row.height / 2;
  const leftLimit = Math.max(0, row.x - 20);
  const rightLimit = row.x + Math.min(row.width, 420);
  for (const frame of page.frames()) {
    const candidates = await frame
      .evaluate(
        ({ rowCenterY, leftLimit, rightLimit }) => {
          return [...document.querySelectorAll<HTMLElement>('button,[role="button"],a,[role="menuitem"]')]
            .map((element) => {
              const rect = element.getBoundingClientRect();
              ...
              const nearRow = Math.abs((rect.y + rect.height / 2) - rowCenterY) <= 26;
              const nearLeft = rect.x >= leftLimit && rect.x <= rightLimit;
              const labelLooksInline =
                /Weitere Optionen anzeigen|Weitere Optionen|Show more options|More options|Open menu|Zeilenaktionen|row/i.test(joined) ||
                text === '...' ||
                text === '⋮' ||
                text === '…' ||
                ariaLabel === '...' ||
                title === '...';
              if (!visible || !nearRow || !nearLeft || !labelLooksInline) return null;
              return {
                text,
                role: element.getAttribute('role') || element.tagName.toLowerCase(),
                tag: element.tagName.toLowerCase(),
                ...
                width: Math.round(rect.width),
                height: Math.round(rect.height),
                source: 'row-inline-menu-candidate'
              };
            })
            .filter(Boolean)
            .sort((left, right) => {
              const dy = Math.abs((left!.y + left!.height / 2) - rowCenterY) - Math.abs((right!.y + right!.height / 2) - rowCenterY);
              if (dy !== 0) return dy;
      ...
      .catch(() => []);
    const first = candidates[0] as UiEntry | undefined;
    if (first) return { frame, entry: first };
  }
  return null;
}

async function openRowInlineMenu(page: Page, row: UiEntry | null) {
  const candidate = await findRowInlineMenuButton(page, row);
  if (!candidate) return { clicked: false, by: 'not-found', entry: null };
  await page.mouse.click(candidate.entry.x + Math.max(4, Math.floor(candidate.entry.width / 2)), candidate.entry.y + Math.max(4, Math.floor(candidate.entry.height / 2)));
  await page.waitForTimeout(1200);
  return { clicked: true, by: 'row-inline-geometry', entry: candidate.entry };
}
...
test('TARGET-027D13C tests row-inline cleanup menu for incomplete INLAND/VAT19 row', async ({ page }) => {
  await fs.mkdir(EVIDENCE_DIR, { recursive: true });
  ...
  const beforeText = await capture(page, 'target-027d13c-010-before-row-inline-cleanup', 'Before row-inline cleanup route.', {
  ...
  await capture(page, 'target-027d13c-020-after-row-focus', 'After focusing the INLAND/VAT19 row before row-inline cleanup.', {
    status: 'after-row-focus',
    row
  });

  const rowInlineMenu = await openRowInlineMenu(page, row);
  actionsTaken.push(`Opened row-inline ellipsis/menu: ${rowInlineMenu.clicked ? rowInlineMenu.by : 'not-found'}.`);
  if (!rowInlineMenu.clicked) {
    blockedBy.push('No visible row-inline ellipsis/menu candidate found directly on the INLAND/VAT19 row.');
  }
  const deleteAction = await findVisibleDeleteAction(page);
  if (!deleteAction) {
    blockedBy.push('No visible row-inline delete action found after opening the row-inline menu.');
  }
  await capture(page, 'target-027d13c-030-row-inline-menu-open', 'Visible row-inline menu context before cleanup click.', {
  ...
  if (deleteAction && rowInlineMenu.clicked && blockedBy.length === 0) {
    await deleteAction.locator.click({ timeout: 4000 }).catch(async () => deleteAction.locator.click({ timeout: 4000, force: true }));
    actionsTaken.push('Clicked the visible row-inline delete action after fresh target-row proof.');
    await page.waitForTimeout(1500);
    dialog = await getVisibleDialogText(page);
    await capture(page, 'target-027d13c-040-before-confirm-dialog-state', 'After row-inline delete click, capture dialog state before confirmation.', {
  ...
  await openMatrix(page);
  const reopenText = await capture(page, 'target-027d13c-050-reopen-after-row-inline-cleanup-proof', 'Reopen proof after row-inline cleanup route.', {
  ...
  const resultStatus = blockedBy.length
    ? 'blocked-row-inline-cleanup-route'
    : !rowStillVisible
      ? 'observed-row-inline-cleanup-row-absent-after-reopen'
  ...
  const nextCase = cleanupDeleteExecuted
    ...
    : 'TARGET-027D13D-VAT-MATRIX-CLEANUP-ALTERNATIVE-ROUTE-DECISION';

  const result = {
    schemaVersion: 1,
    purpose: 'autopilot-result-normalized',
    caseId: CASE_ID,
    source: 'playwright-universaarl-vat-matrix-row-inline-menu-cleanup-route',
    resultStatus,
    ...
    actionsTaken,
    actionsNotTaken: [
      'Toolbar/More Options delete route was not repeated.',
    ...
    proved: [
      ...
      rowInlineMenu.clicked
        ? 'A row-inline menu candidate was clicked at the INLAND/VAT19 row level.'
        : 'No row-inline menu candidate was found at the INLAND/VAT19 row level.',
      dialog.visible
        ? 'The row-inline delete route opened a confirmation dialog that was captured before confirmation.'
        : 'The row-inline delete route did not expose a dialog in the observed wait window.',
    ...
    screenshots: [
      'target-027d13c-010-before-row-inline-cleanup.png',
      'target-027d13c-020-after-row-focus.png',
      'target-027d13c-030-row-inline-menu-open.png',
      'target-027d13c-040-before-confirm-dialog-state.png',
      'target-027d13c-050-reopen-after-row-inline-cleanup-proof.png'
    ],
    evidenceRefs: [
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-027D13C-result.json`,
    ...
    flags: {
      ...
      noToolbarDeleteRepeat: true
    },
    nextStepDecisionCard: {
      currentCase: CASE_ID,
      plannedNextCaseBeforeReview: CASE_ID,
      lastEvidenceSummary: 'D13 proved that the toolbar/menu delete route can show the expected dialog and still leave the INLAND/VAT19 row visible after reopen.',
      isPlannedNextCaseStillSensible: true,
      reason: 'D13C tests a genuinely different row-inline UI surface visible on the target row instead of repeating the toolbar/menu delete route.',
      lookaheadReviewed: [
        {
          caseId: nextCase,
          ...
          reason: cleanupDeleteExecuted
            ...
            : 'Row-inline cleanup did not prove row absence or stopped before confirmation.'
        },
        {
      ...
      queueChangesMade: [
        `Select ${nextCase} based on D13C row-inline cleanup result.`
      ],
      ...
      whySelectedNextCaseIsBest: cleanupDeleteExecuted
        ...
        : 'The row-inline route did not produce trusted row-absence proof, so an alternative-route decision is safer than further setup.',
      risksBeforeNextCase: [
        'Do not recreate the VAT matrix row in the same case.',
        'Do not repeat toolbar delete or row-inline delete if this route fails.',
    ...
    statePatch: {
      current: {
      activeArea: cleanupDeleteExecuted ? 'universaarl-vat-matrix-recreate-after-cleanup' : 'universaarl-vat-matrix-cleanup-alternative-route-decision',
        activeCase: nextCase,
        active_case_file: cleanupDeleteExecuted
          ? '.agent/state/cases/target-027d14-vat-matrix-recreate-after-cleanup.json'
          : '.agent/state/cases/target-027d13d-vat-matrix-cleanup-alternative-route-decision.json',
        ...
        nextStep: cleanupDeleteExecuted
          ...
          : 'Review row-inline cleanup blocker and choose a non-delete alternative route before any further VAT matrix setup work.'
      },
      lastRunSummary: {
        runId: CASE_ID,
        caseId: CASE_ID,
        ...
        instance: EXPECTED_INSTANCE,
        company: TARGET_COMPANY,
        resultPath: `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-027D13C-result.json`,
        summary: cleanupDeleteExecuted
          ...
          : 'D13C did not produce trusted row-inline cleanup proof; alternative route decision is required.',
        nextCase
      }
    },
    nextCase,
    reason: cleanupDeleteExecuted
      ...
      : 'Stop or review because row-inline cleanup did not prove the expected row absence.'
  };
