...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. `PAYMENTS-009` hat die nicht buchende Apply-Readiness fuer den zahlungsreifen Cash-Receipt-Draft erledigt: Rechnungsbezug und Apply-Kontext sind sichtbar, `Journal Check = 0 Issues`, aber `Preview Posting` ist nicht direkt sichtbar und es wurde nichts gebucht. Sinnvollster naechster Lauf ist `PAYMENTS-010` als kontrollierter Vorbereitungs-/Freigabecheck vor einer moeglichen Laborzahlung: Buchungsdialog-/Preview-Risiko klaeren, genaue Zahlungsoption definieren, Sicherheitskriterien fuer genau eine spaetere Zahlung formulieren und erst nach ausdruecklicher Freigabe buchen.
