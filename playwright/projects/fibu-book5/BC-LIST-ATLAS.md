...
| `RO-W1-POSTING-GROUPS` | Posting Groups / Posting Setup | 4 | Buchungsgruppen- und Matrixlisten fuer Kontenfindung vorbereiten | `matrix-route-blocked-park-decision-needed`; `INLAND` und `WAREN` sind nach Reopen sichtbar, Page 314 zeigt die Zielspalten fuer `4400` und `5400`, aber `Liste bearbeiten` + `Neu` sowie `Strg+Einfuegen` erzeugten keine vertrauenswuerdige Eingabezeile. TARGET-032E zeigt zusaetzlich: `Weitere Optionen` liefert keinen Kartenpfad, `Ctrl+Alt+F1` ist hier kein stabiler Page-314-Beweis, und Page 315 ist `MwSt.-Posten`, nicht die Buchungsmatrix-Karte. |
