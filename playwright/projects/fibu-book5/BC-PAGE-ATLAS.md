...
| `RO-W1-POSTING-GROUPS` | Posting Groups / Posting Setup | `universaarl-readonly-observed` | TARGET-028 erfasst General Posting Setup `314`, Customer Posting Groups `110`, Vendor Posting Groups `111`, Inventory Posting Setup `5826` und VAT Posting Setup `472` read-only; TARGET-032A erfasst zusaetzlich Page `312` Geschaeftsbuchungsgruppen, Page `313` Produktbuchungsgruppen und Page `314` Buchungsmatrix als direkte Read-only-Routen | keine neue Gruppe, keine Matrixaenderung, keine Setup-Bereitschaft aus leerer Liste ableiten |
...
| Gen. Business Posting Groups | `312` | Foundation/Posting Groups | TARGET-032A oeffnet Geschaeftsbuchungsgruppen read-only in `playthru` / `UNIVERSAARL-DE`; `Neu`, `Liste bearbeiten`, `Code`, `Beschreibung` sind sichtbar | `evidence/target-032a-general-posting-groups-source-and-ui-discovery/` | keine Gruppe `INLAND`, keine Buchungsmatrix, keine Buchung |
| Gen. Product Posting Groups | `313` | Foundation/Posting Groups | TARGET-032A oeffnet Produktbuchungsgruppen read-only; `Neu`, `Liste bearbeiten`, `Code`, `Beschreibung` und Infotext zur Kategorisierung sind sichtbar | `evidence/target-032a-general-posting-groups-source-and-ui-discovery/` | keine Gruppe `WAREN`, keine Buchungsmatrix, keine Buchung |
| General Posting Setup | `314` | Foundation/Posting Groups | TARGET-028/TARGET-032A oeffnen Buchungsmatrix Einrichtung read-only in `playthru` / `UNIVERSAARL-DE`; Geschaeftsgruppe, Produktgruppe, Warenverkaufskonto und Wareneinkaufskonto sind sichtbar | `evidence/target-028-posting-groups-preflight/`, `evidence/target-032a-general-posting-groups-source-and-ui-discovery/` | keine `INLAND`/`WAREN`-Zeile; keine Kontenrichtigkeit, keine Preview, keine Buchung |
