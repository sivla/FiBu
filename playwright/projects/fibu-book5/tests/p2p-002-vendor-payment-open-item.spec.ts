...
const testId = 'p2p-002';
const pageIdPaymentJournal = 256;
const paymentDocumentNo = 'PAYP2P-108219';
const postedPurchaseInvoiceNo = '108219';
const vendorNo = 'K10000';
const bankAccountNo = 'BANK-RM-01';
const paymentAmount = '25.000,00';
...
function compactPageText(text: string) {
  const interesting =
    /PAYP2P|108219|K10000|BANK-RM-01|Payment Journal|Vendor Ledger Entries|Detailed Vendor|Bank Account Ledger|G\/L Entries|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Amount|Amount \(\$\)|Bal\. Account|Applies-to|Apply Entries|Journal Check|Refresh|Post|Payment|Vendor|Stahlwerk|Remaining Amount|Open|Closed|Application|successful|posted|muss|must/i;
...
async function prepareDraft(page: Page) {
  ...
  await fillControl(initialControls[8], 'P2P-002 lab vendor payment invoice 108219');
  ...
  return {
    ...
    amountVisible: values.some((value) => /25[.,]?000|25000/i.test(value)) || /25[.,]?000|25000/i.test(text),
...
async function openFilteredPageAndCapture(page: Page, target: TraceTarget) {
  ...
  await screenshot(page, target.imageFileName, {
    ...
    purpose: `P2P-002 UI-Postenspur fuer Kreditorenzahlung ${paymentDocumentNo}: ${target.id}.`,
    knownLimitations: [
  ...
  return {
    ...
    amountVisible: /25[.,]000|25000/i.test(text),
...
async function checkVendorEntryStillOpen(page: Page) {
  const target: TraceTarget = {
    ...
    imageFileName: 'p2p-002-010-vendor-ledger-before-payment.png',
...
async function captureApplyEntriesPreflight(page: Page) {
  ...
  const buttons = (await visibleButtonNames(page)).map(sanitizeEvidenceText);
  const opened =
    clicked && /Apply.*Entries|Apply Vendor Entries|Posten ausgleichen|108219|K10000|Remaining Amount/i.test(text);
  await writeTextEvidence(p2pEvidencePath('030-apply-entries-preflight-page-text.txt'), compactPageText(text));
  await writeJsonEvidence(p2pEvidencePath('030-apply-entries-preflight-controls.json'), {
    beforeButtons,
    clicked,
    opened,
    documentVisible: /108219|K10000/i.test(text),
    dangerousActionsVisible: /Set Applies-to ID|Post Application/i.test(`${text}\n${buttons.join('\n')}`),
    dangerousActionsClicked: false,
    buttons
  });
  if (opened) {
    await screenshot(page, 'p2p-002-030-apply-entries-preflight.png', {
      projectName: project.name,
      testId,
      status: 'labor',
      purpose:
        'P2P-002 Apply Entries vor Kreditorenzahlung read-only pruefen; Applies-to ist ueber Journalfelder gesetzt, keine separate Apply-Aktion.',
      knownLimitations: [
...
async function postDraftOnce(page: Page) {
  ...
  await writeTextEvidence(p2pEvidencePath('040-post-confirm-dialog-page-text.txt'), compactPageText(dialogText));
  await screenshot(page, 'p2p-002-040-post-confirm-dialog.png', {
    projectName: project.name,
    testId,
    status: confirmationDialogVisible ? 'labor' : 'rejected',
    purpose: `P2P-002 Buchungsdialog vor der genau einmaligen Labor-Kreditorenzahlung ${paymentDocumentNo}.`,
    knownLimitations: [
  ...
  await writeTextEvidence(p2pEvidencePath('050-post-result-page-text.txt'), compactPageText(resultText));
  await screenshot(page, 'p2p-002-050-post-result.png', {
    projectName: project.name,
    testId,
    status: confirmed ? 'labor' : 'rejected',
    purpose: `P2P-002 Zustand nach Bestaetigung der Labor-Kreditorenzahlung ${paymentDocumentNo}.`,
    knownLimitations: [
...
function renderMarkdown(result: Record<string, any>) {
  return [
    ...
    `| K10000 sichtbar | ${result.preflight.vendorVisible ? 'ja' : 'nein'} |`,
    `| BANK-RM-01 sichtbar | ${result.preflight.bankVisible ? 'ja' : 'nein'} |`,
    `| 108219 als Applies-to Doc. No. sichtbar | ${result.preflight.invoiceReferenceVisible ? 'ja' : 'nein'} |`,
    `| Amount 25.000,00 sichtbar | ${result.preflight.amountVisible ? 'ja' : 'nein'} |`,
...
test('P2P-002 Kreditorenzahlung zu Rechnung 108219 buchen und Postenspur sichern', async ({ page }) => {
  const vendorBefore = await checkVendorEntryStillOpen(page);
  await writeJsonEvidence(p2pEvidencePath('010-vendor-ledger-before-payment.json'), vendorBefore);
  expect(vendorBefore.stillOpenEnoughForPayment, '108219 muss vor P2P-002 noch als offener Kreditorenposten sichtbar sein.').toBe(true);

  await cleanupDraftLine(page);
  await openPaymentJournal(page);
  const draft = await prepareDraft(page);
  await writeJsonEvidence(p2pEvidencePath('020-payment-journal-preflight-controls.json'), {
    documentNo: paymentDocumentNo,
    finalSnapshot: draft.finalSnapshot,
    buttons: draft.buttons,
    journalCheck: draft.journalCheck
  });
  await writeTextEvidence(p2pEvidencePath('020-payment-journal-preflight-page-text.txt'), compactPageText(draft.text));
  await screenshot(page, 'p2p-002-020-payment-journal-preflight.png', {
    projectName: project.name,
    testId,
    status: draft.journalCheck.zeroIssuesTotalVisible ? 'labor' : 'rejected',
    purpose:
      'P2P-002 Payment Journal vor Labor-Kreditorenzahlung: K10000, 108219, BANK-RM-01, Amount und Journal Check.',
    knownLimitations: [
  ...
  if (!preflightPassed) {
    ...
    const blocker = {
      ...
      cleanup,
      posted: false,
      nextStep: 'P2P-002 Payment-Journal-Preflight gezielt korrigieren; bis dahin keine Kreditorenzahlung buchen.'
    };
    await writeJsonEvidence(p2pEvidencePath('P2P-002-result.json'), blocker);
    await writeTextEvidence(p2pEvidencePath('P2P-002-VENDOR-PAYMENT-BLOCKER.md'), renderMarkdown({
      environment: 'MCP_1_20260210',
      company: project.defaultCompany,
      paymentDocumentNo,
    ...
    throw new Error('P2P-002 Preflight nicht vollstaendig; keine Kreditorenzahlung gebucht.');
  }
  ...
  await writeTextEvidence(p2pEvidencePath('035-posting-decision.txt'), decisionSentence);
  ...
  expect(posting.confirmed, 'P2P-002 muss genau einmal bestaetigt werden.').toBe(true);
  ...
  const traceTargets: TraceTarget[] = [
    {
      ...
      imageFileName: 'p2p-002-060-vendor-ledger-invoice-after-payment.png',
      labelPattern: /Vendor Ledger Entries|Vendor Ledger Entry|Kreditorenposten/i
    },
    {
      ...
      imageFileName: 'p2p-002-061-vendor-ledger-payment.png',
      labelPattern: /Vendor Ledger Entries|Vendor Ledger Entry|Kreditorenposten/i
    },
    {
      ...
      imageFileName: 'p2p-002-062-detailed-vendor-ledger-payment.png',
    ...
    },
    {
      ...
      imageFileName: 'p2p-002-063-bank-account-ledger-payment.png',
    ...
    },
    {
      ...
      imageFileName: 'p2p-002-064-gl-entries-payment.png',
  ...
  const result = {
    ...
    proves: [
      ...
      'K10000, 108219, BANK-RM-01, Betrag und Journal Check wurden vor der Buchung sichtbar geprueft.',
  ...
  await writeJsonEvidence(p2pEvidencePath('P2P-002-result.json'), result);
  await writeTextEvidence(p2pEvidencePath('P2P-002-VENDOR-PAYMENT.md'), renderMarkdown(result));
  await writeTextEvidence(
    ...
    [
      ...
      '| `P2P-002-result.json` | JSON-Ergebnis | strukturierter Preflight-, Posting- und Postenspur-Befund | keine Compliance-Finalaussage | labor |',
      '| `P2P-002-VENDOR-PAYMENT.md` | Lernzusammenfassung | Anfaengererklaerung, OP-Ausgleich, Buchwirkung, Grenzen und naechster Schritt | keinen deutschen Finalnachweis | labor |',
      ''
    ].join('\n')
  );
