...
const CASE_ID = 'FIXEDASSETS-097-PURCHASE-INVOICE-DRAFT-LINE-TYPE-PROOF';
const NEXT_CASE_ID = 'FIXEDASSETS-098-PURCHASE-INVOICE-LINE-TYPE-PROOF-REVIEW';
const TEST_ID = 'fixedassets-097';
  ...
  for (const frame of page.frames()) {
    const result = await frame
      .evaluate(() => {
        ...
        const chosen = candidates[0];
        if (!chosen) {
          return { clicked: false, reason: 'fixed-asset-option-not-visible' };
        }
...
async function probeFixedAssetLineTypeOnly(page: Page) {
  ...
  const attempts = [];
  if (!cell.found || !cell.chosen) {
    return { status: 'blocked-item-type-cell-not-found', cell, attempts };
  }
  ...
  for (const key of ['Alt+ArrowDown', 'F4']) {
    ...
    if (signals.fixedAssetLineTypeVisible) {
      ...
      return {
        status: selection.clicked ? `fixed-asset-selected-after-${key}` : `fixed-asset-visible-not-selected-after-${key}`,
        cell,
        attempts,
        selection,
...
function statePatch(resultStatus: string, summary: string) {
  return {
    current: {
      ...
      active_case_file: '.agent/state/cases/fixedassets-098-purchase-invoice-line-type-proof-review.json',
      lastReferenceCase: CASE_ID,
      lastReferenceCaseFile: '.agent/state/cases/fixedassets-097-purchase-invoice-draft-line-type-proof.json',
      nextStep:
        resultStatus === 'observed'
          ? 'FIXEDASSETS-098: review FA-097 evidence locally and decide whether a later target field-mapping case may unlock K30000/FA-CNC-01.'
          : 'FIXEDASSETS-098: review FA-097 blocker evidence locally and choose a safer setup/page capability route.',
    },
    lastRunSummary: {
      schemaVersion: 1,
      runId: CASE_ID,
      date: '2026-06-19',
      workType: 'fixed-asset-purchase-invoice-draft-line-type-proof',
      ...
      nextStep:
        resultStatus === 'observed'
          ? 'Run FIXEDASSETS-098 local review before any target values are unlocked.'
          : 'Run FIXEDASSETS-098 local blocker review before further UI retries.',
    },
    activeCase: {
      status: resultStatus === 'observed' ? 'observed-cleanup-complete' : 'blocked-cleanup-complete',
      lastResult: {
        status: resultStatus,
        resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-097/FIXEDASSETS-097-result.json',
        summary,
      },
      nextSafeAction: 'Run FIXEDASSETS-098 local evidence review.',
    },
  };
}

test('FIXEDASSETS-097 proves Purchase Invoice Line Type route without target values', async ({ page }) => {
  ...
  await writeJsonEvidence(faEvidencePath('030-purchase-invoice-line-type-proof.json'), {
  ...
  const fixedAssetSelected = /^fixed-asset-selected/i.test(routeProbe.status);
  const resultStatus = fixedAssetProved ? 'observed' : 'blocked';
  const summary = fixedAssetProved
    ? `FA-097 proved Purchase Invoice Line Type = Fixed Asset ${fixedAssetSelected ? 'selectable' : 'visible'} for draft ${draftInvoiceNo ?? '(unknown)'}, then cleaned up the draft.`
    : `FA-097 did not prove Purchase Invoice Line Type = Fixed Asset; status=${routeProbe.status}, guard=${guard.status}. Cleanup status=${cleanup.status}.`;

  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-purchase-invoice-draft-line-type-proof-result',
    caseId: CASE_ID,
    source: 'playwright-sandbox-purchase-invoice-line-type-proof',
    resultStatus,
    ...
    proved: [
      ...
      ...(fixedAssetProved ? [`Type = Fixed Asset was ${fixedAssetSelected ? 'selected' : 'visible'} in the Purchase Invoice Lines context.`] : []),
    ...
    notProved: [
      ...(fixedAssetProved ? [] : ['Type = Fixed Asset was not proven as visible/selectable through the Purchase Invoice line route.']),
    ...
    changedFiles: [
      ...
      '.agent/state/cases/fixedassets-097-purchase-invoice-draft-line-type-proof.json',
      '.agent/state/cases/fixedassets-098-purchase-invoice-line-type-proof-review.json',
      '.agent/state/last_run_summary.json',
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-097-purchase-invoice-draft-line-type-proof.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/FIXEDASSETS-097-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/030-purchase-invoice-line-type-proof.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/031-after-route-focused-text.txt',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/090-cleanup-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/README.md',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-097/FIXEDASSETS-097-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-097/030-purchase-invoice-line-type-proof.json',
    ...
    blockedBy: fixedAssetProved ? [] : ['Type = Fixed Asset not visible/selectable through Purchase Invoice line route.'],
    ...
    nextStep: 'Run FIXEDASSETS-098 local evidence review before unlocking any target values.',
  };

  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-097-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-097-learning.md'),
    [
