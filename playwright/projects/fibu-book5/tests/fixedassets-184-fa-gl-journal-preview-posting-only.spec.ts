...
const CASE_ID = 'FIXEDASSETS-184-FA-GL-JOURNAL-PREVIEW-POSTING-ONLY';
const NEXT_CASE_ID = 'FIXEDASSETS-185-FA-GL-JOURNAL-PREVIEW-POSTING-REVIEW';
const TEST_ID = 'fixedassets-184';
...
async function openPreviewPostingOnly(page: Page) {
  ...
  const openedPostingDialog =
    /Ship and Invoice|Liefern und fakturieren|Receive and Invoice|Empfangen und fakturieren|Invoice\b|Rechnung\b/i.test(textAfter) &&
    /OK|Yes|Ja/i.test(textAfter);
  const previewOpened = /Posting Preview|Buchungsvorschau|G\/L Entry|Sachposten|FA Ledger Entry|Anlagenposten/i.test(textAfter);
  const setupErrorVisible = /must have a value|does not exist|is missing|fehlt|muss.*Wert|nicht vorhanden/i.test(textAfter);
...
function statePatch(status: 'observed' | 'blocked', summary: string) {
  return {
    current: {
      ...
      active_case_file: '.agent/state/cases/fixedassets-185-fa-gl-journal-preview-posting-review.json',
      lastReferenceCase: CASE_ID,
      lastReferenceCaseFile: '.agent/state/cases/fixedassets-184-fa-gl-journal-preview-posting-only.json',
      requiresStrongModel: true,
      nextStep:
        'FIXEDASSETS-185: locally review FA-184 Preview Posting evidence before any Fixed Asset G/L Journal posting.',
    },
    activeCase: {
      status: status === 'observed' ? 'observed-preview-posting-only' : 'blocked-preview-posting-only',
      lastResult: {
        status,
        resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-184/FIXEDASSETS-184-result.json',
        summary,
      },
      nextSafeAction: 'Run FIXEDASSETS-185 local review; do not post.',
    },
  };
}

test('FIXEDASSETS-184 opens Preview Posting only for guarded FA G/L Journal line', async ({ page }) => {
  ...
  try {
    ...
    if (!context.environmentInUrl || (!context.companyInUrl && !context.companyInText) || context.wrongEnvironmentVisible) {
      ...
      if (!helperResult.visibleSignals.expectedValueVisible) {
        ...
        await writeTextEvidence(faEvidencePath('020-preview-posting-page-text.txt'), compactPageText(page, {
          include: [/Posting Preview|Buchungsvorschau|G\/L Entry|Sachposten|FA Ledger Entry|Anlagenposten|Value Entry|Wertposten|82000|FA-CNC-01|G05001|Error Messages|Fehlermeldungen|must have a value|does not exist|missing|fehlt/i],
          maxLines: 120,
          maxLineLength: 220,
        }));
        await screenshot(page, 'fixedassets-184-020-preview-posting-only.png', {
          projectName: project.name,
          testId: TEST_ID,
          status: preview.previewOpened ? 'labor' : 'rejected',
          bookUse: preview.previewOpened ? 'evidence' : 'do-not-use',
          purpose:
            'FA-184 Preview Posting-only: oeffnet die Buchungsvorschau fuer die bestehende FA-G/L-Journal-Zeile mit sichtbarem Gegenkonto 82000, ohne zu buchen.',
          expectedPageText: preview.previewOpened ? [/Posting Preview|Buchungsvorschau|G\/L Entry|Sachposten|FA Ledger Entry|Anlagenposten/i] : [],
          knownLimitations: [
            'Nur Preview Posting.',
            'Keine Buchung.',
  ...
  const summary =
    status === 'observed'
      ? 'FA-184 opened Preview Posting for the existing FA G/L Journal line with visible Bal. Account No. = 82000 and did not post.'
      : `FA-184 stayed safe but did not prove Preview Posting: ${blockedBy.join(' | ')}`;
  const patch = statePatch(status, summary);

  await writeJsonEvidence(faEvidencePath('010-journal-pre-preview-check.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-184-journal-pre-preview-check',
    caseId: CASE_ID,
    context,
    frameUrl,
    target: TARGET,
    snapshot,
    helperResult,
    blockedByBeforePreview: blockedBy.filter((entry) => /context|visible|candidate|frame/i.test(entry)),
    omitted: 'No full DOM dump, traces, videos, reports or auth artifacts stored.',
  });

  await writeJsonEvidence(faEvidencePath('020-preview-posting-result.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-184-preview-posting-only',
    caseId: CASE_ID,
    preview,
    closeResult,
    blockedBy,
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-184-preview-posting-only-result',
    caseId: CASE_ID,
    source: 'playwright-preview-only',
    resultStatus: status,
    ...
    proved: [
      ...
      ...(helperResult?.visibleSignals.expectedValueVisible ? ['Bal. Account No. = 82000 was visible/current before Preview Posting.'] : []),
      ...(preview?.clickedPreview ? ['Preview Posting action was clicked.'] : []),
      ...(preview?.previewOpened ? ['Posting Preview context was visible.'] : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-184-fa-gl-journal-preview-posting-only.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-184/',
      ...(screenshotCaptured ? ['playwright/projects/fibu-book5/img/fixedassets-184-020-preview-posting-only.png'] : []),
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-184/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-184/010-journal-pre-preview-check.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-184/020-preview-posting-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-184/FIXEDASSETS-184-learning.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-184/FIXEDASSETS-184-result.json',
      ...(preview?.clickedPreview ? ['playwright/projects/fibu-book5/evidence/fixedassets-184/020-preview-posting-page-text.txt'] : []),
      ...(screenshotCaptured ? ['playwright/projects/fibu-book5/img/fixedassets-184-020-preview-posting-only.png'] : []),
    ],
    warnings: [
      'Preview Posting-only. Do not treat as posting permission.',
      'Posting remains locked until FIXEDASSETS-185 local review.',
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-184-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-184-learning.md'),
    [
      '# FIXEDASSETS-184 Lernzusammenfassung',
      '',
      ...
      '## Lernwert',
      '',
      'Preview Posting ist der naechste Kontrollpunkt nach der sichtbaren Journal-Zeile. Business Central soll vor einer echten Buchung zeigen, welche Vorschau-/Postenarten entstehen oder welcher Setup-Blocker noch greift.',
      '',
      '## Grenzen',
      '',
      '- Keine Buchung.',
      '- Keine echte FA-Ledger-/G/L-Postenspur.',
      '- Keine deutsche Final-Evidence.',
  ...
  expect(preview?.openedPostingDialog ?? false).toBe(false);
