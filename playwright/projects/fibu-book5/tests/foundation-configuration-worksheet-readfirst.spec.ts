...
test.skip(
  process.env.FOUNDATION_CONFIGURATION_WORKSHEET_READFIRST_LIVE_APPROVED !== '1' ||
    process.env.FOUNDATION_CONFIGURATION_WORKSHEET_READFIRST_RUNNER_GUARD_CHECKED !== '1',
  'FOUNDATION-CONFIGURATION-WORKSHEET-READFIRST must run through the guarded runner with --live-approved.'
);

const CASE_ID = 'FOUNDATION-CONFIGURATION-WORKSHEET-READFIRST';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'foundation-configuration-worksheet-readfirst';
...
const CONFIG_ROUTE_RE =
  /Konfigurationsarbeitsblatt|Configuration Worksheet|Konfigurationspakete|Configuration Packages|Config\. Packages|Tabellen|Tables|Felder|Fields|Paketcode|Package Code/i;
...
async function compactRouteText(page: Page) {
  return clean(
    await compactPageText(page, {
      include: [
        CONFIG_ROUTE_RE,
        /U-VAT325-DISC|VAT 325 Discovery|252|325|470|Buchungsmatrix|MwSt|USt|VAT|Anzahl Tabellen|Anzahl Datensatze/i,
        WRITE_CAPABLE_RE
      ],
      maxLines: 220,
      maxLineLength: 240
...
test(`${CASE_ID} proves configuration worksheet route read-first`, async ({ page }) => {
  ...
  await stepTimeline.step(page, {
    ...
    action: 'Open Configuration Worksheet via Tell-Me read-first',
    claim: 'Business Central shows Configuration Worksheet as the foreground target, not merely Role Center or Tell-Me.',
    expectedPageText: [CONFIG_WORKSHEET_RE],
    run: async () => {
  ...
  await stepTimeline.step(page, {
    ...
    claim: 'Business Central shows Configuration Packages page/list without repeating the old Escape-after-Tell-Me route.',
  ...
  const routeDecision =
    tableSignals.configurationWorksheetVisible && tableSignals.configurationPackagesVisible
      ? 'ready-for-write-gate'
      : 'parked-readfirst-blocked';
  const nextCase =
    routeDecision === 'ready-for-write-gate'
  ...
  const result = {
    schemaVersion: 1,
    caseId: CASE_ID,
    source: 'playwright-readfirst-configuration-worksheet-and-packages',
    resultStatus: routeDecision === 'ready-for-write-gate' ? 'observed-readfirst' : 'blocked-readfirst',
    routeDecision,
    startedAt,
    ...
    actionsTaken: [
      ...
      'Opened Configuration Worksheet read-only via Tell-Me and captured target context.',
    ...
    nextStepDecision: {
      ...
      lastEvidenceSummary:
        'Source decision selected Configuration Worksheet and Configuration Packages as read-first proof before any package or setup write.',
      isPlannedNextCaseStillSensible: routeDecision === 'ready-for-write-gate',
      reason:
        routeDecision === 'ready-for-write-gate'
          ? 'Both relevant standard BC setup-data surfaces are reachable and screenshot-classified read-first.'
          : 'At least one standard BC setup-data surface was not sufficiently visible.',
      lookaheadReviewed: [
        {
          caseId: 'FOUNDATION-SETUP-PACKAGE-WRITE-GATE-DECISION',
          status: routeDecision === 'ready-for-write-gate' ? 'ready-next' : 'blocked',
          reason: routeDecision === 'ready-for-write-gate' ? 'Read-first surfaces are proven.' : 'Read-first surface proof is incomplete.'
        },
        {
