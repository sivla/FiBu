...
const specPath = 'playwright/projects/fibu-book5/tests/pws-ff-002-general-posting-setup-readonly-recovery.spec.ts';
const casePath = '.agent/state/cases/pws-ff-002-general-posting-setup-readfirst-recovery.json';
...
function foundationDecisionStatus() {
  if (!existsSync(foundationDecisionPath)) {
    return {
      ready: false,
      blocker: 'foundation-readiness-decision-missing',
      nextStep: 'Run TARGET-075 and write FOUNDATION-READINESS-DECISION.md before PWS-FF-002.'
    };
  }
  ...
  if (/template\/no-evidence|pending-target075-evidence/i.test(text)) {
    return {
      ready: false,
      blocker: 'foundation-readiness-decision-not-finalized',
      nextStep: 'Finalize FOUNDATION-READINESS-DECISION.md from TARGET-075 evidence before PWS-FF-002.'
    };
  }
  if (!/Buchungsmatrix Einrichtung \/ General Posting Setup was not visible enough for accepted proof/i.test(text)) {
    return {
      ready: false,
      blocker: 'foundation-decision-does-not-identify-general-posting-setup-gap',
      nextStep: 'Do not run PWS-FF-002 unless Foundation Readiness identifies the General Posting Setup gap.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When live gate is open, run PWS-FF-002 only with --live-approved as read-first/no-write.'
  };
}

if (help) {
  console.log(`PWS-FF-002 guarded runner

Usage:
  node scripts/agent/run-pws-ff-002-general-posting-setup-readonly.mjs --check
  node scripts/agent/run-pws-ff-002-general-posting-setup-readonly.mjs --list
  node scripts/agent/run-pws-ff-002-general-posting-setup-readonly.mjs --live-approved

This runner opens only Buchungsmatrix Einrichtung / General Posting Setup read-only.
It must not type, save, create, preview, post or use API shortcuts.`);
  process.exit(0);
}
...
const caseReady = existsSync(casePath);
const blockedBy = [
  caseReady ? '' : 'pws-ff-002-case-file-missing',
  foundationDecision.ready ? '' : foundationDecision.blocker,
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-002-general-posting-setup-readonly-check',
        caseId: 'PWS-FF-002-GENERAL-POSTING-SETUP-READFIRST-RECOVERY',
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        foundationDecisionPath,
        foundationReady: foundationDecision.ready,
        targetUrlReady,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-002-general-posting-setup-readonly-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep:
          'Do not run PWS-FF-002 live before Foundation Readiness Decision, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_FF_002_RUNNER_GUARD_CHECKED: '1',
      PWS_FF_002_LIVE_APPROVED: '1',
      PWS_FF_002_BC_TARGET_URL: targetUrl
    }
  })
);
