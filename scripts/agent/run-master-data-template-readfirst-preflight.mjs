...
const specPath = 'playwright/projects/fibu-book5/tests/master-data-template-readfirst-preflight.spec.ts';
const currentPath = '.agent/state/current.json';
const casePath = '.agent/state/cases/master-data-template-readfirst-preflight.json';
...
function localCaseStatus() {
  ...
  if (current.activeCase !== CASE_ID || current.nextCase !== CASE_ID) {
    return { ready: false, blocker: 'master-data-template-readfirst-preflight-not-active-case' };
  }
...
if (help) {
  console.log(`MASTER-DATA template read-first preflight runner

Usage:
  node scripts/agent/run-master-data-template-readfirst-preflight.mjs --check
  node scripts/agent/run-master-data-template-readfirst-preflight.mjs --list
  node scripts/agent/run-master-data-template-readfirst-preflight.mjs --live-approved

This runner may inspect Customers, Vendors and Items/Services read-first in playthru / UNIVERSAARL-DE.
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'master-data-template-readfirst-preflight-runner-check',
        caseId: CASE_ID,
        ...
        nextStep:
          blockedBy.length === 0
            ? 'Run with --live-approved for exactly one read-first/no-write template preflight.'
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'master-data-template-readfirst-preflight-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run template preflight before active case, usable auth, readiness and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      MASTER_DATA_TEMPLATE_READFIRST_PREFLIGHT_RUNNER_GUARD_CHECKED: '1',
      MASTER_DATA_TEMPLATE_READFIRST_PREFLIGHT_LIVE_APPROVED: '1'
    }
  })
);
