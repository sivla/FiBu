...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` idempotent per BC-Standard-API an; Bank Accounts zeigt danach `BANK-RM-01`, `CHECKING` und `SAVINGS` | ja | ja, `payments-002`, `payments-003` | `partial` | P4 | `PAYMENTS-004`: nicht buchende Zahlungsjournal-Readiness pruefen; Bank Account Posting Group/Sachkonto-Fit und Gegenkonto nicht als erledigt markieren. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only und zeigt Felder/Post-Aktionen; `PAYMENTS-003` stellt das Laborbankkonto bereit, erstellt aber keine Zahlungszeile | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003` | `partial` | P4 | Eine einzelne Zahlungsjournal-Readiness-Zeile kontrolliert vorbereiten oder Pflichtfelder pruefen; noch nicht buchen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003` | `partial` | P4 | `PAYMENTS-004`: nicht buchende Zahlungsjournal-Readiness; erst danach ggf. eine einzelne Laborzahlung vorbereiten. |
