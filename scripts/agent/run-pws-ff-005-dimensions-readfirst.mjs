...
const specPath = 'playwright/projects/fibu-book5/tests/pws-ff-005-dimensions-readfirst.spec.ts';
const casePath = '.agent/state/cases/pws-ff-005-dimensions-readfirst.json';
...
function localCaseStatus() {
  if (!existsSync(casePath)) {
    return {
      ready: false,
      blocker: 'pws-ff-005-case-file-missing',
      nextStep: 'Create the PWS-FF-005 case file before any live Dimensions read-first proof.'
    };
  }
  ...
  if (selected !== CASE_ID) {
    return {
      ready: false,
      blocker: 'pws-ff-005-not-selected-next-case',
  ...
  if (caseFile.instance !== EXPECTED_INSTANCE || caseFile.company !== TARGET_COMPANY) {
    return {
      ready: false,
      blocker: 'pws-ff-005-case-target-mismatch',
  ...
  if (caseFile.effectiveBcActionsAllowed !== false) {
    return {
      ready: false,
      blocker: 'pws-ff-005-must-stay-readonly',
      nextStep: 'PWS-FF-005 must remain read-first/no-write.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When auth and live gate are ready, run PWS-FF-005 with --live-approved as read-first/no-write.'
  };
}

if (help) {
  console.log(`PWS-FF-005 guarded runner

Usage:
  node scripts/agent/run-pws-ff-005-dimensions-readfirst.mjs --check
  node scripts/agent/run-pws-ff-005-dimensions-readfirst.mjs --check --overnight
  node scripts/agent/run-pws-ff-005-dimensions-readfirst.mjs --list
  node scripts/agent/run-pws-ff-005-dimensions-readfirst.mjs --live-approved

This runner opens Dimensions, Dimension Values and General Ledger Setup read-first only. It must not create, edit,
set global/default dimensions, create master data, preview, post or switch company.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-005-dimensions-readfirst-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-005-dimensions-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run PWS-FF-005 live before case selection, fresh auth, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_FF_005_RUNNER_GUARD_CHECKED: '1',
      PWS_FF_005_LIVE_APPROVED: '1',
      PWS_FF_005_BC_TARGET_URL: targetUrl
    }
  })
);
