...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only; `PAYMENTS-004` bestaetigt Pflichtfelder; `PAYMENTS-005` bereitet eine Cash-Receipt-Journal-Zeile per UI vor und loescht sie wieder; `PAYMENTS-006` zeigt Amount-Format und Bank-Posting-Group-Blocker; `PAYMENTS-007` loest den Bank-Posting-Group-Blocker, danach bleibt Amount-Issue | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007` | `partial` | P4 | `PAYMENTS-008`: Amount-Feld, Amount-LCY-Anzeige und Spaltenposition im Cash Receipt Journal gezielt klaeren; weiterhin nicht buchen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder; `PAYMENTS-005` belegt UI-Draft; `PAYMENTS-006` zeigt Amount-Format und Bank-Posting-Group-Blocker; `PAYMENTS-007` fittet `BANK-RM-01 = CHECKING`, Journal Check bleibt wegen Amount gesperrt; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007` | `partial` | P4 | `PAYMENTS-008`: Amount-/Journal-Check-Diagnose nach geloestem Bankkonto-Fit; erst danach ggf. neuer nicht buchender Journal-Check. |
