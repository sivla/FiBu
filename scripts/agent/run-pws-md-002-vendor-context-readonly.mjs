...
const specPath = 'playwright/projects/fibu-book5/tests/pws-md-002-vendor-context-readonly.spec.ts';
...
const REAL_CUSTOMER_PROJECT_BOUNDARY = {
  ...
  acceptedDataMode:
    'customer-project-like Universaarl vendor records that are fictional or anonymized, have business purpose, owner, dependencies, UAT/training use and BC setup readiness, exclude real bank data, and are proven through real playthru UI evidence before they become book or process truth'
};
...
function foundationDecisionStatus() {
  const missingBlocker = 'foundation-readiness-decision-missing-or-not-finalized-for-pws-md-002';
  if (!existsSync(foundationDecisionPath)) {
    return {
      ready: false,
      blocker: missingBlocker,
      nextStep: 'Run TARGET-075, write FOUNDATION-READINESS-DECISION.md and include PWS-MD-002 before vendor context read-first proof.'
    };
  }
  ...
  if (/template\/no-evidence|pending-target075-evidence/i.test(text)) {
    return {
      ready: false,
      blocker: missingBlocker,
      nextStep: 'Finalize FOUNDATION-READINESS-DECISION.md from TARGET-075 evidence before vendor context read-first proof.'
    };
  }
  if (/Master Data bleibt geparkt/i.test(text)) {
    return {
      ready: false,
      blocker: 'foundation-readiness-decision-parks-master-data-for-pws-md-002',
      nextStep: 'Resolve Foundation gaps from FOUNDATION-READINESS-DECISION.md before PWS-MD-002 vendor context read-first proof.'
    };
  }
  if (!text.includes('PWS-MD-002')) {
    return {
      ready: false,
      blocker: 'foundation-readiness-decision-missing-pws-md-002',
      nextStep: 'Add an explicit PWS-MD-002 handoff before vendor context read-first proof.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When live gate is open, run PWS-MD-002 only with --live-approved as read-first/no-write.'
  };
}

if (help) {
  console.log(`PWS-MD-002 guarded runner

Usage:
  node scripts/agent/run-pws-md-002-vendor-context-readonly.mjs --check
  node scripts/agent/run-pws-md-002-vendor-context-readonly.mjs --list
  node scripts/agent/run-pws-md-002-vendor-context-readonly.mjs --live-approved

This runner never writes vendor data. Live execution requires:
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-002-vendor-context-readonly-check',
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-002-vendor-context-readonly-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep:
          'Do not run PWS-MD-002 live before TARGET-075 evidence, Foundation Readiness Decision, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_MD_002_RUNNER_GUARD_CHECKED: '1',
      PWS_MD_002_LIVE_APPROVED: '1',
      PWS_MD_002_BC_TARGET_URL: targetUrl
    }
  })
);
