...
| Kapitel | Bewertung /10 | Alltagsszene | Alt+Q-Schrittfolge | Testdaten | Belege | Posten | Kontrollbericht | Fehlerdiagnose | Übung | Lösung | UAT | Negativtest | Deutsch-first | Restlücke |
|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 11. Order-to-Cash | 10/10 | Kunde bestellt `RM-M100` bei RM-SALES. Vertrieb, Lager und Finance arbeiten denselben Auftrag ab. | `Verkaufsangebote (Sales Quotes)`, `Verkaufsaufträge (Sales Orders)`, `Buchungsvorschau (Preview Posting)`, `Gebuchte Verkaufsrechnungen (Posted Sales Invoices)`. | Debitor `D10000`, Artikel `RM-M100`, Menge `1`, Preis `68.000 EUR`, USt `19 %`, Lagerort `FRA-ZL`. | Verkaufsangebot, Verkaufsauftrag, gebuchte Verkaufslieferung, gebuchte Verkaufsrechnung, Zahlung. | Debitorenposten, Sachposten, USt-Posten, Artikelposten, Wertposten. | Debitorenposten, Sachposten, Lagerbewertung, Finanzbericht `RM-GUV-MONAT`. | Falscher Lagerort, falsche USt-Gruppe, fehlende Dimension mit Diagnose über Rechnung, Posten und Belegzeilen. | Übung `RM-M100` verkaufen mit Rolle Vertrieb. | Lösung nennt Seite, Feld, Wert, Buchungsvorschau, Buchen und Postenprüfung. | `UAT-O2C-001` mit Angebots-, Auftrags-, Liefer-, Rechnungs- und Zahlungsschritten. | Falscher Lagerort oder fehlende Pflichtdimension; Korrektur über Gutschrift/Neubuchung oder Dimensionskorrektur nach Regel. | Deutsche Seiten zuerst, englische Begriffe nur in Klammern. | Keine. |
| 12. Procure-to-Pay | 10/10 | RM-PROD kauft Stahl, damit die Fertigung `RM-M100` produzieren kann. | `Einkaufsbestellungen (Purchase Orders)`, `Gebuchte Einkaufslieferungen (Posted Purchase Receipts)`, `Gebuchte Einkaufsrechnungen (Posted Purchase Invoices)`, `Kreditorenposten (Vendor Ledger Entries)`. | Kreditor `K10000`, Artikel `RAW-STEEL`, Menge `10`, Lagerort `FRA-ZL`, USt `19 %`. | Einkaufsbestellung, gebuchte Einkaufslieferung, gebuchte Einkaufsrechnung. | Kreditorenposten, Sachposten, USt-Posten, Artikelposten, Wertposten. | Kreditorenposten, Artikelposten, Lagerbewertung, Sachposten. | Rechnung über falsche Menge, falscher Einstandspreis, falsche USt-Produktbuchungsgruppe. | Übung Rohmaterial einkaufen. | Lösung führt Wareneingang, Rechnung, Postenprüfung und Korrekturpfad aus. | `UAT-P2P-001` mit Bestellung, Eingang, Rechnung und Zahlungsvorbereitung. | Eingangsrechnung über `10`, obwohl nur `6` geliefert; Korrektur über Mengenprüfung/Rücksprache, keine Direktbuchung. | Deutsch-first konsequent. | Keine. |
| 13. Inventory und Warehouse | 10/10 | Lager `FRA-ZL` arbeitet gesteuert, Lager `MZ-EINFACH` einfach. Der gleiche Artikel zeigt zwei Lagerlogiken. | `Artikel (Items)`, `Lagerorte (Locations)`, `Lagereingänge (Warehouse Receipts)`, `Lagereinlagerungen (Warehouse Put-aways)`, `Kommissionierungen (Picks)`, `Artikelposten (Item Ledger Entries)`. | Artikel `RAW-STEEL`, `SP-PUMP-01`, Lagerorte `FRA-ZL`, `MZ-EINFACH`, Lagerplatz `A-01-01`. | Einkaufsbestellung, Lagereingang, Einlagerung, Umlagerungsauftrag, Kommissionierung. | Artikelposten, Wertposten, Lagerplatzposten, Sachposten nach Lagerwertbuchung. | Lagerbewertung, Lagerplatzinhalt, Artikelverfügbarkeit nach Lagerort. | Falscher Lagerort, fehlender Lagerplatz, negativer Bestand, Pick nicht registriert. | Übung einfacher Eingang und gesteuerte Einlagerung. | Lösung nennt `Lagereingänge (Warehouse Receipts)`, `Quelldokumente holen (Get Source Documents)`, `Wareneingang buchen (Post Receipt)`. | `UAT-WHS-001` prüft Eingang, Einlagerung, Pick, Lagerplatz und Lagerwert. | Ware auf falschem Lagerort `MZ-EINFACH` statt `FRA-ZL`; Korrektur über Umlagerung oder Storno nach Status. | Deutsche Lagerbegriffe mit englischer Suchhilfe. | Keine. |
| 14. Planning, Assembly und Manufacturing | 10/10 | RM-PROD produziert `RM-M100`, weil RM-SALES Liefertermin und Menge zugesagt hat. | `Planungsarbeitsblätter (Planning Worksheets)`, `Freigegebene Fertigungsaufträge (Released Production Orders)`, `Verbrauchs Buch.-Blätter (Consumption Journals)`, `Output Buch.-Blätter (Output Journals)`. | Fertigungsauftrag `PROD-3001`, Artikel `RM-M100`, Menge `3`, Material `RAW-STEEL`, Lagerort `FRA-ZL`. | Planungszeile, Fertigungsauftrag, Verbrauchsbuchung, Outputbuchung. | Artikelposten für Verbrauch und Output, Wertposten, Kapazitätsposten, Sachposten nach Kostenregulierung. | Fertigungsauftragsstatistik, Artikelposten, Wertposten, Lagerbewertung. | Verbrauch fehlt, Output falsche Menge, falscher Arbeitsplan, Material nicht verfügbar. | Übung `RM-M100` fertigen. | Lösung klickt Fertigungsauftrag, Verbrauch, Output und Postenprüfung durch. | `UAT-MFG-001` mit Planung, Freigabe, Verbrauch, Output und Kostenprüfung. | Output ohne Verbrauch; Diagnose über Fertigungsauftragsstatistik und Artikelposten, Korrektur über Verbrauchsnachbuchung nach Freigabeprozess. | Deutsche Fertigungsseiten zuerst. | Keine. |
| 15. Service | 10/10 | Montagmorgen steht eine Kundenmaschine. RM-SERVICE disponiert Techniker, Ersatzteil und Faktura/Kulanz. | `Serviceaufträge (Service Orders)`, `Serviceartikel (Service Items)`, `Servicezeilen (Service Lines)`, `Ressourcen (Resources)`, `Gebuchte Servicerechnungen (Posted Service Invoices)`. | Serviceauftrag `SERV-4001`, Debitor `D10000`, Serviceartikel `RM-M100-SN1001`, Artikel `SP-PUMP-01`, Ressource `RES-TECH`, Lagerort `VAN-SERV`. | Serviceauftrag, gebuchte Servicelieferung oder Servicerechnung, Kulanzdokumentation. | Artikelposten, Wertposten, Ressourcenposten, Debitorenposten bei Faktura, Sachposten, USt-Posten. | Serviceauftragsstatistik, Artikelposten, Debitorenposten, Finanzbericht. | Ersatzteil auf falschem Fahrzeuglager, Garantie falsch fakturiert, Ressource ohne Menge. | Übung Service mit Ersatzteil und zwei Technikerstunden. | Lösung nennt Serviceauftrag, Zeilen, Lagerort, Buchung, Postenprüfung. | `UAT-SRV-001` mit Faktura- und Kulanzentscheidung. | Lagerort `VAN-SERV` leer; Diagnose über Artikelverfügbarkeit nach Lagerort, Korrektur über Umlagerung. | Deutsch-first in Servicebegriffen. | Keine. |
| 16. Projects | 10/10 | RM-SERVICE installiert Sondermaschine `RM-X500` als Projekt mit Meilensteinrechnung. | `Projekte (Projects)`, `Projektaufgaben (Project Tasks)`, `Projekt Buch.-Blätter (Project Journals)`, `Projektplanzeilen (Project Planning Lines)`, `Verkaufsrechnungen (Sales Invoices)`. | Projekt `PROJ-5001`, Aufgabe `2000`, Ressource `RES-PM`, Artikel `SP-PUMP-01`, Meilenstein `40 %`. | Projekt, Projektplanzeile, Projektbuchung, Verkaufsrechnung, Meilensteinrechnung. | Projektposten, Ressourcenposten, Artikelposten/Wertposten, Debitorenposten, Sachposten, USt-Posten. | Projektstatistik, Projektposten, Finanzbericht nach Dimension `PROJECT`. | Aufwand ohne Projektaufgabe, falsche Fakturamethode, fehlende Dimension. | Übung Meilensteinrechnung mit Material und Ressource. | Lösung führt Projektanlage, Buchung, Faktura und Projektstatistik aus. | `UAT-PRJ-001` mit Aufwand, WIP-/Faktura-Prüfung und Marge. | Ressource auf falsche Aufgabe gebucht; Korrektur über Storno und Neubuchung auf richtige Aufgabe. | Deutsche Projektseiten zuerst. | Keine. |
| 17. Shopify, Dropshipping und Sonderverkauf | 10/10 | Kunde `D11000` bestellt `SP-PUMP-01` im Onlineshop; ein zweiter Auftrag wird direkt vom Lieferanten geliefert. | `Shopify-Shops (Shopify Shops)`, `Shopify-Aufträge (Shopify Orders)`, `Verkaufsaufträge (Sales Orders)`, `Einkaufsbestellungen (Purchase Orders)`, `Direktlieferung (Drop Shipment)`. | Auftrag `WEB-24001`, Debitor `D11000`, Artikel `SP-PUMP-01`, Dropshipping-Lieferant `K20000`, Menge `2`. | Shopify-Auftrag, Verkaufsauftrag, Einkaufsbestellung für Direktlieferung, gebuchte Verkaufsrechnung. | Debitorenposten, Kreditorenposten bei Lieferantenrechnung, Sachposten, USt-Posten, Artikelposten nur bei Lagerverkauf, keine Lagerbewegung bei echter Direktlieferung. | Shopify-Synchronisationsprotokoll, Verkaufsrechnungen, Debitorenposten, Margenbericht. | Shop-Mapping fehlt, falscher Steuercode, Direktlieferung erzeugt unerwartete Lagerbewegung. | Übung Shopauftrag importieren und Dropshipping prüfen. | Lösung nennt Shop-Sync, Verkaufsauftrag, Direktlieferungskennzeichen, Einkauf, Faktura, Postenprüfung. | `UAT-SHOP-001` mit Shopauftrag, Dropshipping, Zahlung und Steuerprüfung. | Artikelmapping auf falschen Artikel; Diagnose über Shopify-Auftrag und Artikelnummer, Korrektur über Mapping und erneute Synchronisation. | Deutsch-first, Shopify-Begriffe als Suchhilfe. | Keine. |
| 18. Intercompany und Ausland | 10/10 | RM-PROD verkauft intern an RM-SALES; RM-SALES verkauft anschließend ins EU-Ausland und Drittland. | `Intercompany-Postausgang (IC Outbox Transactions)`, `Intercompany-Posteingang (IC Inbox Transactions)`, `Verkaufsaufträge (Sales Orders)`, `Einkaufsbestellungen (Purchase Orders)`, `USt-IdNr.-Prüfung (VAT Registration No. Validation)`. | IC-Beleg `IC-7001`, RM-PROD, RM-SALES, EU-Debitor `D12000-EU`, Drittlanddebitor `D13000-US`. | IC-Verkaufsrechnung, IC-Einkaufsrechnung, Ausgangsrechnung Ausland, Nachweisdokumente. | Debitorenposten, Kreditorenposten, Sachposten, USt-Posten je Steuerlogik, Artikelposten/Wertposten bei Warenbewegung. | IC-Abstimmung, USt-Posten, Zusammenfassende Meldung, Exportnachweis. | Falsche USt-Gruppe, IC-Beleg nicht akzeptiert, fehlender Ausfuhrnachweis. | Übung IC-Verkauf und EU-B2B-Verkauf. | Lösung führt IC-Ausgang, IC-Eingang, Annahme, Buchung und USt-Prüfung aus. | `UAT-IC-001` mit IC-Abstimmung und Auslandsteuerfall. | EU-Kunde ohne gültige USt-ID; Diagnose über Debitorenkarte und USt-Posten, Korrektur vor Buchung. | Deutsche Begriffe führen, internationale Begriffe in Klammern. | Keine. |
| 19. Debitoren, Kreditoren und OP-Ausgleich | 10/10 | Finance sieht offene Rechnungen aus Verkauf und Einkauf und gleicht Zahlungseingänge sowie Zahlungsausgänge aus. | `Debitorenposten (Customer Ledger Entries)`, `Kreditorenposten (Vendor Ledger Entries)`, `Zahlungseingangs Buch.-Blätter (Cash Receipt Journals)`, `Zahlungsausgangs Buch.-Blätter (Payment Journals)`, `Posten ausgleichen (Apply Entries)`. | Debitor `D10000`, Rechnung `80.920 EUR`, Zahlung `80.920 EUR`, Kreditor `K10000`, Skonto-/Teilzahlung. | Zahlungseingang, Zahlungsausgang, Ausgleich, Ausgleichsaufhebung. | Debitorenposten, detaillierte Debitorenposten, Kreditorenposten, detaillierte Kreditorenposten, Sachposten. | OP-Liste, Debitorensaldo, Kreditorensaldo, Sachkonto Bank. | Zahlung nicht ausgeglichen, falscher Debitor, Überzahlung, Skonto falsch. | Übung Zahlung mit Rechnung ausgleichen. | Lösung klickt Buch.-Blatt, Kontoart, Betrag, `Posten ausgleichen`, Buchung und Detailpostenprüfung. | `UAT-OP-001` mit Ausgleich und Ausgleich aufheben. | Zahlung auf falschen Debitor; Diagnose über Debitorenposten, Korrektur über Ausgleich aufheben und Umbuchung. | Deutsch-first konsequent. | Keine. |
| 20. Bank, Payments und Bankabstimmung | 10/10 | RM-SHARED importiert Bankauszug und weist Zahlungen Rechnungen zu. | `Bankkonten (Bank Accounts)`, `Bankkontenabstimmungen (Bank Account Reconciliations)`, `Zahlungsausgangs Buch.-Blätter (Payment Journals)`, `Zahlungsverkehr (Payment Management)`. | Bankkonto `BANK-RM-01`, Kontoauszug `BA-2026-06-30`, Zahlung `80.920 EUR`, Debitor `D10000`. | Bankkontenabstimmung, Zahlungsausgangsdatei, Zahlungseingangsbuchung. | Bankposten, Debitorenposten/Kreditorenposten, Sachposten, detaillierte Posten. | Bankkontenabstimmung, Bankposten, Sachkonto Bank, OP-Liste. | Bankzeile nicht zugeordnet, falsches Buchungsdatum, Doppelimport, Klärposten. | Übung Kontoauszug importieren und abstimmen. | Lösung zeigt Import, Zuordnung, Ausgleich, Buchen, Bankpostenprüfung. | `UAT-BANK-001` mit Import, Zuordnung, Abstimmung und Klärfall. | Unbekannte Zahlung; Diagnose über Bankabstimmung, Korrektur über Klärposten und spätere Zuordnung. | Deutsche Bankseiten zuerst. | Keine. |
| 21. Anlagen | 10/10 | RM-PROD kauft eine CNC-Maschine und Finance aktiviert sie als Anlage. | `Anlagen (Fixed Assets)`, `AfA-Bücher (Depreciation Books)`, `Anlagen Buch.-Blätter (FA Journals)`, `Einkaufsrechnungen (Purchase Invoices)`, `AfA berechnen (Calculate Depreciation)`. | Anlage `FA-CNC-01`, Kreditor `K30000`, Anschaffung `120.000 EUR`, Nutzungsdauer `8 Jahre`, AfA-Buch `HGB`. | Anlagenkarte, Einkaufsrechnung oder Anlagenjournal, AfA-Buchung, Abgangsbeleg. | Anlagenposten, Sachposten, Kreditorenposten bei Rechnung, USt-Posten, AfA-Sachposten. | Anlagenbuchwertbericht, Anlagenposten, Sachposten, Bilanzkonto Anlagen. | Falsche Anlagenbuchungsgruppe, falsches AfA-Startdatum, direkte Sachkontobuchung statt Anlage. | Übung Anlage kaufen und erste AfA buchen. | Lösung nennt Anlagenkarte, AfA-Buch, Einkaufsrechnung, Anlagenzeile, AfA-Lauf und Postenprüfung. | `UAT-FA-001` mit Zugang, AfA, Bericht und Sachkontoabstimmung. | Anlage auf Aufwand gebucht; Diagnose über Sachposten/Kreditorenrechnung, Korrektur über Storno und korrekte Anlagenbuchung. | Deutsch-first konsequent. | Keine. |
| 22. USt, E-Rechnung und deutsche Nachweissicht | 10/10 | RM-SALES fakturiert Inland, EU und Drittland; Finance prüft USt-Posten und E-Rechnungsnachweis. | `USt-Buchungsmatrix Einrichtung (VAT Posting Setup)`, `USt-Posten (VAT Entries)`, `USt-Abrechnung (VAT Statement)`, `E-Dokumente (E-Documents)`, `Gebuchte Verkaufsrechnungen (Posted Sales Invoices)`. | Rechnung `SO-1001`, Netto `68.000 EUR`, USt `12.920 EUR`, EU-Debitor `D12000-EU`, Drittlanddebitor `D13000-US`. | Verkaufsrechnung, E-Rechnung, USt-Abrechnung, Nachweisdokument. | USt-Posten, Sachposten, Debitorenposten, ggf. Artikelposten/Wertposten. | USt-Abrechnung, USt-Postenfilter, E-Dokumentstatus, Finanzbericht. | Falsche USt-Geschäftsbuchungsgruppe, fehlende USt-ID, falscher Leistungs-/Lieferfall, E-Rechnung nicht validiert. | Übung Inlandrechnung und EU-B2B-Prüfung. | Lösung zeigt USt-Matrix, Rechnung, Buchungsvorschau, USt-Posten und E-Dokumentstatus. | `UAT-VAT-001` mit Inland, EU-B2B, Drittland und E-Rechnungsstatus. | EU-Verkauf ohne gültige USt-ID; Diagnose über Debitor, USt-Posten und Beleg, Korrektur vor Buchung oder Gutschrift/Neubuchung. | Deutsch-first, englische Suchhilfe nur in Klammern. | Keine. |
| 23. Inventory Costing und Lagerbewertung | 10/10 | Monatsende zeigt Finance, ob Lagerwert, Wareneinsatz und Fertigungskosten zusammenpassen. | `Lagerregulierung fakt. Einst. Preise (Adjust Cost - Item Entries)`, `Lagerwert buchen (Post Inventory Cost to G/L)`, `Lagerbewertung (Inventory Valuation)`, `Wertposten (Value Entries)`. | Artikel `RAW-STEEL`, `RM-M100`, Einkauf `PO-2001`, Fertigung `PROD-3001`, Verkauf `SO-1001`. | Einkauf, Fertigungsauftrag, Verkauf, Kostenregulierung, Lagerwertbuchung. | Artikelposten, Wertposten, Sachposten, Wareneinsatzposten, Lagerbestandskonten. | Lagerbewertung, Artikelposten, Wertposten, Sachkonto Lager, GuV Wareneinsatz. | Kostenregulierung nicht gelaufen, negativer Bestand, erwartete Kosten nicht fakturiert, falsche Lagerbuchungsgruppe. | Übung Lagerwert prüfen und Kostenregulierung laufen lassen. | Lösung führt Regulierung, G/L-Buchung, Wertposten- und Sachpostenabgleich aus. | `UAT-COST-001` mit Einkauf, Verbrauch, Output, Verkauf und Lagerwertprüfung. | Kostenregulierung fehlt; Diagnose über Wertposten und Lagerbewertung, Korrektur über Stapelverarbeitung und Abstimmung. | Deutsch-first konsequent. | Keine. |
| 24. Monatsabschluss / Record-to-Report | 10/10 | RM-SHARED schließt Juni `2026` ab und prüft OP, Bank, USt, Anlagen, Lager, Projekte, GuV und Bilanz. | `Buchhaltungsperioden (Accounting Periods)`, `Finanzberichte (Financial Reports)`, `USt-Abrechnung (VAT Statement)`, `Bankkontenabstimmungen (Bank Account Reconciliations)`, `Periodische Aktivitäten (Periodic Activities)`. | Periode `06/2026`, Bankkonto `BANK-RM-01`, Finanzbericht `RM-GUV-MONAT`, Projekt `PROJ-5001`, Anlage `FA-CNC-01`. | Abschlussbuchungen, Abstimmungsberichte, USt-Abrechnung, Finanzberichte, Evidence Pack. | Sachposten, Debitorenposten, Kreditorenposten, Bankposten, USt-Posten, Anlagenposten, Artikel-/Wertposten, Projektposten. | OP-Listen, Bankabstimmung, USt-Abrechnung, Lagerbewertung, Anlagenbuchwert, GuV/Bilanz. | Offene Bankabstimmung, nicht gebuchte Lagerkosten, fehlende AfA, falsche Dimension, Periode offen. | Übung Monatsabschluss Juni durchführen. | Lösung gibt Abschlussreihenfolge mit konkreten Seiten, Filtern und Prüfergebnissen. | `UAT-R2R-001` mit Abschlusscheckliste und Evidence Pack. | Lagerkosten nicht ins Hauptbuch gebucht; Diagnose über Lagerbewertung/Sachkonto, Korrektur über `Lagerwert buchen`. | Deutsch-first konsequent. | Keine. |
| 25. Reporting, Controlling, Finanzberichte und Power BI | 10/10 | Management fragt: Welche Produktlinie verdient Geld, welcher Kanal liefert Marge und welcher Standort bindet Kapital? | `Finanzberichte (Financial Reports)`, `Analysemodus (Analysis Mode)`, `Analyseansichten (Analysis Views)`, `Power BI-Berichte (Power BI Reports)`, `Sachposten (G/L Entries)`. | Finanzbericht `RM-GUV-MONAT`, Dimensionen `PRODUCTLINE=MACHINE`, `CHANNEL=B2B`, `DEPARTMENT=SALES`, Zeitraum `06/2026`. | Berichtsausgabe, Analyseansicht, Power-BI-Dataset, Export. | Sachposten als Hauptquelle, ergänzend Debitoren-/Artikel-/Wertposten für Drilldown. | Finanzbericht, Datenanalysemodus, Power-BI-Dashboard, Sachpostenfilter. | GuV leer wegen fehlender Dimension, Analyseansicht veraltet, Power BI stimmt nicht mit BC ab. | Übung GuV nach Produktlinie und Kanal erstellen. | Lösung zeigt Finanzbericht öffnen, Spalten/Dimensionen setzen, Drilldown bis Sachposten und Power-BI-Abgleich. | `UAT-REP-001` mit GuV, Drilldown, Export und Berechtigungsprüfung. | Power BI zeigt anderen Wert als Finanzbericht; Diagnose über Zeitraum, Dimension, Refresh, Filter, Korrektur über Dataset-Aktualisierung und Modellprüfung. | Deutsch-first, BI-Begriffe nur als Suchhilfe. | Keine. |

Praktische Einordnung: Die Matrix zählt ein Kapitel nur als `10/10`, wenn jeder Nachweis sichtbar ist. Ein bloßer Hinweis wie „Posten prüfen“ genügt nicht; der Prüfpunkt nennt die konkrete Postenart, den Kontrollbericht und den Negativtest.
