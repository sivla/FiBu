...
Ohne ausdrueckliche Freigabe ist der naechste sinnvolle praktische Schritt `FIXEDASSETS-006` als read-only-Lauf: vorhandene CRONUS-Konten in `FA Posting Groups` lesen und dokumentieren, damit ein spaeterer `MACHINES`-Fit nicht auf geratenen Konten basiert.
