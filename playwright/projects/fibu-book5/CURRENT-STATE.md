...
6. `INVENTORY-001` hat O2C- und P2P-Inventory-Spuren read-only zusammengefuehrt. `INVENTORY-002` hat danach `Inventory Valuation` mit Datum, Artikel- und Lagerortfilter als Zahlenbericht nachgewiesen. `INVENTORY-003` erklaert den negativen `RM-M100`-Wert als Bestands-/Kostenketten-Lernfall. `INVENTORY-004` legt den Zielbestandsplan fest. `INVENTORY-005` belegt `Item Journals`/Page `40` als read-only Einstieg: naechster praktischer Inventory-Schritt ist eine kontrollierte Journalzeile fuer `RM-M100 +2`, Preview-/Dimensionsklaerung und nur bei passendem Zielbild genau eine Buchung; keine zweite O2C-Buchung und keine Warehouse-Aktivierung.
