...
- Anlagen: `FIXEDASSETS-001`/`FIXEDASSETS-002` sind Labor-Readiness; `FIXEDASSETS-003` oeffnet direkte Zielseiten read-only; `FIXEDASSETS-004` prueft die Zielwerte gefiltert; `FIXEDASSETS-005` erreicht `FA Posting Groups` ueber die UI. Praktisch sichtbar sind Anlagenliste `5601`, AfA-Buecher `5611`, Einkaufsrechnungen `9308`, Anlagenposten `5604` und jetzt die Seite `FA Posting Groups`. `Purchase Invoices` bleibt als Zugangspfad erreichbar. Nicht sichtbar sind weiter `FA-CNC-01`, `HGB`, `MACHINES` und `K30000`. `FA Posting Groups` ueber direkte Page-ID `5606` ist rejected, der Tell-Me-/UI-Pfad ist seit `FIXEDASSETS-005` aber belastbar; vorhandene CRONUS-Gruppen sind sichtbar. Offen bleiben UI-Setup-Fit fuer `MACHINES`, Anlage/Kreditor/AfA-Buch, Aktivierung, AfA-Buchung, Anlagenpostenspur fuer `FA-CNC-01` und deutscher Anlagen-Endstand.
