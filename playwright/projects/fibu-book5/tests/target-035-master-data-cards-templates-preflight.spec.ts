...
const CASE_ID = 'TARGET-035-MASTER-DATA-CARDS-TEMPLATES-PREFLIGHT';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'target-035-master-data-cards-templates-preflight';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-035-result.json');
...
async function probePage(page: Page, probe: Probe, index: number): Promise<ProbeResult> {
  ...
  const textSignals = compact
    ...
    .slice(0, 45);
  const screenshot = `target-035-${String(index).padStart(3, '0')}-${probe.id}.png`;
...
test('TARGET-035 master-data template preflight without record creation', async ({ page }) => {
  ...
  const result = {
    ...
    caseId: CASE_ID,
    source: 'playwright-readonly-masterdata-cards-templates-preflight',
    ...
    changedFiles: [
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-035-result.json`,
      ...
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/target-035-*.png`
    ],
    evidenceRefs: [
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-035-result.json`,
    ...
    nextStepDecisionCard: {
      ...
      lastEvidenceSummary: 'TARGET-034 classified Foundation as partial and allowed only read-only master-data card/template preflight.',
      isPlannedNextCaseStillSensible: true,
      reason: 'Master-data list surfaces must be visible and screenshot-qualified before any controlled New/template/create case can be attempted.',
      lookaheadReviewed: [
        {
          caseId: 'TARGET-036-MASTER-DATA-FIRST-CONTROLLED-WRITE-DECISION',
          status: blocked.length ? 'blocked' : 'needs-source-check-first',
          reason: blocked.length ? 'At least one master-data page route is not safe enough.' : 'The next step must decide which first record can be created and which fields/templates stay locked.'
        },
        {
          caseId: 'TARGET-037-FIRST-PREVIEW-GATE',
          status: 'blocked',
          reason: 'Preview remains blocked until real master data, accepted VAT/posting boundaries and expected entry traces exist.'
        },
        {
          caseId: 'TARGET-038-FIRST-POSTING-GATE',
          status: 'blocked',
          reason: 'Posting remains blocked until preview evidence and expected ledger/subledger traces exist.'
        },
        {
          caseId: 'TARGET-039-FOUNDATION-BLOCKER-REVISIT',
          status: 'ready-after-current',
          reason: 'Parked Page 314, VAT matrix, inventory posting and default-dimension blockers can be revisited after the first master-data route decision.'
        }
      ],
      queueChangesMade: [],
      selectedNextCase: blocked.length ? 'TARGET-035B-MASTERDATA-PAGE-ROUTE-FOLLOWUP' : 'TARGET-036-MASTER-DATA-FIRST-CONTROLLED-WRITE-DECISION',
      whySelectedNextCaseIsBest: blocked.length
        ? 'Resolve unsafe page routes before any master-data creation.'
        : 'The first controlled write decision must choose one low-risk record family and keep templates/save actions gated.',
      risksBeforeNextCase: [
  ...
  await fs.writeFile(
    ...
    [
      '# TARGET-035 Master Data Cards/Templates Preflight',
      '',
