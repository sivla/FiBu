...
Als naechstes gezielt entweder den Payments-Lernblock fortsetzen oder Reporting kontrolliert weiterklaeren. `PAYMENTS-010` hat den Buchungsdialog-/Freigabecheck vor einer moeglichen Laborzahlung erledigt: `Post` fuehrt in einen `Ja`/`Nein`-Dialog; `Nein` bricht ab; keine Zahlung wurde gebucht. Sinnvollster Payments-Lauf ist `PAYMENTS-011` nur nach ausdruecklicher Freigabe als kontrollierte Laborzahlung mit Postenspur. Ohne diese Freigabe bleibt Reporting der risikoarme Nutzenblock, aber die einfachen read-only Pfade `Dimensions - Detail`, G/L-Entries-Data-Analysis und `Analysis by Dimensions` sind aktuell negativ oder nicht belastbar. Naechster Reporting-Hebel ist deshalb ein freigegebener Analysis-View-Fit fuer `PRODUCTLINE`/`CHANNEL` oder ein alternativer offizieller Reporting-Einstieg.
