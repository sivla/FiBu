...
const specPath = 'playwright/projects/fibu-book5/tests/customer-setup-dependency-readfirst.spec.ts';
const casePath = '.agent/state/cases/customer-setup-dependency-readfirst.json';
...
function caseStatus() {
  if (!existsSync(casePath)) {
    return {
      ...
      nextStep: 'Create CUSTOMER-SETUP-DEPENDENCY-READFIRST case file before running.'
    };
  }
  let parsed;
  try {
    ...
    return {
      ...
      nextStep: 'Fix CUSTOMER-SETUP-DEPENDENCY-READFIRST case JSON before running.'
    };
  }
  ...
  const ready = correctCase && correctTarget && readOnly && mayRun;
  return {
    ready,
    blocker: ready ? '' : 'case-file-does-not-match-readfirst-target-gate',
    nextStep: ready
      ? 'Run only with --live-approved; this case is read-first/no-write.'
      : 'Align case file to playthru / UNIVERSAARL-DE read-first/no-write before running.'
  };
}

if (help) {
  console.log(`Customer setup dependency read-first guarded runner

Usage:
  node scripts/agent/run-customer-setup-dependency-readfirst.mjs --check
  node scripts/agent/run-customer-setup-dependency-readfirst.mjs --list
  node scripts/agent/run-customer-setup-dependency-readfirst.mjs --live-approved

This runner reads U-CUST-100 customer setup dependencies and setup pages with screenshot QA.
It must not edit, save, create documents, Preview Posting or Post.`);
  process.exit(0);
}
...
const blockedBy = [
  ...
  activeCaseMatches ? '' : 'active-case-is-not-customer-setup-dependency-readfirst',
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-setup-dependency-readfirst-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-setup-dependency-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run dependency read-first live before case gate, live gate, usable auth and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      CUSTOMER_SETUP_DEP_RUNNER_GUARD_CHECKED: '1',
      CUSTOMER_SETUP_DEP_LIVE_APPROVED: '1',
      CUSTOMER_SETUP_DEP_BC_TARGET_URL: targetUrl
    }
  })
);
