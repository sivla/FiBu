...
| Aktiver Case | `CUSTOMER-U-CUST-100-SETUP-REOPEN-PROOF` |
...
| Master Data | customer-setup-values-proven-readiness-next | Debitor `U-CUST-100 / Saarland Maschinenbau AG` ist auf echter `playthru`-Debitorenkarte sichtbar. `CUSTOMER-SETUP-VALUE-WRITE-GATE` hat `INLAND` als Debitorenbuchungsgruppe, `INLAND` als Geschaeftsbuchungsgruppe und `NET30` als Zlg.-Bedingungscode mit separaten sichtbaren Reopen-Screenshots bewiesen. Naechster Schritt ist nur ein read-only Reopen-/Boundary-Proof; noch kein Beleg, kein Preview, kein Posting. |
...
6. `CUSTOMER-SETUP-VALUE-WRITE-GATE` ist beobachtet: `U-CUST-100 / Saarland Maschinenbau AG` zeigt `INLAND / INLAND / NET30` in echten `playthru / UNIVERSAARL-DE`-Screenshots. Naechster Schritt ist `CUSTOMER-U-CUST-100-SETUP-REOPEN-PROOF` read-only, damit O2C nicht vor VAT-/Dimensions-/Payment-Method-Boundary-Entscheidung startet. Kein Beleg, kein Preview, kein Posting.
