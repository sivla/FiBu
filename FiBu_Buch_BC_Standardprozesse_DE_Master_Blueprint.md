...
Evidence-Pack-Stand: `FIXEDASSETS-005` belegt den UI-Pfad zu `FA Posting Groups`, `FIXEDASSETS-006` belegt vorhandene CRONUS-Konten, `FIXEDASSETS-007` belegt AfA-Bücher und Anlagenklassen. `FIXEDASSETS-012` zeigt nur leere Karten und den Vendor-Template-Dialog; diese Bilder sind keine Zielstammdaten-Screenshots. `FIXEDASSETS-014` hat `HGB` praktisch ausgeführt: `HGB` ist in `Depreciation Books` sichtbar. `FIXEDASSETS-016` hat danach `MACHINES` praktisch ausgeführt: `MACHINES` ist auf der `FA Posting Group Card` sichtbar, inklusive `12210` und `82000`. `FIXEDASSETS-017` legt als nächsten engen Lernschritt die Anlagenkarte `FA-CNC-01` fest: Erst muss sichtbar werden, wo die Anlage selbst, Beschreibung, AfA-Buch-Bezug und Anlagenbuchungsgruppe auf der Karte gepflegt oder geprüft werden. `K30000`, Einkaufsrechnung, Zugang und AfA bleiben danach eigene, spätere Klickpfade. Diese Evidence ist ein Labor-Nachweis für die Setup-Reihenfolge, kein finaler deutscher Anlagenprozess. Für finale Buchscreenshots fehlen weiterhin `FA-CNC-01`, `K30000`, Zugang, Anlagenposten, AfA-Posten und deutscher Kontenplan-/USt-Nachweis.

Didaktische Konsequenz nach `FIXEDASSETS-017`: Der nächste Buch-Screenshot darf nicht nur eine leere Anlagenliste zeigen. Er muss das zeigen, was der Leser prüfen soll: `FA-CNC-01` oder die konkrete Fixed-Asset-Card mit den relevanten Feldern. Wenn `FA-CNC-01` noch nicht sichtbar ist, ist das Bild ein Preflight- oder Fehlerbild, kein fertiges Stammdatenbild. Genau deshalb kommt die Anlagenkarte vor Kreditor `K30000` und vor der Einkaufsrechnung.
