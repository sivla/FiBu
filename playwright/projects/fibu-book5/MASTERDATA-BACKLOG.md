...
4. Inventory einfach vertiefen: `INVENTORY-002` ist als read-only Zahlenbericht erledigt, `INVENTORY-003` erklaert den negativen `RM-M100`-Wert und `INVENTORY-004` legt den Zielbestandsplan fest; naechster Schritt ist die kontrollierte positive `RM-M100`-Bestandsbewegung als eigener Lauf.
...
Fuehre genau einen kontrollierten Inventory-Readiness-Lauf aus: Identifiziere den stabilsten BC-Pfad fuer den geplanten Trainings-/Opening-Balance-Zugang `RM-M100 +2` in `FRA-ZL`. Vor einer Buchung Setup und moegliche Preview pruefen; falls sicher und freigegeben, genau eine positive Bestandsbewegung erzeugen und Artikelposten, Wertposten, Sachposten sowie `Inventory Valuation` dokumentieren. Keine neue O2C-/P2P-Buchung, keine Warehouse-Aktivierung.
```
