...
const specPath = 'playwright/projects/fibu-book5/tests/pws-ff-005b-dimension-values-related-action-route-recovery.spec.ts';
const casePath = '.agent/state/cases/pws-ff-005b-dimension-values-related-action-route-recovery.json';
...
function localCaseStatus() {
  if (!existsSync(casePath)) {
    return {
      ready: false,
      blocker: 'pws-ff-005b-case-file-missing',
      nextStep: 'Create the PWS-FF-005B case file before any live Dimension Values route recovery.'
    };
  }
  ...
  if (selected !== CASE_ID) {
    return {
      ready: false,
      blocker: 'pws-ff-005b-not-selected-next-case',
  ...
  if (caseFile.instance !== EXPECTED_INSTANCE || caseFile.company !== TARGET_COMPANY) {
    return {
      ready: false,
      blocker: 'pws-ff-005b-case-target-mismatch',
  ...
  if (caseFile.effectiveBcActionsAllowed !== false) {
    return {
      ready: false,
      blocker: 'pws-ff-005b-must-stay-readonly',
      nextStep: 'PWS-FF-005B must remain read-first/no-write.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When auth and live gate are ready, run PWS-FF-005B with --live-approved as read-first/no-write.'
  };
}

if (help) {
  console.log(`PWS-FF-005B guarded runner

Usage:
  node scripts/agent/run-pws-ff-005b-dimension-values-related-action-route-recovery.mjs --check
  node scripts/agent/run-pws-ff-005b-dimension-values-related-action-route-recovery.mjs --check --overnight
  node scripts/agent/run-pws-ff-005b-dimension-values-related-action-route-recovery.mjs --list
  node scripts/agent/run-pws-ff-005b-dimension-values-related-action-route-recovery.mjs --live-approved

This runner opens the accepted Dimensions list and tries Dimension > Dimensionswerte read-first only.
It must not create, edit, set global/default dimensions, create master data, preview, post or switch company.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-005b-dimension-values-related-action-route-recovery-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-005b-dimension-values-related-action-route-recovery-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run PWS-FF-005B live before case selection, fresh auth, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_FF_005B_RUNNER_GUARD_CHECKED: '1',
      PWS_FF_005B_LIVE_APPROVED: '1',
      PWS_FF_005B_BC_TARGET_URL: targetUrl
    }
  })
);
