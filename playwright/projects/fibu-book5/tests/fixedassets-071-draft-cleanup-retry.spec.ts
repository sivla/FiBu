...
const targetInvoiceNo = process.env.FIXEDASSETS_071_CLEANUP_PURCHASE_INVOICE_NO || '107211';
      ...
      .evaluate(() => {
        ...
        const candidates = Array.from(document.querySelectorAll<HTMLElement>('button,[role="button"],[role="menuitem"],a,[aria-label],[title]'))
          ...
          .map((element) => {
            ...
            if (/line|zeile|ausgew.hlte Zeile/i.test(label)) score += 80;
        ...
        if (!chosen) {
          return { clicked: false, reason: 'delete-action-not-found', candidates: candidates.slice(0, 20).map(({ element: _element, ...entry }) => entry) };
        }
...
test('FIXEDASSETS-071 cleanup temporary Purchase Invoice draft', async ({ page }) => {
  ...
  const cleanup = visibleBefore
    ? await clickDeleteSelectedInvoice(page, targetInvoiceNo)
    : { deleteResult: { clicked: false, reason: 'not-visible-before' }, confirmation: { confirmed: false, button: 'not-needed' } };
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-071-draft-cleanup-retry-result',
    caseId: 'FIXEDASSETS-071-GUARDED-DRAFT-LINE-TYPE-CONTEXT-PROBE',
    ...
    createdRecords: [
      {
        ...
        purpose: 'Cleanup follow-up for FA-071 guarded draft line-type context probe.',
        createdAt: '',
    ...
    nextStep:
      visibleBefore && !visibleAfter
        ? 'Continue with a selector fix for Type = Fixed Asset; do not enter target values yet.'
