...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort; `FRA-ZL` + `RESALE` wurde mit `Inventory Account = 14140` als CRONUS-Laborfit eingerichtet; O2C und P2P zeigen `Item Ledger Entry` und `Value Entry`; nach Laborbuchungen sind Wertposten und Artikelposten sichtbar | `masterdata-004`, `masterdata-005`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001`, `p2p-001` | Inventory-Posting-Setup-Luecke ist im Labor geschlossen; O2C-Artikelposten wurde ueber `Item Ledger Entry No. = 792` gefunden, P2P-Artikelposten ueber `Item Ledger Entry No. = 793`; P2P-Wertposten zeigt `RAW-STEEL` und Kosten `25.000` | Artikelposten und Wertposten in deutscher Zielcompany | Lagerbewertung/Inventory Valuation oder Kostenregulierung read-only pruefen |
...
2. P2P nicht erneut buchen: `106049` -> `108219` ist die Labor-Evidence. Naechster P2P-naher Schritt ist Zahlung/OP-Ausgleich oder Inventory/Lagerbewertung read-only.
3. Postenspur aus `UAT-O2C-001` und `UAT-P2P-001` als Anfaenger-Lernbild erklaeren: gebuchte Rechnung, Nebenbuchposten, Sachposten, Wertposten und Artikelposten.
