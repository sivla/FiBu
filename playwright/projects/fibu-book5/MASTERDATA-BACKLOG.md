...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` idempotent per BC-Standard-API an; `PAYMENTS-004` bestaetigt live, dass `BANK-RM-01` weiter sichtbar ist und Cash Receipt Journal Pflichtfelder, Gegenkonto-/Ausgleichshinweise und Journal Check zeigt | ja | ja, `payments-002`, `payments-003`, `payments-004` | `partial` | P4 | `PAYMENTS-005`: kontrollierte, bereinigbare Zahlungsjournal-Entwurfszeile vorbereiten; Bank Account Posting Group/Sachkonto-Fit und Buchung weiter nicht als erledigt markieren. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only; `PAYMENTS-004` bestaetigt im Cash Receipt Journal Posting Date, Document Type/No., Account Type/No., Amount, Bal. Account, Apply Entries und Journal Check, erstellt aber keine Zahlungszeile | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003`, `payments-004` | `partial` | P4 | Eine bereinigbare Zahlungsjournal-Entwurfszeile kontrolliert vorbereiten; noch nicht buchen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder, Gegenkonto-/Ausgleichshinweise und Journal Check read-only; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004` | `partial` | P4 | `PAYMENTS-005`: eine einzelne bereinigbare Cash-Receipt-Journal-Entwurfszeile vorbereiten; erst danach ggf. eine Laborzahlung explizit freigeben. |
...
2. P2P-Laborbuchung ist erledigt: `106049` -> `108219`. Nicht erneut buchen; `PAYMENTS-001` hat offene Posten als Startpunkt belegt, `PAYMENTS-002` hat Journal-/Apply-Readiness belegt, `PAYMENTS-003` hat `BANK-RM-01` als Laborbankkonto angelegt und `PAYMENTS-004` hat Cash Receipt Journal Pflichtfelder/Journal Check read-only bestaetigt. Naechster P2P-/O2C-naher Schritt ist eine bereinigbare Zahlungsjournal-Entwurfszeile ohne Buchung.
