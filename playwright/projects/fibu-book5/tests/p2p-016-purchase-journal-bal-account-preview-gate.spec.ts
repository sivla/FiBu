...
function compactPageText(text: string) {
  const interesting =
    /P2P016|Purchase Journals|Batch Name|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Vendor|Amount|Gen\. Bus\. Posting Group|Gen\. Prod\. Posting Group|DOMESTIC|RETAIL|Journal Check|Preview Posting|Post|K10000|Invoice|2\.500|2500|issue|must have a value/i;
...
test('p2p-016 sets Purchase Journal posting groups and gates preview by Journal Check', async ({ page }) => {
  const documentNo = `P2P016-${Date.now().toString().slice(-6)}`;
  ...
  if (midControls[14]) await fillControl(midControls[14], balAccountNo).catch(() => undefined);
  await page.waitForTimeout(2500);
  ...
  await writeTextEvidence(p2pEvidencePath('020-after-bal-account-gate-page-text.txt'), compactPageText(finalText));
  await writeJsonEvidence(p2pEvidencePath('020-after-bal-account-gate-controls.json'), finalSnapshot);
  await screenshot(page, 'p2p-016-020-bal-account-gate.png', {
    projectName: project.name,
    testId: TEST_ID,
    status: 'labor',
    bookUse: 'evidence',
    purpose: `Purchase Journal Bal.-Account-Gate fuer ${documentNo}; Preview nur nach sauberem Journal Check.`,
  ...
  const accountNoBlockerFixed = !/Account No\./i.test(journalCheck.issueText);
  const highImpactExecute = journalCheckErrorFixed || accountNoBlockerFixed || preview.opened;
  const result = {
    schemaVersion: 1,
    caseId: 'P2P-016-PURCHASE-JOURNAL-BAL-ACCOUNT-PREVIEW-GATE',
    ...
    signals: [
      ...
      ...(accountNoBlockerFixed ? ['journal-check-error-fixed'] : []),
    ...
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/p2p-016/010-initial-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-016/020-after-bal-account-gate-page-text.txt',
      'playwright/projects/fibu-book5/evidence/p2p-016/020-after-bal-account-gate-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-016/030-cleanup-page-text.txt'
    ],
    screenshots: ['playwright/projects/fibu-book5/img/p2p-016-020-bal-account-gate.png'],
    journalCheck,
    preview,
    proves: [
      'Purchase Journal posting group values DOMESTIC/RETAIL and Bal. Account No. 82000 were attempted through UI controls.',
      journalCheckErrorFixed
        ? accountNoBlockerFixed ? 'The previous Account No. Journal Check blocker is no longer the first visible issue.' : 'The Account No. blocker was tested but not proven fixed.'
        : 'The Gen. Bus. Posting Group blocker remained absent after the balancing-account attempt.',
    ...
    blockedBy: [
      ...
      ...(accountNoBlockerFixed ? [] : ['account-no-blocker-not-proven-fixed'])
    ],
    ...
    nextStep: journalCheckErrorFixed
      ? 'P2P-017: resolve the next Journal Check blocker or run a guarded Preview/Posting route if Journal Check is clean.'
      : 'P2P-017: inspect Purchase Journal balancing account field mapping before repeating any value route.'
  };
  await writeJsonEvidence(p2pEvidencePath('P2P-016-result.json'), result);
  await writeTextEvidence(
    p2pEvidencePath('README.md'),
    [
      '# P2P-016 Purchase Journal Bal. Account / Preview Gate',
      '',
      ...
      '## Ergebnis',
      '',
      `- Gen. Bus. Posting Group sichtbar: `,
