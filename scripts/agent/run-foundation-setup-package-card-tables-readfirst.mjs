...
const specPath = 'playwright/projects/fibu-book5/tests/foundation-setup-package-card-tables-readfirst.spec.ts';
const currentPath = '.agent/state/current.json';
const casePath = '.agent/state/cases/foundation-setup-package-card-tables-readfirst.json';
...
function localCaseStatus() {
  ...
  if (activeCase !== CASE_ID) {
    return {
      ready: false,
      blocker: 'foundation-package-card-tables-readfirst-not-active-case',
  ...
  return {
    ready: true,
    blocker: '',
    nextStep: 'Run the package card/table read-first proof only with --live-approved.'
  };
}

if (help) {
  console.log(`FOUNDATION package card/table read-first runner

Usage:
  node scripts/agent/run-foundation-setup-package-card-tables-readfirst.mjs --check
  node scripts/agent/run-foundation-setup-package-card-tables-readfirst.mjs --list
  node scripts/agent/run-foundation-setup-package-card-tables-readfirst.mjs --live-approved

This runner may only inspect Configuration Packages, existing package row signals, package-card/table surfaces and visible actions read-only in playthru / UNIVERSAARL-DE.
It must not create or edit packages, get tables, import, export, validate, apply, edit in Excel or write setup values.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-setup-package-card-tables-readfirst-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-setup-package-card-tables-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      FOUNDATION_PACKAGE_CARD_TABLES_RUNNER_GUARD_CHECKED: '1',
      FOUNDATION_PACKAGE_CARD_TABLES_LIVE_APPROVED: '1'
    }
  })
);
