...
Update nach `FIXEDASSETS-048-K30000-VENDOR-DEFAULTS-MANUAL-PERSONALIZE-OR-SETUP-GATE-DECISION`: Kapitel 21 bleibt vor dem Anlagenkauf gesperrt. Aus `FIXEDASSETS-047` folgt keine Kaufbeleg-Readiness und kein Default-/Setup-Fit, weil die kritischen Kreditorenfelder weiterhin nicht sichtbar belegt sind. Das Buch darf jetzt klarer lehren: `Personalisieren` ist der naechste Diagnosepfad fuer ausgeblendete Page-Felder, aber eine personalisierte Ansicht muss als solche markiert werden und ersetzt keine fachliche Setup-Entscheidung. Naechster Evidence-Schritt ist `FIXEDASSETS-049`; kein Zugang, keine AfA, keine Buchung und kein deutscher Finalnachweis.

Update nach `FIXEDASSETS-047-K30000-VENDOR-DEFAULTS-VISIBILITY-VALUE-DISCOVERY-READONLY`: Kapitel 21 hat jetzt einen scharfen praktischen Negativ-/Teilnachweis vor dem Anlagenkauf. Die `K30000`-Kreditorenkarte zeigt `Tax Liable`, `Tax Area Code`, `1M(8D)` und `BANK`, aber nicht `Vendor Posting Group`, `Gen. Bus. Posting Group`, `Currency Code` oder `VAT Bus. Posting Group`. Das Buch darf daraus nicht ableiten, dass der Kaufbeleg oder Anlagenzugang vorbereitet werden kann. Der naechste Buch-/Evidence-Schritt ist eine Gate-Entscheidung: Sichtbarkeit per Personalisieren herstellen oder einen engen UI-first Default-/Setup-Fit vorbereiten. Kein Zugang, keine AfA, keine Buchung und kein deutscher Finalnachweis.

Update nach `FIXEDASSETS-046-K30000-VENDOR-DEFAULTS-GATE-DECISION`: Kapitel 21 bleibt vor dem Anlagenkauf gesperrt. `FIXEDASSETS-045` reicht nicht fuer einen Setup-/Default-Fit, weil `Personalisieren` nur ein Diagnoseeinstieg ist und keine Werte fuer `Vendor Posting Group`, `Gen. Bus. Posting Group`, `Currency Code` oder `VAT Bus. Posting Group` beweist. Das Buch darf jetzt klarer sagen: Fehlende Kreditoren-Defaults werden zuerst UI-first/read-only als sichtbare Felder, Lookup-Optionen oder Nicht-Sichtbarkeit belegt. Kein Kaufbeleg, kein Zugang, keine AfA, keine Buchung und kein deutscher Finalnachweis.

Update nach `FIXEDASSETS-045-K30000-VENDOR-PERSONALIZE-PAGEINSPECTION-DIAGNOSIS-READONLY`: Kapitel 21 hat jetzt einen praktischen Diagnosebefund zu Personalisieren/Page Inspection. Die `K30000`-Kreditorenkarte zeigt weiter nur Teilwerte; `Personalisieren` ist im Einstellungen-Pane sichtbar, aber es wurde keine Personalisierung gespeichert und kein fehlendes Default-Feld eingeblendet. Page Inspection per `Ctrl+Alt+F1` war in diesem Browserkontext nicht stabil verfuegbar. Das Buch darf daraus nur ableiten: Fehlende Felder zuerst als UI-/Diagnoseproblem behandeln. Es darf weiterhin keine Einkaufsrechnung, keinen Anlagenzugang, keine AfA, keine Buchung und keinen deutschen Finalnachweis behaupten.

Update nach `FIXEDASSETS-043-K30000-VENDOR-INVOICING-FASTTAB-VISIBILITY-READONLY`: Kapitel 21 hat jetzt einen praktischen FastTab-Teilnachweis. Die `K30000`-Kreditorenkarte zeigt im geoeffneten `Invoicing`-Bereich `Tax Liable`, `Tax Area Code`, `Posting Details` und Withholding-Tax-Felder; `Payments` zeigt weiter `1M(8D)` und `BANK`. Das Buch darf daraus nicht ableiten, dass Einkaufsrechnung, Anlagenzugang oder Buchung freigegeben sind, weil `Vendor Posting Group`, `Gen. Bus. Posting Group`, `Currency Code` und `VAT Bus. Posting Group` weiterhin nicht sichtbar belegt sind. Naechster Buch-/Evidence-Schritt ist ein Gate-Entscheid vor jedem Kaufbeleg-Preflight.
...
| Anlagen/Projekte/Service/Manufacturing | 14-16, 21 | Fixed Assets/Warehouse/Manufacturing/Service sind als Readiness bzw. Buch-Sync teilweise belegt; Fixed Assets hat `HGB` und `MACHINES` als Labor-Setup-Prerequisites sichtbar, `FIXEDASSETS-033` plus `FIXEDASSETS-034` zeigen die vorhandene `FA-CNC-01`-Karte mit `Description = CNC Maschine FRA`, `FA Class Code = TANGIBLE`, `FA Subclass Code = EQUIPMENT`, `HGB`, `MACHINES`, `Book Value = 0,00`, AfA-Start `01.01.2026`, `8,00` Jahre und AfA-Ende `31.12.2033`; `FIXEDASSETS-038` zeigt `K30000` als Labor-Kreditor; `FIXEDASSETS-043` zeigt `Invoicing` mit `Tax Liable`/`Tax Area Code` und `Payments` mit `1M(8D)`/`BANK`, aber nicht Vendor Posting Group, Gen. Bus. Posting Group, Currency Code oder VAT Bus. Posting Group; Projects ist mit `PROJECTS-001` als Readiness gestartet und mit `PROJECTS-002` im Buch synchronisiert | teilweise | teilweise | teilweise | partial-readiness / fixedassets-labor-masterdata-fit / k30000-fasttab-partial / posting-locked | `FIXEDASSETS-044-K30000-VENDOR-PURCHASE-INVOICE-GATE-DECISION`: vor Kaufbeleg entscheiden, ob Personalisieren/Page Inspection oder ein enger no-posting Kaufbeleg-Preflight folgt; Einkaufsrechnung/Zugang/AfA nur mit spaeterem Gate |
