...
| Bank-/Journal-Readiness | `PAYMENTS-002` prueft Bankkonten, Cash Receipt Journal, Payment Journal und Apply-Entries-Pfade kontrolliert ohne Buchung. `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an und zeigt es in Bank Accounts; dieser API-Fit ist Laborhistorie, keine Buch-Klickanleitung. `PAYMENTS-004` bestaetigt live `BANK-RM-01` und zeigt im Cash Receipt Journal Pflichtfelder und Journal Check. `PAYMENTS-005` bereitet eine Zahlungsjournal-Entwurfszeile vollstaendig ueber die UI vor und loescht sie wieder. `PAYMENTS-006` klaert die Amount-Validierung: Rohzahl `-68000` fuehrt zum Amount-Issue, lokales Format `-68.000,00` loest es; nach Refresh blockiert jetzt `Bank Account Posting Group` am Balance Account `BANK-RM-01`. Keine Zahlung, kein Ausgleich, keine Bankabstimmung |
...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. Sinnvollster naechster Lauf ist `PAYMENTS-007` als UI-only Bankkonto-Posting-Fit-Diagnose: Bankkonto `BANK-RM-01` in der BC-UI oeffnen, `Bank Account Posting Group` und zugehoerigen Sachkonto-Fit pruefen, gegebenenfalls kontrolliert per UI fitten, danach denselben Cash-Receipt-Draft erneut ohne Buchung testen. Keine Zahlung buchen, keinen Ausgleich setzen und keine Bankabstimmung starten. Begruendung: `PAYMENTS-006` hat den Amount-Fehler geloest und den naechsten echten Setup-Blocker sichtbar gemacht.
