...
const CASE_ID = 'FIXEDASSETS-214-FA-GL-JOURNAL-PREVIEW-DOM-INVENTORY-AFTER-HGB-FIT';
const NEXT_CASE_ID = 'FIXEDASSETS-215-FA-GL-JOURNAL-PREVIEW-DOM-INVENTORY-REVIEW';
const NEXT_CASE_FILE = '.agent/state/cases/fixedassets-215-fa-gl-journal-preview-dom-inventory-review.json';
const TEST_ID = 'fixedassets-214';
const EXPECTED_INSTANCE = 'MCP_1_20260210';
const EXPECTED_COMPANY = project.defaultCompany;
const FA_GL_JOURNAL_PAGE_ID = 5628;
const REVIEW_EVIDENCE = 'playwright/projects/fibu-book5/evidence/fixedassets-213/FIXEDASSETS-213-result.json';
...
function statePatch(status: 'observed' | 'blocked', summary: string) {
  return {
    current: {
      ...
      lastReferenceCaseFile: '.agent/state/cases/fixedassets-214-fa-gl-journal-preview-dom-inventory-after-hgb-fit.json',
      requiresStrongModel: true,
      nextStep: 'FIXEDASSETS-215: locally review FA-214 DOM inventory before deciding any further Preview detail route or posting readiness.',
    },
    activeCase: {
      status: status === 'observed' ? 'observed-preview-dom-inventory-after-hgb-fit' : 'blocked-preview-dom-inventory-after-hgb-fit',
      lastResult: {
        status,
        resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-214/FIXEDASSETS-214-result.json',
        summary,
      },
      nextSafeAction: 'Run FIXEDASSETS-215 local review; do not post.',
    },
  };
}

test('FIXEDASSETS-214 captures Preview Posting DOM inventory after HGB fit', async ({ page }) => {
  ...
  const priorReviewAccepted =
    reviewEvidence?.resultStatus === 'blocked-ui-detail-route-next-dom-inventory' &&
    reviewEvidence?.decision?.nextCase === CASE_ID;

  try {
    if (!priorReviewAccepted) blockedBy.push('FA-213 did not route to FA-214 DOM inventory.');
  ...
  const summary =
    status === 'observed'
      ? `FA-214 captured Preview Posting DOM inventory without clicking entry groups; G/L Entry visible=${domSignals?.glEntryLabelVisible}; FA Ledger Entry visible=${domSignals?.faLedgerEntryLabelVisible}; clickable candidates=${domSignals?.candidateClickableElements}.`
      : `FA-214 stayed safe but did not complete DOM inventory: ${blockedBy.join(' | ')}`;
  const patch = statePatch(status, summary);

  await writeJsonEvidence(faEvidencePath('010-preview-dom-inventory.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-214-preview-dom-inventory-after-hgb-fit',
    caseId: CASE_ID,
    context,
    frameUrl,
    target: TARGET,
  ...
  await writeTextEvidence(faEvidencePath('020-preview-dom-inventory-text.txt'), previewContext?.compactText || 'No compact Preview Posting text was captured.');

  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-214-preview-dom-inventory-result',
    caseId: CASE_ID,
    source: 'playwright-preview-posting-dom-inventory-only',
    resultStatus: status,
    ...
    proved: [
      ...
      ...(priorReviewAccepted ? ['FA-213 routed this case to DOM/accessibility inventory after the clickable-detail blocker.'] : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-214-fa-gl-journal-preview-dom-inventory-after-hgb-fit.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-214/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-214/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-214/010-preview-dom-inventory.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-214/020-preview-dom-inventory-text.txt',
      'playwright/projects/fibu-book5/evidence/fixedassets-214/FIXEDASSETS-214-result.json',
    ],
    warnings: [
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-214-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('README.md'),
    [
      '# FIXEDASSETS-214 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `010-preview-dom-inventory.json` | JSON | exakten Preview-Pfad und DOM-/Accessibility-Signale um Posting Preview | keine Detailseite, keine Buchung | `labor` oder `blocked` |',
      '| `020-preview-dom-inventory-text.txt` | Text | kompakten sichtbaren Preview-/Fehlertext | keine Rohseite, keine Postenspur | `labor` oder `blocked` |',
      '| `FIXEDASSETS-214-result.json` | JSON | Ergebnis, Safety Flags, State-Patch-Plan | keinen deutschen Finalnachweis | `labor` |',
      '',
