...
Solange diese Option nicht nach erneutem Oeffnen sichtbar aktiv ist, wird kein Lieferant angelegt. Die Stammdatenbasis wird zuerst sortiert: Der Lagerort `SAAR-HL` ist vorhanden, der Debitor `U-CUST-100` ist vorhanden, der erste Kreditor wartet auf eine saubere Nummernserie, und der erste Artikel braucht wegen Basiseinheit, Artikelart, Lager- und Buchungsfeldern einen eigenen Kontrollschritt. So bleibt die Einrichtung nachvollziehbar und es entstehen keine Stammdaten, deren spaetere Buchungswirkung unklar ist.

Der erste Artikel heisst `U-ITEM-HW100` mit der Beschreibung `Universaarl Hardware 100`. Er ist ein Lagerartikel und verwendet die Basiseinheit `STK`. Diese Basiseinheit muss vorher auf der Seite `Einheiten` sichtbar sein. Ohne Basiseinheit kann ein Artikel zwar als Idee geplant werden, aber er ist fuer Einkauf, Verkauf und Lagerbewegungen nicht sauber erklaerbar.

Nach dem erneuten Oeffnen der Artikelliste ist `U-ITEM-HW100` mit `STK` sichtbar. Damit ist der erste einfache Artikel vorhanden. Das ist aber noch keine Buchungsreife. Auf der Artikelkarte bleiben die buchungsrelevanten Felder eigene Kontrollpunkte: Lagerbuchungsgruppe, Produktbuchungsgruppe, USt-Produktbuchungsgruppe, Einstandspreismethode und spaetere Lagerbuchungseinrichtung duerfen nicht nebenbei gesetzt werden. Sie bestimmen spaeter, welche Artikelposten, Wertposten, Sachposten und USt-Posten entstehen.

Auf der Artikelkarte hilft `Mehr anzeigen`, weil Business Central viele Felder in FastTabs versteckt. Die Karte zeigt dadurch mehr Kontext, zum Beispiel `Basiseinheit`, `Lagerbestand` und den Bereich `Einstandspreise und Buchung`. Sichtbarkeit ist aber noch keine richtige Einrichtung. Wenn ein Feld sichtbar ist, weiss man nur: Dieses Feld gehoert zur Karte und kann fachlich relevant werden. Ob der Wert richtig ist, entscheidet erst ein eigener Setup-Schritt mit erneutem Oeffnen und spaeterer Belegvorschau.

Beim Debitor ist die Lage aehnlich. `U-CUST-100` ist sichtbar, aber die wichtigen Felder fuer Forderungen, Geschaeftsbuchungsgruppe, USt-Geschaeftsbuchungsgruppe und Zahlungsbedingungen sind noch nicht als korrekt gepflegt bewiesen. Deshalb ist der Debitor ein erster Stammdatensatz, aber noch kein fertiger O2C-Ausgangspunkt.

An dieser Stelle ist die Universaarl GmbH noch nicht buchungsbereit. Ein Lagerort, ein Debitor und ein Artikel reichen fuer die Orientierung in Listen und Karten. Fuer echte Belege fehlen aber noch mehrere Grundlagen: Buchungsgruppen auf Stammdaten, USt-Einrichtung, Artikel- und Lagerbuchung, globale oder Standarddimensionen sowie ein kontrollierter Vorschau- und Buchungspfad. Deshalb wird zuerst die Foundation erneut geprueft, bevor weitere Stammdaten oder Belege angelegt werden.
