...
| Projects | `PROJECTS-001` prueft read-only die Kapitel-16-Vorstufe: Tell-Me zeigt `Projects`, `Project Planning Lines`, `Project Journals`, `Project Ledger Entries`, `Project Statistics` und `Project WIP`; `D10000` ist als Projektkunde sichtbar. Die konkreten Zielobjekte `PROJ-5001`, `RES-TECH`, `SP-SENSOR-02` und `PROJ-LAG` sind im gefilterten Laborlauf nicht als Nummern sichtbar. Keine Einrichtung, kein Projekt, keine Projektaufgaben, keine Planzeilen, kein Projektjournal, keine WIP-Berechnung, keine Rechnung, keine Buchung |
...
- `PROJECTS-001` als read-only Evidence fuer Kapitel 16: Project-/Job-Einstiege sichtbar, `D10000` sichtbar, Zielobjekte `PROJ-5001`, `RES-TECH`, `SP-SENSOR-02` und `PROJ-LAG` noch nicht tragfaehig belegt; Setup, WIP, Faktura und Buchung bleiben gate-gesperrt
...
- `PROJECTS-001` ist als read-only Evidence-Lauf gelaufen: Kapitel-16-Einstiege `Projects`, `Project Planning Lines`, `Project Journals`, `Project Ledger Entries`, `Project Statistics` und `Project WIP` sind ueber Tell-Me sichtbar. `D10000` ist als Kunde sichtbar; `PROJ-5001`, `RES-TECH`, `SP-SENSOR-02` und `PROJ-LAG` sind als konkrete Zielobjekte in `RM-DEMO` nicht sichtbar. Keine Einrichtung, keine Projektplanzeile, kein Projektjournal, keine WIP-Berechnung, keine Rechnung und keine Buchung.
...
Als naechstes gezielt `AUTOPILOT-STATE.json` und `POSTING-AND-SETUP-GATES.md` beachten. `FIXEDASSETS-008`, `WAREHOUSE-002`, `MANUFACTURING-002`, `SERVICE-001`, `SERVICE-002` und `PROJECTS-001` sind erledigt. Ohne ausdrueckliche Gate-Freigabe bleiben Fixed Assets fuer Setup und Buchung, Warehouse-Aktivierung, Manufacturing-/Assembly-Setup oder -Buchung sowie Service-/Project-Setup oder -Buchung gesperrt. Naechster sicherer Schritt ohne Freigabe ist `PROJECTS-002-BOOK-SYNC`: Kapitel 16 mit der Readiness-Evidence synchronisieren, sichtbare Project-/Job-Seiten als Readiness markieren und fehlende Zielobjekte erklaeren. Payments nur nach Payment-Gate, Reporting nur nach Analysis-View-Gate, Steuer nur nach DE-VAT-Gate, Warehouse-Aktivierung/Manufacturing/Service/Projects nur nach eigenem Gate. Shopify/Online Store ist kein naechster Prozessblock mehr; Kapitel 17 wird spaeter nur als Dropshipping/Sonderverkauf ohne Connector-Scope bearbeitet. `PAYMENTS-010` und `PAYMENTS-EVIDENCE-PACK-SYNC.md` haben den Zahlungsweg bis zur letzten Sicherheitsgrenze didaktisch geschlossen; `POSTING-TRACE-001` und `POSTING-TRACE-002` haben die gebuchten O2C-/P2P-/Inventory-Postenspuren als Anfaenger-Lernatlas und konkrete O2C-Buchstelle verbunden. `REPORTING-009` hat den letzten einfachen Sachposten-Dimensionsversuch read-only abgeschlossen: Shortcut-Spalten ja, `PRODUCTLINE`/`CHANNEL` nein. `TAX-001` hat die deutsche USt-Grenze als Readiness geklaert: ohne Freigabe kein `VAT19`-Setup und keine neue 19-%-Buchung.
