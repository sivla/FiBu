...
const specPath = 'playwright/projects/fibu-book5/tests/customer-setup-route-recovery-readfirst.spec.ts';
const casePath = '.agent/state/cases/customer-setup-route-recovery-readfirst.json';
...
function caseStatus() {
  ...
  return {
    ready,
    blocker: ready ? '' : 'case-file-does-not-match-route-recovery-readfirst-gate'
  };
}

if (help) {
  console.log(`Customer setup route recovery read-first guarded runner

Usage:
  node scripts/agent/run-customer-setup-route-recovery-readfirst.mjs --check
  node scripts/agent/run-customer-setup-route-recovery-readfirst.mjs --list
  node scripts/agent/run-customer-setup-route-recovery-readfirst.mjs --live-approved

This runner tries bounded read-only routes to setup pages blocked by direct page-id navigation.
...
const blockedBy = [
  ...
  activeCaseMatches ? '' : 'active-case-is-not-customer-setup-route-recovery-readfirst',
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-setup-route-recovery-readfirst-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-setup-route-recovery-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      CUSTOMER_SETUP_ROUTE_RECOVERY_RUNNER_GUARD_CHECKED: '1',
      CUSTOMER_SETUP_ROUTE_RECOVERY_LIVE_APPROVED: '1',
      CUSTOMER_SETUP_ROUTE_RECOVERY_BC_TARGET_URL: targetUrl
    }
  })
);
