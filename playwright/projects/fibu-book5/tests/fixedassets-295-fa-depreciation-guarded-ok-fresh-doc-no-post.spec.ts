...
const TEST_ID = 'fixedassets-295';
const CASE_ID = 'FIXEDASSETS-295-FA-DEPRECIATION-GUARDED-OK-FRESH-DOC-NO-POST';
...
test('fixedassets-295 executes guarded Calculate Depreciation OK with fresh document number and searches journal without post', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-guarded-ok-fresh-doc-no-post',
    caseId: CASE_ID,
    ...
    rebuildInstruction:
      'In the future German target company, run Calculate Depreciation only after the German fixed asset, depreciation book, batch and posting setup are proven; then trace the resulting depreciation journal line, Preview Posting and posted entries separately.',
    mustRecreateInFinalSandbox: true,
    targetGermanCompanyImpact:
      'German final evidence must recreate Calculate Depreciation, the depreciation journal line, Preview Posting and posted FA/G/L entries with German UI labels and German accounting setup.',
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 31.01.2027, FADEP-295-OK and FA-CNC-01 were visibly proven on the request page before OK.']
        : []),
      ...(okExecution.clicked ? ['OK was confirmed exactly once.'] : []),
      ...(journalLineFound ? ['FADEP-295-OK was found in Fixed Asset G/L Journals after OK.'] : ['FADEP-295-OK was not found in Fixed Asset G/L Journals after OK.']),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-295-fa-depreciation-guarded-ok-fresh-doc-no-post.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-295/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-295/FIXEDASSETS-295-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-295/FIXEDASSETS-295-GUARDED-OK-FRESH-DOC-NO-POST.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-295/010-ok-execution-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-295/020-journal-line-search.json',
    ],
    statePatch: {
      current: {
        activeCase: journalLineFound
          ? 'FIXEDASSETS-296-FA-DEPRECIATION-FRESH-OK-JOURNAL-LINE-REVIEW'
          : 'FIXEDASSETS-296-FA-DEPRECIATION-FRESH-OK-RESULT-BLOCKER-REVIEW',
        active_case_file: journalLineFound
          ? '.agent/state/cases/fixedassets-296-fa-depreciation-fresh-ok-journal-line-review.json'
          : '.agent/state/cases/fixedassets-296-fa-depreciation-fresh-ok-result-blocker-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-295-fa-depreciation-guarded-ok-fresh-doc-no-post.json',
        nextStep: journalLineFound
          ? 'FIXEDASSETS-296: locally review the FADEP-295-OK journal line and decide whether Preview Posting is now safe.'
      ...
      coverage: {
        ...
        depreciationReadiness: journalLineFound
          ? 'FA-295 confirmed OK once and found FADEP-295-OK in Fixed Asset G/L Journals. Preview Posting and Post remain locked pending local review.'
          : 'FA-295 confirmed OK once but did not find FADEP-295-OK in Fixed Asset G/L Journals. Any further repeat remains locked pending local review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-295-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-295-GUARDED-OK-FRESH-DOC-NO-POST.md'),
    [
      ...
      `- FADEP-295-OK im Journal sichtbar: ${journalLineFound ? 'ja' : 'nein'}`,
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-295 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-295-result.json` | JSON-Ergebnis | AfA-OK-Ausfuehrung nach FA-289 und Journal-Suchstatus | kein Preview/Post | labor-reference |',
      '| `FIXEDASSETS-295-GUARDED-OK-FRESH-DOC-NO-POST.md` | Lernzusammenfassung | OK-Grenze, Ergebnis und naechster Gate-Schritt | keinen deutschen Finalnachweis | labor-reference |',
      ...
      '| `020-journal-line-search.json` | JSON | Journal-Suchbefund fuer `FADEP-295-OK` | keine Buchung | compact |',
      '| `010-target-value-preflight-text.txt` | kompakter Text | sichtbarer Request-Page-Kontext | kein Rohdump | compact |',
      '',
      'Migration: `needed-for-german-final`, `mustRecreateInFinalSandbox=true`, `finalScreenshotNeeded=true`.',
      '',
    ].join('\n'),
  );
