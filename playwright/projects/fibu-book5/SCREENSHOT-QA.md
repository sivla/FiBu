...
Update nach `FIXEDASSETS-172-FA-GL-JOURNAL-BALACCOUNT-82000-PREFLIGHT`: `fixedassets-172-010-fa-gl-journal-balaccount-before.png` und `fixedassets-172-020-fa-gl-journal-balaccount-after.png` wurden visuell geprueft. Beide Bilder zeigen die richtige Seite `Fixed Asset G/L Journals`, `Batch Name = DEFAULT`, die Zeile `CNC Maschine FRA`, `Amount = 0,00`, `Bal. Account Type = G/L Account` und die Spalte `Bal. Account No.` leer. Status: brauchbare Labor-/Blockerbilder fuer Journal-Grid-/Gegenkonto-Zellhandling. Grenze: Die Bilder beweisen nicht `Bal. Account No. = 82000`, nicht `Amount = 68.000`, keine Preview Posting, keine Buchung, keine Postenspur und keinen deutschen Finalstand; sie sind deshalb keine Buch-Zielbilder fuer die Anlagenanschaffung.

Update nach `FIXEDASSETS-170-FA-BALACCOUNT-82000-SETUP-FIT`: `fixedassets-170-010-machines-balaccount-before.png` und `fixedassets-170-020-machines-balaccount-after.png` wurden visuell geprueft. Das Vorherbild zeigt die richtige Page `FA Posting Group Card`, `MACHINES`, den Abschnitt `Balancing Account` und die leere Zielzeile `Acquisition Cost Bal. Acc.`. Das Nachherbild zeigt dieselbe Zielzeile mit `82000`; der Code ist im Feldbereich lesbar und damit als Setup-After-Laborbild brauchbar. Grenze: Es beweist nur den CRONUS-USA-Labor-Setup-Fit, keine Journalwerte, keine Preview Posting, keine Buchung, keine Postenspur und keinen deutschen Kontenplan-Endstand.

Update nach `FIXEDASSETS-168-FA-BALACCOUNT-FIELD-LOOKUP-READONLY`: `fixedassets-168-010-machines-balaccount-field-context.png` und `fixedassets-168-020-fa-posting-group-patterns-readonly.png` wurden visuell geprueft. Das erste Bild ist ein brauchbares Labor-/Debuggingbild, weil es `FA Posting Group Card`, `MACHINES`, den Abschnitt `Balancing Account` und die leere Zeile `Acquisition Cost Bal. Acc.` sichtbar zeigt. Das zweite Bild ist ein brauchbares Muster-/Kandidatenbild, weil es `EQUIPMENT` mit `Acquisition Cost Bal. Acc. = 82000` sichtbar zeigt. Grenze: Keines der Bilder beweist einen gespeicherten Wert fuer `MACHINES`; kein Setup-Fit, keine Journalwerte, keine Preview Posting, keine Buchung und kein deutscher Finalstand.

Update nach `FIXEDASSETS-166-FA-BALACCOUNT-CANDIDATE-READONLY`: `fixedassets-166-010-gl-account-candidates-readonly.png` visuell geprueft. Nutzbar als Kandidaten-/Debuggingbild, weil `Chart of Accounts`, Kontencodes und Namen sichtbar sind. Status bleibt `candidate`, nicht `book-final`: Es zeigt Kontenplan-Kandidaten, aber keinen geschriebenen Wert in `FA Posting Group MACHINES / Acquisition Cost Bal. Acc.` und keine Buchungsvorschau.

Update nach `FIXEDASSETS-164-FA-POSTING-GROUP-BALACCOUNT-FIELD-PROOF`: `fixedassets-164-010-fa-posting-group-balaccount-field-proof.png` wurde visuell geprueft. Das Bild zeigt die richtige Page `FA Posting Group Card`, den Code `MACHINES`, die normalen Konto-Felder mit `12210`/`82000` und den Bereich `Balancing Account` mit `Acquisition Cost Bal. Acc.`. Status: brauchbares Labor-/Debuggingbild fuer Kapitel 21. Grenze: Die Zeile `Acquisition Cost Bal. Acc.` ist leer; das Bild beweist also gerade kein konkretes Gegenkonto und darf nicht als Freigabe fuer `Amount`, `Bal. Account No.`, Preview Posting oder `Post` verwendet werden.

Update nach `FIXEDASSETS-163-FA-GL-JOURNAL-GL-BALACCOUNT-READONLY`: `fixedassets-163-010-fa-gl-journal-balaccount-type-readonly.png` wurde visuell geprueft. Das Bild zeigt die richtige Seite `Fixed Asset G/L Journals`, `Batch Name = DEFAULT`, `Description = CNC Maschine FRA`, `Amount = 0,00`, `Bal. Account Type = G/L Account` und eine leere `Bal. Account No.`-Spalte. Status: brauchbares Labor-/Debuggingbild fuer Gegenkonto-Typ-Kontext. Grenze: Das Bild zeigt keinen Zielkontocode, keinen Betrag `68.000`, keine FA Posting Groups, keine Preview, keine Buchung, keine Postenspur und keinen deutschen Finalstand. Es darf deshalb nicht als Zielkonto- oder Anschaffungsnachweis verwendet werden.

Update nach `FIXEDASSETS-161-FA-GL-JOURNAL-GUARDED-VALUE-PREFLIGHT`: `fixedassets-161-010-fa-gl-journal-target-consistency-blocker.png` wurde visuell geprueft. Das Bild zeigt die richtige Seite `Fixed Asset G/L Journals`, `Batch Name = DEFAULT`, die rechte Journalzone mit `Description = CNC Maschine FRA`, `Amount = 0,00`, `Bal. Account Type = G/L Account` und leerer `Bal. Account No.`. Status: brauchbares Labor-/Blockerbild fuer Gegenkonto-Typ-Logik. Grenze: Das Bild zeigt nicht gleichzeitig `G05001`, `FA-CNC-01` und `HGB`; es beweist nicht `68.000`, nicht `K30000`, keine Vorschau, keine Buchung, keine Postenspur und keinen deutschen Finalstand.

Update nach `FIXEDASSETS-159-FA-GL-JOURNAL-AMOUNT-BALACCOUNT-READONLY`: Der erste Screenshotversuch war nicht buchgeeignet, weil die gesuchten Spalten `Amount` und `Bal. Account ...` nicht sichtbar waren. Der korrigierte Screenshot `fixedassets-159-010-fa-gl-journal-amount-balaccount-readonly.png` wurde visuell geprueft. Er zeigt die richtige Seite `Fixed Asset G/L Journals`, `Batch Name = DEFAULT`, `Description = CNC Maschine FRA`, `Amount = 0,00`, `Bal. Account Type = G/L Account` und die Spalte `Bal. Account No.` leer. Status: brauchbares Labor-/Debuggingbild fuer horizontales Zeilen-/Spaltenhandling und Feldsichtbarkeit. Grenze: Es beweist nicht `68.000`, nicht `K30000`, keine Vorschau, keine Buchung, keine Postenspur und keinen deutschen Finalstand.

Update nach `FIXEDASSETS-157-FA-GL-JOURNAL-LINE-OWNERSHIP-READONLY`: `fixedassets-157-010-fa-gl-journal-line-ownership-readonly.png` wurde visuell geprueft. Das Bild zeigt die richtige Seite `Fixed Asset G/L Journals`, `Batch Name = DEFAULT`, eine Zeile mit `G05001`, `Fixed Asset`, `FA-CNC-01`, `HGB`, `CNC Maschine FRA` sowie `Number of Lines = 1`, `Balance = 0.00`, `Total Balance = 0.00`. Status: brauchbares Labor-/Diagnosebild fuer Zeilenbesitz/Cleanup-Frage. Grenze: Betrag `68.000`, Gegenkonto, Preview, Buchung und Postenspur sind nicht sichtbar. Die maschinelle DOM-/Textauswertung erfasste nur Shell-Text; dieser Extractor-Blocker ist in der Evidence als technische Grenze dokumentiert.
