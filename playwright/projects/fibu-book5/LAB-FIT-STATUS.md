...
| Posting Groups | labor | Debitor-/Artikel-Posting-Fit fuer CRONUS-Techniklauf hergestellt; Inventory Posting Setup fuer `FRA-ZL` + `RESALE` auf `Inventory Account = 14140` gesetzt; O2C-Preview nach Fix erneut geprueft | `masterdata-006`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001`, `MICROSOFT-DOC-VALIDATION.md` | konkrete Inventory-Posting-Setup-Luecke ist im Labor geschlossen; `Preview Posting` zeigt echte Vorschauzeilen statt Inventory-Fehler | deutsche Posting Groups, General Posting Setup und Inventory Posting Setup | Preview-Zeilen erklaeren; deutschen Steuer-/VAT-Fit getrennt klaeren |
...
| Order-to-Cash | labor | Sales Orders gesucht, Auftrag per API erzeugt, Kopf/Zeile fotografiert, Dimensionen geprueft, Preview Posting mit Vorschauzeilen und Cleanup nachgewiesen | `playwright/projects/fibu-book5/img/uat-o2c-001-*`, `evidence/uat-o2c-001/` | Klickpfad, EUR, Menge, Lagerort, Preis, Dimension und Posting Preview funktionieren; der fruehere Inventory-Posting-Setup-Blocker ist nicht mehr vorhanden; Steuer bleibt 0 % | deutscher O2C mit 19 % USt, Buchung und Postenspur | Preview-Zeilen fachlich erklaeren; danach Steuer-/VAT-Fit oder bewussten Buchungsentscheid vorbereiten |
...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort; `FRA-ZL` + `RESALE` wurde mit `Inventory Account = 14140` als CRONUS-Laborfit eingerichtet; Preview zeigt `Item Ledger Entry = 1` und `Value Entry = 1` | `masterdata-004`, `masterdata-005`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001` | Inventory-Posting-Setup-Luecke ist im Labor geschlossen; Artikel-/Wertposten sind als Vorschau sichtbar, aber noch nicht echt gebucht | Artikelposten und Wertposten nach Lieferung/Fakturierung | Keine Buchung ohne Steuer-/Finalentscheid; Preview-Arten als Lernbild dokumentieren |
...
1. Preview-Posting-Zeilen aus `UAT-O2C-001` als Anfaenger-Lernbild erklaeren: Sachposten, Debitorenposten, Artikelposten, detaillierter Debitorenposten und Wertposten.
2. Deutsche USt-Grenze weiter offen halten: CRONUS-USA zeigt weiter keinen 19-%-USt-Endstand.
3. Danach entscheiden, ob zuerst Tax/VAT-Fit oder ein bewusst abgegrenzter Labor-Buchungslauf vorbereitet wird.
