...
Laborbefund aus `INVENTORY-006` und `INVENTORY-007`: Der Einstieg fuer diesen Trainingszugang ist `Artikeljournale (Item Journals)`. In `RM-DEMO` konnte eine Zeile `Positive Adjmt.`, `RM-M100`, Lagerort `FRA-ZL`, Menge `2` vorbereitet werden. Business Central ermittelte dabei `PCS`, Unit Amount/Amount/Unit Cost `42.000,00`/`84.000,00`; der Dimensionsdialog zeigte `PRODUCTLINE = MACHINE`. Fuer den Preflight bleibt die rechte Infobox bewusst sichtbar: `Journal Check` meldete `1 Lines checked`, `0 Lines with issues`, `0 Issues Total` und `No issues found`. Dieser Check ist wichtig, weil `Post` zwar sichtbar ist, aber ein sichtbarer Button noch keine fachliche Buchungsfreigabe bedeutet. Erst nach Zielwert-, Dimensions- und Preflight-Pruefung darf in einem Laborlauf bewusst genau einmal gebucht werden. Bis dahin beweisen die Screenshots nur den vorbereiteten Journalzustand, nicht den positiven Bestand.
