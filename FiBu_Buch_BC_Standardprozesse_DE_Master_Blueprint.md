...
| Kreditor | `K30000` | sichtbar als `Zollspedition Nord GmbH` nach `FIXEDASSETS-038`; `FIXEDASSETS-039` und `FIXEDASSETS-041` zeigen auf der Kreditorenkarte beziehungsweise im Payments-FastTab-Header `Payment Terms Code = 1M(8D)` und `Payment Method Code = BANK`, aber nicht sichtbar Vendor Posting Group, Gen. Bus. Posting Group, Currency Code oder Tax/VAT-Kontext | Labor-Kreditor ist angelegt/bestaetigt, aber nur teilweise default-geprueft. Vor Einkaufsrechnung oder Anlagenzugang muss die fehlende Buchungs-/Waehrungs-/Tax-Sichtbarkeit ueber gezielte FastTab-Expansion, Personalisieren, Seitenpruefung oder Field-/Setup-Diagnose geklaert werden |
