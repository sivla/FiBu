...
| 2 | O2C | Direkte Artikelpostenspur nach Laborbuchung finden | Page `38` blieb mit Filter `Order No. = S-ORD101068` leer; naechster Read-only-Weg ueber `Find entries...` oder gebuchte Lieferung |
| 3 | O2C | Dimension `PRODUCTLINE = MACHINE` nach Buchung in Posten/Reporting final sichtbar nachweisen | Im O2C-Labor ueber `Line` -> `Related Information` -> `Dimensions` nachgewiesen; in Postenspur noch nicht sichtbar |
| 4 | O2C | Gebuchte Postenspur didaktisch erklaeren | Laborbuchung ist erfolgt; Buchtext muss erklaeren, warum Beleg, Debitorenposten, Sachposten und Wertposten entstehen und was noch Laborgrenze ist |
