...
const TEST_ID = 'fixedassets-275';
const CASE_ID = 'FIXEDASSETS-275-FA-DEPRECIATION-ELIGIBILITY-AND-BATCH-TARGET-READONLY-DIAGNOSIS';
...
const COMPARISON_DOC_NO = 'FADEP-267-OK';
...
function classifyDiagnosis(contexts: any[]) {
  ...
  } else if (acquisitionDateVisible) {
    hypotheses.push(`Target depreciation date ${TARGET_DEPRECIATION_DATE} is after acquisition posting date ${ACQUISITION_DATE}; the old pre-acquisition-date explanation is not sufficient for ${MISSING_DOC_NO}.`);
  }
  ...
  if (!facts.journalHasMissingDoc && !facts.journalHasComparisonDoc) {
    hypotheses.push(`Current Fixed Asset G/L Journal context does not show ${MISSING_DOC_NO} or comparison document ${COMPARISON_DOC_NO}; batch/filter/output-target context remains plausible.`);
  } else if (!facts.journalHasMissingDoc && facts.journalHasComparisonDoc) {
    hypotheses.push(`Comparison document ${COMPARISON_DOC_NO} is visible but ${MISSING_DOC_NO} is not; the new post-acquisition OK result still needs output-target or eligibility diagnosis.`);
  }
  ...
  return {
    ...
    classification: facts.journalHasMissingDoc
      ...
      : targetDateBeforeAcquisition
        ...
        : 'post-acquisition-journal-line-still-not-visible-after-readonly-diagnosis',
    hypotheses,
    nextCase: 'FIXEDASSETS-276-FA-DEPRECIATION-ELIGIBILITY-BATCH-RESULT-REVIEW',
    nextCaseFile: '.agent/state/cases/fixedassets-276-fa-depreciation-eligibility-batch-result-review.json',
    nextStep: 'FIXEDASSETS-276: locally review FA-275 read-only evidence and decide whether the next step is output-target diagnosis, a setup-fit plan, or a guarded Preview-only gate if a real journal line is found.',
  };
}
...
test('FIXEDASSETS-275 diagnoses depreciation eligibility and batch target read-only', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-eligibility-and-batch-target-readonly-diagnosis',
    caseId: CASE_ID,
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-275-fa-depreciation-eligibility-and-batch-target-readonly-diagnosis.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-275/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-275/FIXEDASSETS-275-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-275/FIXEDASSETS-275-ELIGIBILITY-AND-BATCH-TARGET-READONLY-DIAGNOSIS.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-275/README.md',
    ],
    statePatch: {
      current: {
        ...
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-275-fa-depreciation-eligibility-and-batch-target-readonly-diagnosis.json',
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-275-result.json'), result);
  await writeTextEvidence(faEvidencePath('FIXEDASSETS-275-ELIGIBILITY-AND-BATCH-TARGET-READONLY-DIAGNOSIS.md'), renderLearning(result));
