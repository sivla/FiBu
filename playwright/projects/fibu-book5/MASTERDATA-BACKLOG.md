...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an, aber API-basiert und damit nicht als Buch-Klickpfad; `PAYMENTS-004` bestaetigt live Sichtbarkeit; `PAYMENTS-005` nutzt `BANK-RM-01` in einer UI-Draft-Zeile; `PAYMENTS-006` zeigt im Journal Check den fehlenden `Bank Account Posting Group`-Fit am Balance Account | ja | ja, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006` | `partial` | P4 | `PAYMENTS-007`: Bankkonto in der UI oeffnen, Bank Account Posting Group/Sachkonto-Fit pruefen und ggf. kontrolliert per UI fitten; Zahlungsbuchung weiter gesperrt. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only; `PAYMENTS-004` bestaetigt Pflichtfelder; `PAYMENTS-005` bereitet eine Cash-Receipt-Journal-Zeile per UI vor und loescht sie wieder; `PAYMENTS-006` zeigt: Rohzahl `-68000` erzeugt Amount-Issue, lokales Format `-68.000,00` loest es, danach blockiert `Bank Account Posting Group` | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006` | `partial` | P4 | Bankkonto-Posting-Fit klaeren, danach UI-Draft erneut ohne Buchung testen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder; `PAYMENTS-005` belegt UI-Draft; `PAYMENTS-006` loest Amount-Validierung per lokalem Format, zeigt aber fehlende `Bank Account Posting Group`; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006` | `partial` | P4 | `PAYMENTS-007`: Bank Account Posting Group/Sachkonto-Fit per UI pruefen/fitten; erst danach ggf. neuer nicht buchender Journal-Check. |
