...
`PROJECTS-001` hat Project-/Job-Einstiege und Zielobjekte read-only geprueft. Tell-Me zeigt `Projects`, `Project Planning Lines`, `Project Journals`, `Project Ledger Entries`, `Project Statistics` und `Project WIP`; `D10000` ist sichtbar. Die konkreten Zielobjekte `PROJ-5001`, `RES-TECH`, `SP-SENSOR-02` und `PROJ-LAG` sind im gefilterten Laborlauf nicht als Nummern sichtbar. Ohne ausdrueckliche Freigabe bleibt Project-Setup und jede Projektbuchung gesperrt: kein Projekt anlegen, keine Projektaufgaben oder Planzeilen anlegen, kein Projektjournal, keine WIP-Berechnung, keine Meilensteinrechnung. Naechster Schritt ohne Gate ist `PROJECTS-002-BOOK-SYNC`.
