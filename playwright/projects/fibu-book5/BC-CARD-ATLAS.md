...
| Chart of Accounts / Kontenplan | `16` | Foundation/Finance | `Nr.`, `Name`, `GuV/Bilanz`, `Kontoart`; FactBox `Ursprungswaehrungen` | `Neu`, `Liste bearbeiten`, Zeile oeffnet Sachkontokarte | TARGET-026B: read-only blank/insufficient list; TARGET-026F: `1200 Bank Saarland` mit `Bilanz` und `Buchung` nach Reopen | Ein einzelnes Konto ist kein vollstaendiger Kontenplan, kein SKR04-Finalclaim und keine VAT-/Posting-Setup-Freigabe |
| G/L Account Card / Sachkontokarte | `17` | Foundation/Finance | `Nr.`, `Name`, `GuV/Bilanz`, `Kontokategorie`, `Kontoart` | `Bearbeiten`; Feldfluss von `Name` per `Tab` nach `GuV/Bilanz`; Kartenwerte danach ueber Page `16` reopen pruefen | TARGET-026F: Listen-Zelle `GuV/Bilanz` blieb erst `GuV`; erfolgreiche Korrektur ueber Sachkontokarte und Reopen-Proof | Kartenfeldoptionen koennen ohne normales `aria-label` gerendert werden; nicht als normales Textinput behandeln |
