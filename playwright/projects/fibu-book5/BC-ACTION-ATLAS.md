...
| `RO-W1-GEN-POSTING-SETUP-314` | Page-314-Actions, Tooltips, `Weitere Optionen`, Spaltenmenues | TARGET-032H beobachtet `Neu`, `Liste bearbeiten`, `Kopieren...`, `Weitere Optionen`, Tooltiptexte und finalen Reopen-Proof ohne Werteingabe; TARGET-032I klassifiziert den Routenkandidaten als unzureichend | `Neu`, `Liste bearbeiten`, `Kopieren...` oder Spaltenmenues als Write-Route behandeln; `Kopieren...` verlangt laut Tooltip einen vorher angelegten neuen Datensatz und ist daher kein sicherer erster Weg | Anfaenger sehen die Buchungsmatrix-Oberflaeche; intern bleibt klar, dass Sichtbarkeit keine Kontenfindungs- oder Postingbereitschaft beweist. |
| `GATE-W1-GEN-POSTING-SETUP-314-NEW` | Eigenstaendiger `Neu`-Button auf Page 314 | TARGET-032J waehlt `Neu` als naechsten engen Standard-UI-Gate, weil Configuration Packages fuer diesen Blocker zu breit sind; TARGET-032K zeigt: `Neu` oeffnet Page 395 `Buchungsmatrixkarte Einricht.` als Karten-/Overlayroute mit leeren Feldern und ohne Zielzeile nach Reopen; TARGET-032L zeigt eine partielle Recovery mit `INLAND`/`WAREN` und `Warenverkaufskonto=4400` | `Neu` erneut fuer eine Duplikatzeile, `Liste bearbeiten`, `Kopieren...`, Konfigurationspaket-Import/Apply, VAT Setup, Preview oder Posting | Der naechste Write-Gate muss die bestehende `INLAND`/`WAREN`-Zeile oeffnen, `Warenverkaufskonto=4400` nur pruefen und ausschliesslich ein eindeutig sichtbares `Wareneinkaufskonto=5400` schreiben. Ohne dieses Feld bleibt keine Buchungsreife. |
