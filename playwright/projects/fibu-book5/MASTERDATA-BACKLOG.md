...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an, aber API-basiert und damit nicht als Buch-Klickpfad; `PAYMENTS-004` bestaetigt live Sichtbarkeit; `PAYMENTS-005` nutzt `BANK-RM-01` in einer UI-Draft-Zeile; `PAYMENTS-006` zeigt den fehlenden `Bank Account Posting Group`-Fit; `PAYMENTS-007` fittet `BANK-RM-01` per UI auf `CHECKING`; `PAYMENTS-008` zeigt den Draft nach Refresh ohne Journal-Check-Issues; `PAYMENTS-009` zeigt Apply Entries read-only aus dem Draft | ja | ja, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007`, `payments-008`, `payments-009` | `partial` | P4 | Bankkonto-Posting-Fit, Journal Check und Apply-Readiness sind im Labor erledigt; Zahlung/Ausgleich/Bankwirkung bleiben gesperrt bis Zahlungsfreigabecheck und ausdrueckliche Buchungsfreigabe. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only; `PAYMENTS-004` bestaetigt Pflichtfelder; `PAYMENTS-005` bereitet eine Cash-Receipt-Journal-Zeile per UI vor und loescht sie wieder; `PAYMENTS-006` zeigt Amount-Format und Bank-Posting-Group-Blocker; `PAYMENTS-007` loest den Bank-Posting-Group-Blocker; `PAYMENTS-008` zeigt Amount-Feld, `Amount ($)` und `Journal Check = 0 Issues` nach Refresh; `PAYMENTS-009` oeffnet Apply Entries read-only und zeigt, dass Preview Posting im Cash Receipt Journal nicht direkt sichtbar ist | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007`, `payments-008`, `payments-009` | `partial` | P4 | `PAYMENTS-010`: Zahlungsfreigabecheck/Buchungsdialog-Risiko klaeren; weiterhin keine Zahlung ohne ausdrueckliche Freigabe. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an; `PAYMENTS-004` belegt Cash Receipt Journal Pflichtfelder; `PAYMENTS-005` belegt UI-Draft; `PAYMENTS-006` zeigt Amount-Format und Bank-Posting-Group-Blocker; `PAYMENTS-007` fittet `BANK-RM-01 = CHECKING`; `PAYMENTS-008` zeigt einen nicht buchenden Draft mit `Journal Check = 0 Issues`; `PAYMENTS-009` zeigt Apply Entries read-only aus dem Draft und keine direkte Preview-Posting-Aktion; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002`, `payments-003`, `payments-004`, `payments-005`, `payments-006`, `payments-007`, `payments-008`, `payments-009` | `partial` | P4 | `PAYMENTS-010`: separate Laborzahlungsfreigabe und Buchungsdialog-/Preview-Risiko bewerten; weiterhin nicht buchen ohne Freigabe. |
