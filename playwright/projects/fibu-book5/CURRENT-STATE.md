...
- `FIXEDASSETS-001` ist als read-only Anlagen-Readiness gelaufen: Tell-Me zeigt `Fixed Assets` und `FA Ledger Entries` als Einstiegskontexte; `Depreciation Books` und `FA Posting Groups` wurden mit den englischen Suchbegriffen nicht belastbar sichtbar erreicht. Keine Anlage `FA-CNC-01`, kein AfA-Buch `HGB`, keine FA Posting Group `MACHINES`, keine Aktivierung, keine AfA und keine Anlagenposten. Buch/Testdaten muessen vor dem naechsten Schritt harmonisiert werden: Kapitel 21 nennt `120.000 EUR`, `resources-assets-projects.json` nennt `250.000`.
...
Als naechstes gezielt entweder `FIXEDASSETS-002` als Zielwert-/Seitenpfad-Klaerung fuer Kapitel 21 vorbereiten, den Payments-Lernblock nur nach Freigabe fortsetzen, Reporting nur nach Analysis-View-Freigabe weiterklaeren oder den Steuerblock nur nach DE-VAT-Setup-/Umgebungsfreigabe praktisch starten. `PAYMENTS-010` und `PAYMENTS-EVIDENCE-PACK-SYNC.md` haben den Zahlungsweg bis zur letzten Sicherheitsgrenze didaktisch geschlossen; `POSTING-TRACE-001` und `POSTING-TRACE-002` haben die gebuchten O2C-/P2P-/Inventory-Postenspuren als Anfaenger-Lernatlas und konkrete O2C-Buchstelle verbunden. `REPORTING-009` hat den letzten einfachen Sachposten-Dimensionsversuch read-only abgeschlossen: Shortcut-Spalten ja, `PRODUCTLINE`/`CHANNEL` nein. `REPORTING-008` bleibt deshalb der naechste Reporting-Hebel: ein freigegebener Analysis-View-Fit fuer `PRODUCTLINE`/`CHANNEL`. `TAX-001` hat die deutsche USt-Grenze als Readiness geklaert: ohne Freigabe kein `VAT19`-Setup und keine neue 19-%-Buchung. `FIXEDASSETS-001` hat den Anlagenblock nur als Tell-Me-Readiness gestartet; vor Setup/Buchung Zielbetrag und Seitenpfade klaeren. Ohne Freigaben nur read-only oder Buch-/Evidence-Sync waehlen.
...
10. `FIXEDASSETS-001` beweist nur erste Anlagen-Einstiegspfade ueber Tell-Me. Offen bleiben Zielwertabgleich, robuste Seitenpfade, Anlagenkarte, AfA-Buch, FA Posting Group, Einkauf/Aktivierung, AfA und Anlagenpostenspur.
...
- Anlagen: `FIXEDASSETS-001` ist nur Labor-Readiness. Praktisch sichtbar sind Tell-Me-Kontexte fuer `Fixed Assets` und `FA Ledger Entries`; nicht belastbar sichtbar mit den aktuellen Suchbegriffen sind `Depreciation Books` und `FA Posting Groups`. Nicht geprueft sind Anlagenkarte `FA-CNC-01`, AfA-Buch `HGB`, FA Posting Group `MACHINES`, Aktivierung, AfA, Anlagenposten und deutscher Anlagen-Endstand.
...
| `playwright/projects/fibu-book5/evidence/fixedassets-001/FIXEDASSETS-001-READINESS.md` | Anlagen-Readiness fuer Kapitel 21: Tell-Me-Einstiege sichtbar/negativ markiert, keine Anlage, kein Setup, keine Buchung; Zielbetrag muss harmonisiert werden |
