...
const specPath = 'playwright/projects/fibu-book5/tests/customer-payment-terms-write-gate.spec.ts';
const casePath = '.agent/state/cases/customer-payment-terms-write-gate.json';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const ACTIVE_CASE = 'CUSTOMER-PAYMENT-TERMS-WRITE-GATE';
...
function activeCaseStatus() {
  if (!existsSync(casePath)) {
    return { ready: false, blocker: 'case-file-missing' };
  }
  let parsed;
  try {
  ...
  const selected = parsed.selectedValue ?? {};
  ...
  const ready =
    ...
    selected.code === 'NET30' &&
    selected.dueDateCalculation === '<30D>' &&
  ...
  return {
    ready,
    blocker: ready ? '' : 'case-file-does-not-match-payment-terms-write-gate'
  };
}

if (help) {
  console.log(`CUSTOMER-PAYMENT-TERMS-WRITE-GATE guarded runner

Usage:
  node scripts/agent/run-customer-payment-terms-write-gate.mjs --check
  node scripts/agent/run-customer-payment-terms-write-gate.mjs --list
  node scripts/agent/run-customer-payment-terms-write-gate.mjs --live-approved

This runner may create or verify exactly one Payment Terms row:
  NET30 / 30 Tage netto / <30D>

It must not edit customers, posting groups, VAT, dimensions, documents, Preview Posting, Posting, payment or APIs.`);
  process.exit(0);
}
...
const blockedBy = [
  ...
  activeCaseMatches ? '' : 'active-case-is-not-customer-payment-terms-write-gate',
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-payment-terms-write-gate-check',
        caseId: ACTIVE_CASE,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'customer-payment-terms-write-gate-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run payment terms write gate before case gate, live gate, usable auth and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      CUSTOMER_PAYMENT_TERMS_RUNNER_GUARD_CHECKED: '1',
      CUSTOMER_PAYMENT_TERMS_LIVE_APPROVED: '1',
      CUSTOMER_PAYMENT_TERMS_BC_TARGET_URL: targetUrl
    }
  })
);
