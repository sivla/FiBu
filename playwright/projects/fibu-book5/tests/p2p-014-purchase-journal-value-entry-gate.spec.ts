...
function compactPageText(text: string) {
  const interesting =
    /P2P014|Purchase Journals|Batch Name|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Vendor|Amount|Bal\. Account|Journal Check|Post|K10000|Invoice|2\.500|2500/i;
  ...
  return {
    ...
    issueText:
      text
        .split('\n')
        .map((line) => line.trim())
        .find((line) => /must have a value|muss.*Wert enthalten|does not exist|Posting Group|Gen\. Journal Line/i.test(line)) ?? ''
  };
}
...
test('P2P-014 enters Purchase Journal vendor invoice values and cleans up without posting', async ({ page }) => {
  const documentNo = `P2P014-${Date.now().toString().slice(-6)}`;
  ...
  const attempts: Array<{ index: number; value: string; ok: boolean }> = [];
  const fill = async (index: number, value: string) => {
    const ok = await fillControl(initialControls[index], value)
      .then(() => true)
      .catch(() => false);
    attempts.push({ index, value, ok });
    await page.waitForTimeout(900);
  };
  ...
  if (midControls[7]) await fillControl(midControls[7], `P2P-014 lab purchase journal ${documentNo}`).catch(() => undefined);
  ...
  const evidenceText = compactPageText(finalText);
  await writeTextEvidence(p2pEvidencePath('020-after-value-entry-page-text.txt'), evidenceText);
  await writeJsonEvidence(p2pEvidencePath('020-after-value-entry-controls.json'), finalSnapshot);
  await screenshot(page, 'p2p-014-020-purchase-journal-values.png', {
    projectName: project.name,
    testId: TEST_ID,
    status: finalText.includes(documentNo) || finalValues.includes(documentNo) ? 'labor' : 'rejected',
    bookUse: 'evidence',
    purpose: `Purchase Journal Wert-Eingabe-Gate fuer ${documentNo}; kein Posting.`,
    expectedPageText: [/Purchase Journals/i],
    knownLimitations: ['Keine Buchung', 'Keine Preview', 'Kein deutscher Finalnachweis']
  });

  const cleanup = await cleanupDraftLine(page, documentNo);
  const cleanupText = compactPageText(await pageText(page));
  await writeTextEvidence(p2pEvidencePath('030-cleanup-page-text.txt'), cleanupText);

  const documentNoVisible = finalText.includes(documentNo) || finalValues.includes(documentNo);
  const vendorVisible = finalText.includes(vendorNo) || finalValues.includes(vendorNo);
  const amountVisible = /2[.,]500|2500/i.test(finalText) || finalValues.some((value) => /2[.,]500|2500/i.test(value));
  const result = {
    schemaVersion: 1,
    caseId: 'P2P-014-PURCHASE-JOURNAL-VALUE-ENTRY-GATE',
    parentCaseId: 'BC-DEEP-RUN-001-MULTI-ROUTE-SANDBOX-PROGRESS',
    source: 'playwright-ui-labor-journal-value-entry',
    ...
    posted: false,
    setupChanges: [],
    createdRecords: cleanup.cleaned ? [] : [{ type: 'Purchase Journal Line', documentNo, keepReason: 'cleanup-not-proven' }],
    ...
    targetValues: { postingDate, documentType: 'Invoice', accountType: 'Vendor', vendorNo, amount },
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/p2p-014/010-initial-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-014/020-after-value-entry-page-text.txt',
      'playwright/projects/fibu-book5/evidence/p2p-014/020-after-value-entry-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-014/030-cleanup-page-text.txt'
    ],
    screenshots: ['playwright/projects/fibu-book5/img/p2p-014-020-purchase-journal-values.png'],
    journalCheck,
    proves: [
      'Purchase Journal value entry was attempted through UI controls.',
    ...
    blockedBy: [
      ...
      ...(vendorVisible ? [] : ['vendor-not-visible-after-entry']),
      ...(amountVisible ? [] : ['amount-not-visible-after-entry']),
    ...
    statePatch: {},
    nextStep:
      documentNoVisible && cleanup.cleaned
        ? 'P2P-015: decide whether Purchase Journal can be advanced to Preview/Posting gate or whether Purchase Invoice line gate is safer.'
        : 'Use the captured controls to refine Purchase Journal mapping, or switch to Payment/Apply controlled lab case.'
  };
  await writeJsonEvidence(p2pEvidencePath('P2P-014-result.json'), result);
  await writeTextEvidence(
    p2pEvidencePath('README.md'),
    [
      '# P2P-014 Purchase Journal Value Entry Gate',
      '',
      'Status: `execute-attempt`, `field-value-entry`, `no-posting`, `needs-german-final-rebuild`.',
      '',
      `Document No.: \`${documentNo}\``,
      '',
      'Dieser Lauf versucht bewusst eine Purchase-Journal-Werteingabe als P2P-Wertpfad. Er bucht nicht und oeffnet kein Preview Posting.',
      '',
      '## Ergebnis',
      '',
      `- Document No sichtbar: ${documentNoVisible ? 'ja' : 'nein'}`,
      `- Vendor sichtbar: ${vendorVisible ? 'ja' : 'nein'}`,
      `- Amount sichtbar: ${amountVisible ? 'ja' : 'nein'}`,
      `- Cleanup bewiesen: ${cleanup.cleaned ? 'ja' : 'nein'}`,
      '',
      '## Grenze',
      '',
      'Keine Buchung, kein Preview, kein Ledger Trace, kein deutscher Finalnachweis.'
    ].join('\n')
  );
