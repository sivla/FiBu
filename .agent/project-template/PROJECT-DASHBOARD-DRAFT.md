...
| Aktiver Case | `ITEM-SERVICE-MASTERDATA-READFIRST` |
...
| Master Data | customer-boundaries-decided-item-service-next | Debitor `U-CUST-100 / Saarland Maschinenbau AG` ist auf echter `playthru`-Debitorenkarte sichtbar. `CUSTOMER-VAT-DIMENSION-BOUNDARY-DECISION` haelt fest: VAT Business Posting Group, Dimensionen und Zahlungsform bleiben bewusst unbewiesen; kein Beleg, kein Preview, kein Posting. Naechster Schritt ist `ITEM-SERVICE-MASTERDATA-READFIRST`: realistische fiktive Universaarl-Artikel/Services read-first pruefen, bevor O2C/P2P-Routen entstehen. |
...
6. `CUSTOMER-VAT-DIMENSION-BOUNDARY-DECISION` ist abgeschlossen: U-CUST-100 ist fuer Debitorenkarten-Handbuch/Training nutzbar, aber O2C-Write/Preview/Posting bleibt setup-gated. Naechster Schritt ist `ITEM-SERVICE-MASTERDATA-READFIRST` mit Screenshot-QA. Kein Beleg, kein Preview, kein Posting.
7. Masterdaten sollen realistische fiktive Universaarl-Geschaeftspartner und Produkte verwenden, keine flachen Mockups und keine vertraulichen echten Kundendaten.
