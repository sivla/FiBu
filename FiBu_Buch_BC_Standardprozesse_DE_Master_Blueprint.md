...
|---|---|---|---|
| Anlage | `FA-CNC-01` | im Listen-/Seitentext vor `Neu` nicht sichtbar; leere `Fixed Asset Card` als Preflight sichtbar; nach `FIXEDASSETS-020` sind die Feldpfade fuer `No.`, `Description`, `FA Class Code`, `FA Subclass Code`, `Depreciation Book Code` und `Posting Group` nach `Mehr anzeigen` sichtbar; `FIXEDASSETS-024` mappt diese Feldzeilen als aktive Vordergrundkarten-Controls | Anlagenkarte erst nach Werte-/Lookup- oder Save-Gate-Entscheidung speichern; `FIXEDASSETS-024` beweist Control-Erreichbarkeit, aber noch keine Zielwerte `FA-CNC-01`, `HGB` oder `MACHINES` |
...
Nachweis nach `FIXEDASSETS-024`: Der Active-Card-Control-Helper trennt die sichtbare Anlagenkarte von der Hintergrundliste. Playwright kann die Feldzeilen fuer `FA Class Code`, `FA Subclass Code`, `Depreciation Book Code`, `Posting Group` und AfA-Daten als Vordergrundkarten-Controls erkennen. Das ist ein technischer Fortschritt fuer stabile Klickanleitungen, aber noch kein Stammdatennachweis. Fuer Leser heisst das: Ein Feld ist erreichbar, aber der Wert ist erst dann fachlich belegt, wenn der Code selbst sichtbar im Feld steht. Der naechste Schritt entscheidet deshalb Werte-/Lookup-Plan oder Save-Gate; `FA-CNC-01` bleibt bis dahin ungespeichert.
