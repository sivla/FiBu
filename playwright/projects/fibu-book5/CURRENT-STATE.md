...
- `SECURITY-002` ist als Buch-/Evidence-Sync ohne BC-Lauf dokumentiert: Kapitel 27 hat jetzt eine Statusbox zum Labor-Nachweis in `RM-DEMO`. Das Buch erklaert, dass Profil/Rolle, Permission Sets, Security Groups, User Setup, Job Queue und Change Log getrennte Prueffelder sind. Der Lauf hat keine Benutzer-, Berechtigungs-, Profil-/Rollen-, Security-Group-, User-Setup-, Job-Queue- oder Change-Log-Aenderung und keine Buchung erzeugt. Naechster Schritt ohne Gate ist `MIGRATION-001-READINESS`.
