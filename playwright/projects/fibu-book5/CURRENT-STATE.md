...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. Sinnvollster naechster Lauf ist `PAYMENTS-009` als nicht buchender Apply-Entries-/Preview-Readiness-Lauf fuer den jetzt zahlungsreifen Cash-Receipt-Draft: denselben Draft fuer `D10000`/`PS-INV103297` mit `BANK-RM-01`, Betrag `-68.000,00` und `Journal Check = 0 Issues` vorbereiten, Apply-/Applies-to-Bezug sowie ggf. Buchungsvorschau pruefen, Screenshot/Evidence sichern und Entwurf wieder loeschen. Keine Zahlung buchen, keinen OP-Ausgleich buchen und keine Bankabstimmung starten. Begruendung: `PAYMENTS-008` hat den Amount-Preflight nach Bankkonto-Fit geloest; offen ist jetzt die nicht buchende Kontrolle des Ausgleichsbezugs vor einer spaeteren, ausdruecklich freigegebenen Laborzahlung.
