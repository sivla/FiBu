...
const CASE_ID = 'FIXEDASSETS-138-FA-CNC-01-POSTING-GROUP-ASSIGNMENT-FIT';
const TEST_ID = 'fixedassets-138';
const NEXT_CASE_ID = 'FIXEDASSETS-139-FA-CNC-01-ASSIGNMENT-FIT-REVIEW';
...
async function fillPostingGroup(page: Page) {
  const frame = await findCardFrame(page);
  const target = await frame.evaluate((input) => {
    const clean = (value: string | null | undefined) => (value || '').replace(/\s+/g, ' ').trim();
    const visible = (element: Element) => {
      const rect = element.getBoundingClientRect();
      const style = window.getComputedStyle(element);
    ...
    const labels = Array.from(document.querySelectorAll<HTMLElement>('a,span,div,label,[role="button"]'))
      ...
      .filter((entry) => exact.test(entry.text) || exact.test(entry.aria) || exact.test(entry.title))
      .filter((entry) => entry.y > 130 && entry.x > 40 && entry.x < 1250);
    ...
    const controls = Array.from(document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('input,textarea,select'))
      ...
      .filter((control) => !control.disabled && !control.readOnly && control.y > 130 && control.x > 40 && control.x < 1750);
    ...
    return chosen
      ...
      : { found: false, inputIndex: -1 };
  }, { targetValue: TO_POSTING_GROUP });

  if (!target.found || target.inputIndex < 0) {
    return { attempted: false, filled: false, reason: 'posting-group-editable-control-not-found', target };
  }

  await page.mouse.click(target.x + Math.min(target.width - 4, 80), target.y + 8);
  await page.keyboard.press('Control+A').catch(() => undefined);
  await page.keyboard.type(TO_POSTING_GROUP, { delay: 20 });
  await page.keyboard.press('Tab').catch(() => undefined);
  await page.waitForTimeout(1200);
  let rowsAfter = await readVisibleRows(page);
  let valueAfter = valueOf(rowsAfter, 'Posting Group');
  let dropdownFallback: Record<string, unknown> | undefined;
  if (normalize(valueAfter) !== TO_POSTING_GROUP) {
    await page.mouse.click(target.x + target.width - 18, target.y + 10);
    await page.waitForTimeout(1200);
    const targetPattern = new RegExp(`^${TO_POSTING_GROUP}$`, 'i');
    let selected = false;
    const attempts: string[] = [];
    for (const scope of [page, ...page.frames()]) {
      const candidates = [scope.getByRole('row', { name: new RegExp(TO_POSTING_GROUP, 'i') }), scope.getByText(targetPattern)];
      for (const locator of candidates) {
        const count = await locator.count().catch(() => 0);
        for (let index = 0; index < count; index += 1) {
          const candidate = locator.nth(index);
          const text = await candidate.innerText({ timeout: 500 }).catch(() => '');
          if (/\b(New|Neu|Edit|Delete|Post|Preview|Acquire|OK|Yes|Ja)\b/i.test(text)) {
            attempts.push(`rejected-dangerous-candidate:${text.replace(/\s+/g, ' ').trim()}`);
            continue;
          }
          if (!(await candidate.isVisible({ timeout: 500 }).catch(() => false))) {
            continue;
          }
          selected = await candidate.click({ timeout: 4000 }).then(() => true).catch(() => false);
          if (selected) break;
        }
        if (selected) break;
      }
    ...
    rowsAfter = await readVisibleRows(page);
    valueAfter = valueOf(rowsAfter, 'Posting Group');
    dropdownFallback = { selected, attempts, valueAfter };
  }
  return {
    attempted: true,
...
function renderLearning(result: Record<string, any>) {
  return [
    '# FIXEDASSETS-138 - FA-CNC-01 Posting Group Assignment Fit',
    '',
...
async function writeResult(page: Page, partial: Record<string, any>) {
  const conditionalEvidenceRefs = [
    ...(partial.after ? ['playwright/projects/fibu-book5/evidence/fixedassets-138/050-after-visible-rows.json'] : []),
    ...(partial.screenshotCaptured ? ['playwright/projects/fibu-book5/evidence/fixedassets-138/fixedassets-138-060-fa-cnc-01-posting-group-machines.screenshot.json'] : []),
    ...(partial.screenshotCaptured ? ['playwright/projects/fibu-book5/img/fixedassets-138-060-fa-cnc-01-posting-group-machines.png'] : []),
  ];
  const result = {
    schemaVersion: 1,
    ...
    changedFiles: [
      'playwright/projects/fibu-book5/tests/fixedassets-138-fa-cnc-01-posting-group-assignment-fit.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-138/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-138/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-138/FIXEDASSETS-138-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-138/FIXEDASSETS-138-posting-group-assignment-fit.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-138/010-before-visible-rows.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-138/020-fa-ledger-entries-safety.txt',
    ...
    statePatch: {
      current: {
        updatedAt: '2026-06-20T20:55:00.000Z',
        activeCase: NEXT_CASE_ID,
        active_case_file: '.agent/state/cases/fixedassets-139-fa-cnc-01-assignment-fit-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-138-fa-cnc-01-posting-group-assignment-fit.json',
        requiresStrongModel: true,
        nextStep: 'FIXEDASSETS-139: review FA-138 assignment-fit evidence before any acquisition route is unlocked.',
      },
      activeCase: {
        status: partial.resultStatus,
        lastResult: {
          resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-138/FIXEDASSETS-138-result.json',
          summary: partial.summary,
        },
        nextSafeAction: 'FIXEDASSETS-139: local review of assignment-fit evidence.',
      },
    },
    ...partial,
  };

  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-138-result.json'), result);
  await writeTextEvidence(faEvidencePath('FIXEDASSETS-138-posting-group-assignment-fit.md'), renderLearning(result));
  await writeTextEvidence(
    faEvidencePath('README.md'),
    [
      '# fixedassets-138 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-138-result.json` | JSON | Ergebnis, Safety, Flags, State-Patch-Plan | keinen deutschen Finalnachweis | `labor` |',
      '| `FIXEDASSETS-138-posting-group-assignment-fit.md` | Markdown | Lernwert und Buchwirkung | keine Anschaffung/Buchung | `labor` |',
      ...
      ...(partial.screenshotCaptured
        ? ['| `../../img/fixedassets-138-060-fa-cnc-01-posting-group-machines.png` | Screenshot | sichtbarer Kartenkontrollpunkt oder Blockerbild | nicht alleinige Wahrheit | `candidate/labor` |']
        : []),
      '',
...
test('FIXEDASSETS-138 fits FA-CNC-01 Posting Group to MACHINES with UI-first safety gates', async ({ page }) => {
  ...
  if (safetyIssues.length > 0) {
    ...
    await clickVisibleShowMoreControls(page);
    await screenshot(page, 'fixedassets-138-060-fa-cnc-01-posting-group-machines.png', {
      projectName: project.name,
      testId: TEST_ID,
      status: 'labor',
      bookUse: 'field-proof',
      purpose:
        'FA-138 Blockerbild: FA-CNC-01 zeigt Posting Group EQUIPMENT, aber Book Value/Acquired sind fuer den Safety-Gate nicht sichtbar genug; deshalb keine Aenderung.',
    ...
    await writeResult(page, {
      ...
      summary: `FA-138 blocked before edit: ${safetyIssues.join('; ')}`,
      screenshotCaptured: true,
      bookImpact: 'Kapitel 21 darf den Posting-Group-Fit nicht behaupten, solange der Sicherheitsgate blockiert.',
      nextStep: 'FIXEDASSETS-139: Blocker lokal bewerten und naechsten sicheren UI-first Hebel festlegen.',
    });
    return;
  }
  ...
  if (beforePostingGroup === TO_POSTING_GROUP) {
    await screenshot(page, 'fixedassets-138-060-fa-cnc-01-posting-group-machines.png', {
      ...
      status: 'candidate',
      bookUse: 'field-proof',
      purpose: 'FA-138: FA-CNC-01 zeigt bereits MACHINES als Posting Group; kein Edit erforderlich.',
    ...
    await writeResult(page, {
      ...
      nextStep: 'FIXEDASSETS-139: review assignment evidence before acquisition route.',
    });
    return;
  }
  ...
  if (!editIcon.clicked) {
    ...
    if (!postingGroupStillEditable) {
      await writeResult(page, {
        ...
        requiresReview: true,
        summary: 'FA-138 blocked: edit icon could not be clicked safely and Posting Group is not editable.',
        bookImpact: 'Kapitel 21 muss den Karten-Fit weiter als offen markieren.',
        nextStep: 'FIXEDASSETS-139: review edit action blocker.',
      });
      return;
    }
  }
  ...
  if (!dialogBeforeFill.ok) {
    await writeResult(page, {
      ...
      requiresReview: true,
      summary: 'FA-138 blocked: a dangerous dialog appeared before filling Posting Group.',
      bookImpact: 'Keine Buchwirkung; zuerst Dialogursache klaeren.',
      nextStep: 'FIXEDASSETS-139: review dangerous dialog evidence.',
    });
    return;
  }
  ...
  if (!fillResult.filled) {
    await screenshot(page, 'fixedassets-138-060-fa-cnc-01-posting-group-machines.png', {
      ...
      bookUse: 'field-proof',
      purpose:
        'FA-138 Feldfuell-Blocker: FA-CNC-01 zeigt weiter EQUIPMENT als Posting Group; Tippen/Dropdown-Fallback haben MACHINES nicht final gesetzt.',
    ...
    await writeResult(page, {
      ...
      requiresReview: true,
      screenshotCaptured: true,
      summary: 'FA-138 blocked: Posting Group field was not filled to MACHINES.',
      bookImpact: 'Kapitel 21 muss den Posting-Group-Kartenfit weiter offen halten.',
      nextStep: 'FIXEDASSETS-139: review field-fill blocker.',
    });
    return;
  }
  ...
  await screenshot(page, 'fixedassets-138-060-fa-cnc-01-posting-group-machines.png', {
    ...
    purpose: changedPostingGroup
      ? 'FA-138: FA-CNC-01 zeigt nach Neuoeffnen MACHINES als Posting Group; kontrollierter UI-first Setup-Fit ohne Anschaffung/Buchung.'
      : 'FA-138: Kartenbild nach Fill-Versuch; MACHINES ist nicht final als Posting Group nachgewiesen.',
  ...
  await writeResult(page, {
    ...
    summary: changedPostingGroup
      ? 'FA-138 changed FA-CNC-01 Posting Group from EQUIPMENT to MACHINES through the UI and confirmed the value after reopening the card.'
      : `FA-138 did not confirm MACHINES after reopen. Visible value: ${after.postingGroup || '(blank)'}.`,
    ...
    nextStep: 'FIXEDASSETS-139: review FA-138 assignment-fit evidence before unlocking any acquisition route.',
  });
});
