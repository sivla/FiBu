...
| Reportingdimensionen in Posten und Analysepfaden suchen | `G/L Entries`, `Item Ledger Entries`, `Financial Reports`, `Analysis Views`, `Analysis by Dimensions` | `REPORTING-002` bis `REPORTING-010` | `playwright/projects/fibu-book5/img/reporting-002-046-item-ledger-entry-792-dimensions.png`, `playwright/projects/fibu-book5/img/reporting-004-030-revenue-analysis-view-card.png`, `playwright/projects/fibu-book5/img/reporting-006-010-gl-entries-before-analysis.png`, `playwright/projects/fibu-book5/img/reporting-007-010-tell-me-analysis-by-dimensions.png`, `playwright/projects/fibu-book5/img/reporting-009-020-gl-entry-dimensions-result.png` | `playwright/projects/fibu-book5/evidence/reporting-002/` bis `playwright/projects/fibu-book5/evidence/reporting-010/` | geprüft als read-only Laborbefund: `PRODUCTLINE=MACHINE` und `CHANNEL=B2B` sind am Artikelposten sichtbar; G/L Entries zeigen in breiter Ansicht nur Shortcut-Spalten wie `Department Code`/`Customergroup Code`; `PRODUCTLINE`/`CHANNEL`, `REVENUE`, `Dimensions - Detail`, G/L-Entries-Data-Analysis und der Tell-Me-Pfad `Analysis by Dimensions` liefern noch keinen belastbaren Reportingnachweis. Naechster echter Hebel ist nur mit Freigabe ein Analysis-View-Fit oder ein anderer belegbarer Standardpfad |
...
Im Kontenplan ist deshalb nicht nur die Nummer wichtig, sondern auch der fachliche Zweck. Fuer die Universaarl GmbH ist `1800 Bank Saarland` das Bank-Sachkonto. `1200` gehoert im SKR04-Kontext nicht in die Banklogik, sondern in den Forderungsbereich. Wenn eine falsche Kombination sichtbar ist, wird sie nicht fuer Buchungsgruppen, Zahlungen oder Umsatzsteuer verwendet, bis sie fachlich bereinigt ist.

Im Kontenplan sieht man bei jedem Sachkonto die Nummer, den Namen, `GuV/Bilanz` und die Kontoart. `1800 Bank Saarland` steht als Bilanzkonto fuer spaetere Bank- und Zahlungsprozesse. `1406 Abziehbare Vorsteuer 19 Prozent` steht ebenfalls als Bilanzkonto und wird spaeter fuer die Einkaufs-USt gebraucht. `3300 Verbindlichkeiten aus Lieferungen und Leistungen` ist das spaetere Sammelkonto fuer Kreditoren. `3806 Umsatzsteuer 19 Prozent` ist das spaetere Umsatzsteuerkonto fuer Verkaufsprozesse. Umsatz- und Aufwandskonten gehoeren dagegen in die GuV. `4400 Umsatzerloese Inland 19 Prozent` und `5400 Wareneingang / Materialaufwand` duerfen deshalb erst fuer Verkaufs- oder Einkaufsbuchungen verwendet werden, wenn sie im Kontenplan als `GuV` und `Buchung` sichtbar sind. Diese Konten allein buchen aber noch keine Vorsteuer, Umsatzsteuer, Erloese, Aufwendungen oder Kreditorenposten. Erst wenn Buchungsgruppen, Buchungsmatrix und VAT Posting Setup zusammenpassen, kann Business Central beim Einkauf oder Verkauf automatisch die richtigen Sach- und Steuerposten erzeugen.
...
Aktueller Laborhinweis für `RM-DEMO`: Der O2C-Mindeststand ist praktisch nachgewiesen (`PRODUCTLINE=MACHINE`, `CHANNEL=B2B`, `DEPARTMENT=SALES`, `LOCATION-GROUP=DIRECTED`). Zusätzlich wurden die ersten P1-Erweiterungswerte für Einkauf und Lager vorbereitet: `DEPARTMENT=PURCH`, `DEPARTMENT=WHSE`, `PRODUCTLINE=SPARE` und `LOCATION-GROUP=SIMPLE`. Die vollständige Buchmatrix ist damit noch nicht automatisch fertig. `COMPANY-GROUP` sowie Service-, Projekt-, Shop- und Intercompany-Werte bleiben im aktuellen CRONUS-USA-Labor Folgearbeit. Die Standard-API kann diese Dimensionsstammdaten lesen, erlaubt aber keinen Insert; fehlende Werte werden deshalb nicht heimlich erzeugt, sondern als gezielter UI-Setup-Schritt geplant.
...
Der Playwright-Lauf `UAT-P2P-001` ist inzwischen als kontrollierter CRONUS-USA-Laborprozess belegt. `K10000` und `RAW-STEEL` existieren in der Spielwiese. Der Kreditor brauchte ein Vendor Template, weil Business Central Einkaufsbelege nur dann belastbar erzeugen kann, wenn die Kreditoren-/Posting-Vorgaben tragen. Der Artikel `RAW-STEEL` nutzt für den Laborfit `Gen. Prod. Posting Group = RETAIL`, `Inventory Posting Group = RESALE` und `Tax Group Code = FURNITURE`; das ist ein technischer CRONUS-Fit, kein deutscher Rohmaterial-/Vorsteuer-Endstand.

Die Einkaufsbestellung `106049` wurde mit `K10000`, `RAW-STEEL`, Menge `10`, Lagerort `FRA-ZL` und `Direct Unit Cost = 2.500` angelegt. Vor `Preview Posting` musste `Vendor Invoice No.` gepflegt werden. Das ist fachlich die externe Belegnummer des Lieferanten; ohne sie zeigt BC eine Fehlermeldung und stoppt vor Preview/Buchung. Technisch ist das Feld nicht in der v2.0-Standard-API `purchaseOrders` enthalten, aber über ODataV4 `purchaseDocuments.vendorInvoiceNumber` erreichbar.
...
2. Erfasse `Kontenart = Debitor`, `Kontonr. = D10000` und den offenen Rechnungsbetrag; im aktuellen CRONUS-Labor ist das `-68.000,00`, im deutschen Zielbeispiel spaeter `-80.920,00`.
...
13. Öffne OP-Liste Debitoren und dokumentiere den Nachweis.
...
| Evidence Pack | Aktuell vorhanden: Buch.-Blatt-Zeile, Apply-Entries-Readiness, Journal Check, Post-Dialog, gebuchte Laborzahlung, Debitorenposten nach Ausgleich, detaillierte Debitorenposten, Sachposten und Bank Account Ledger Entries ueber Page `372`. Offen: Bankabstimmung, deutscher Finalnachweis. |
...
| `fixedassets-297` | read-only Ursachenpruefung: der alte Beleg `FADEP-267-OK` hatte ein Datumsproblem, weil `30.06.2026` vor dem Zugang `01.01.2027` lag; fuer `FADEP-295-OK` bleibt die genaue Ursache offen | der alte Datumsfehler erklaert nicht automatisch alle spaeteren OK-Laeufe; ohne neue Hypothese kein weiterer OK-, Preview- oder Post-Versuch |
...
- `playwright/projects/fibu-book5/evidence/fixedassets-297/FIXEDASSETS-297-ELIGIBILITY-CAUSE-READONLY.md`
...
11. Oeffne `AfA berechnen (Calculate Depreciation)` und berechne AfA fuer den deutschen Zielstichtag erst, nachdem Zugang, AfA-Startdatum, Buchwert und AfA-Faelligkeit sichtbar plausibel sind.
...
| Labor-Akzeptanz bisher | Anlagenkarte/Setup-Basis und Zugang `G05001` sind als Labortrace verwertbar; Einkaufsrechnung mit `Art = Anlage`, AfA-Journalzeile, AfA-Preview und AfA-Buchung sind noch offen/blockiert. `fixedassets-296/297` erklaeren den Blocker: `OK` wurde zweimal kontrolliert probiert, aber keine Journalzeile sichtbar; der alte Datumsfehler ist eingegrenzt, die spaetere Ursache bleibt offen. |
| Evidence Pack | Ziel: Anlagenkarte, gebuchte Einkaufsrechnung, Anlagenposten, AfA-Journal, Sachposten, Anlagenspiegel, Negativtest. Labor bisher: `fixedassets-225`, `fixedassets-227`, `fixedassets-229`, `fixedassets-231`, `fixedassets-291`, `fixedassets-295`, `fixedassets-296`, `fixedassets-297`. |
...
>
> Dieses Kapitel ist eine UAT-Zielbibliothek, kein Beleg dafuer, dass alle UAT-Faelle bereits bestanden sind. Der aktuelle RM-DEMO-Laborstand ordnet vorhandene Evidence ein: O2C ist mit `S-ORD101068` -> `PS-INV103297` als CRONUS-USA-Laborfall belegt, P2P mit `106049` -> `108219`, Inventory mit `INV008-899959`. Payments ist mit `PAY011-PS103297` als erste UI-first Laborzahlung belegt: Rechnung `PS-INV103297` zeigt `Remaining Amount = 0,00`, detaillierte Debitorenposten zeigen `Payment Discount` und `Application`, Sachposten zeigen `15110`, `18200`, `40910`. Bank Account Ledger Entries sind seit `PAYMENTS-013` ueber Page `372` belegt; Bankabstimmung bleibt offen. Reporting ist nur teilweise/negativ belegt: `PRODUCTLINE=MACHINE` und `CHANNEL=B2B` sind am Artikelposten sichtbar, aber noch nicht als Financial-Reports-Summenwirkung. Deutsche `19 %` USt, deutscher Kontenplan, deutsche E-Rechnung, Security-Setup, Migration, Integrationen, Operations und produktive Architekturentscheidungen bleiben Final- oder Gate-Folgearbeit.
>
...
>
> Kapitel 33 ist eine Übungs- und Lösungsbibliothek, kein Nachweis, dass alle Übungen praktisch abgeschlossen wurden. Der aktuelle Laborstand liefert belastbare Muster für Verkauf/O2C (`PS-INV103297`), Einkauf/P2P (`108219`), Inventory (`INV008-899959`) und eine erste Debitorenzahlung (`PAY011-PS103297`). Payments zeigt jetzt Zahlung, OP-Ausgleich, Payment Discount, Application und Sachposten als CRONUS-USA-Labor; Bank Account Ledger Entries sind seit `PAYMENTS-013` ueber Page `372` belegt; Bankabstimmung bleibt offen. Reporting ist nur teilweise belegt: `PRODUCTLINE=MACHINE` und `CHANNEL=B2B` sind am Artikelposten sichtbar, aber nicht als Financial-Reports-Summenwirkung nachgewiesen.
>
...
Aktueller Evidence-Stand: O2C, P2P, Inventory und die erste Debitorenzahlung sind als CRONUS-USA-Laboranker belegt. Payments zeigt mit `PAY011-PS103297` Zahlung, OP-Ausgleich, Payment Discount, Application und Sachposten; Bank Account Ledger Entries sind seit `PAYMENTS-013` ueber Page `372` belegt; Bankabstimmung bleibt offen. Reporting zeigt `PRODUCTLINE=MACHINE` und `CHANNEL=B2B` am Artikelposten, aber noch keine belastbare Financial-Reports-Summenwirkung. Deutsche `19 %` USt, deutsche Finalscreenshots, deutscher Kontenplan, Fixed Assets, Warehouse-Aktivierung, Manufacturing, Service, Projects, Dropshipping/Sonderverkauf, Intercompany/Ausland, Security-Setup, Migration, Integrationen, Betrieb und echte Gesamt-UATs bleiben offen, readiness-belegt oder gatepflichtig.
...
Reporting-Grenze aus `REPORTING-015`/`REPORTING-016`: Die Dimension `PRODUCTLINE=MACHINE` ist im RM-DEMO-Labor nach der Buchung im Artikelposten- und Wertposten-Dimensionskontext sichtbar. Das ist ein guter Lernnachweis fuer die Postenspur, aber noch kein Reporting-Nachweis. Sachposten zeigen in diesem Lauf Konto `14140` und Betrag, aber keinen sichtbaren `PRODUCTLINE`-/`CHANNEL`-Nachweis; `Analysis Views` war read-only erreichbar, zeigte `PRODUCTLINE`/`CHANNEL` aber nicht als belegte Auswertungsachse. Fuer Anfaenger ist das wichtig: "Dimension am Posten sichtbar" und "Bericht wertet nach Dimension aus" sind zwei verschiedene Nachweise. Die deutsche Zielinstanz muss diese Kette spaeter neu belegen: Dimension vor Buchung, Dimensionssicht an Posten, Sachposten-/Dimensionsdetails und erst dann Analysis Views oder Financial Reports.
...
Aktueller Laborstand aus `PAYMENTS-001` bis `PAYMENTS-013`:
