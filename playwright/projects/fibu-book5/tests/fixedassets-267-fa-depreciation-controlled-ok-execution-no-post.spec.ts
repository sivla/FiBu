...
const TEST_ID = 'fixedassets-267';
const CASE_ID = 'FIXEDASSETS-267-FA-DEPRECIATION-CONTROLLED-OK-EXECUTION-NO-POST';
...
test('FIXEDASSETS-267 executes Calculate Depreciation OK once and searches journal without post', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-controlled-ok-execution-no-post',
    caseId: CASE_ID,
    source: 'playwright-controlled-ok-execution-no-post',
    resultStatus,
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 30.06.2026, FADEP-267-OK and FA-CNC-01 were visibly proven on the request page before OK.']
        : []),
      ...(okExecution.clicked ? ['OK was confirmed exactly once.'] : []),
      ...(journalLineFound ? ['FADEP-267-OK was found in Fixed Asset G/L Journals after OK.'] : ['FADEP-267-OK was not found in Fixed Asset G/L Journals after OK.']),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-267-fa-depreciation-controlled-ok-execution-no-post.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-267/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-267/FIXEDASSETS-267-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-267/FIXEDASSETS-267-CONTROLLED-OK-EXECUTION-NO-POST.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-267/010-ok-execution-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-267/020-journal-line-search.json',
    ],
    statePatch: {
      current: {
        activeCase: journalLineFound
          ? 'FIXEDASSETS-268-FA-DEPRECIATION-JOURNAL-LINE-REVIEW'
          : 'FIXEDASSETS-268-FA-DEPRECIATION-OK-RESULT-BLOCKER-REVIEW',
        active_case_file: journalLineFound
          ? '.agent/state/cases/fixedassets-268-fa-depreciation-journal-line-review.json'
          : '.agent/state/cases/fixedassets-268-fa-depreciation-ok-result-blocker-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-267-fa-depreciation-controlled-ok-execution-no-post.json',
        nextStep: journalLineFound
          ? 'FIXEDASSETS-268: locally review the FADEP-267-OK journal line and decide the next no-post evidence step.'
      ...
      coverage: {
        ...
        depreciationReadiness: journalLineFound
          ? 'FA-267 confirmed OK once and found FADEP-267-OK in Fixed Asset G/L Journals. Preview Posting and Post remain locked pending local review.'
          : 'FA-267 confirmed OK once but did not find FADEP-267-OK in Fixed Asset G/L Journals. Repeat OK remains locked pending local review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-267-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-267-CONTROLLED-OK-EXECUTION-NO-POST.md'),
    [
      ...
      `- FADEP-267-OK im Journal sichtbar: ${journalLineFound ? 'ja' : 'nein'}`,
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-267 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-267-result.json` | JSON-Ergebnis | OK-Ausfuehrung und Journal-Suchstatus | kein Preview/Post | labor |',
      '| `FIXEDASSETS-267-CONTROLLED-OK-EXECUTION-NO-POST.md` | Lernzusammenfassung | OK-Grenze, Ergebnis und naechster Gate-Schritt | keinen deutschen Finalnachweis | labor |',
      ...
      '| `020-journal-line-search.json` | JSON | Journal-Suchbefund fuer `FADEP-267-OK` | keine Buchung | compact |',
