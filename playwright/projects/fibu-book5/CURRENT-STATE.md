...
Als naechstes gezielt entweder den Payments-Lernblock nur nach Freigabe fortsetzen oder Reporting kontrolliert weiterklaeren. `PAYMENTS-010` und `PAYMENTS-EVIDENCE-PACK-SYNC.md` haben den Zahlungsweg bis zur letzten Sicherheitsgrenze didaktisch geschlossen; `POSTING-TRACE-001` hat die gebuchten O2C-/P2P-/Inventory-Postenspuren als Anfaenger-Lernatlas verbunden. `REPORTING-008` definiert den naechsten Reporting-Hebel: ein freigegebener Analysis-View-Fit fuer `PRODUCTLINE`/`CHANNEL`. Ohne Payment- oder Reporting-Setup-Freigabe keinen weiteren Post-Dialog bestaetigen und keine Analysis View anlegen/aendern; dann nur read-only oder Buch-/Evidence-Sync waehlen.
