...
test.skip(
  process.env.PWS_FF_005_LIVE_APPROVED !== '1' || process.env.PWS_FF_005_RUNNER_GUARD_CHECKED !== '1',
  'PWS-FF-005 must be run through the guarded runner with --live-approved.'
);

const CASE_ID = 'PWS-FF-005-DIMENSIONS-READFIRST';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'pws-ff-005-dimensions-readfirst';
...
const probes: Probe[] = [
  ...
  {
    id: 'general-ledger-dimension-context',
    pageId: 118,
    label: 'Finanzbuchhaltung Einrichtung / General Ledger Setup',
    searchTerm: 'Finanzbuchhaltung Einrichtung',
    searchResult: /^Finanzbuchhaltung Einrichtung$|^General Ledger Setup$/i,
    expectedSignals: [/Finanzbuchhaltung Einrichtung|General Ledger Setup|Sachbuchhaltung Einrichtung/i, /Dimension/i],
    include: [/Finanzbuchhaltung Einrichtung|General Ledger Setup|Sachbuchhaltung Einrichtung|Globaler Dimensionscode|Global Dimension Code|Shortcut Dimension|Dimension/i],
    beginnerLearning:
      'Die Finanzbuchhaltung-Einrichtung zeigt, wo Business Central globale und Shortcut-Dimensionen fuehrt. Aenderungen dort sind wirksam und brauchen einen eigenen Setup-Case.',
    importantUi: ['Seitentitel', 'globale Dimensionsfelder', 'Shortcut-Dimensionsfelder', 'keine bestaetigten Aenderungsdialoge'],
    doesNotProve: [
      'PRODUCTLINE ist nicht als globale Dimension 1 bewiesen.',
      'COSTCENTER ist nicht als globale Dimension 2 bewiesen.',
      'Keine Aenderung an globalen Dimensionen wurde ausgefuehrt.'
    ]
  }
];
...
function targetInstanceUrl() {
  const url = new URL(process.env.PWS_FF_005_BC_TARGET_URL || requireBcUrl('FIBU_BOOK5'));
  const segments = url.pathname.split('/').filter(Boolean);
  if (!segments.length) {
    throw new Error('Business Central URL must include a tenant/environment path before PWS-FF-005 can build a page URL.');
  }
...
async function openDimensionsList(page: Page) {
  ...
  await clickFirstVisible(page, /^Dimensionen$/i);
  await waitForBusinessCentralShell(page);
  await page.waitForLoadState('domcontentloaded').catch(() => undefined);
...
async function openDimensionValuesFromDimensionsList(page: Page, probe: Probe) {
  ...
  const groupClicked = await clickFirstVisible(page, /^Dimension$/i);
  if (!groupClicked) return false;
  const valuesClicked = await clickFirstVisible(page, /^Dimensionswerte$|^Dimension Values$/i);
  ...
  if (probe.pageId === 537 && probe.filter) {
    ...
    if (openedViaRelatedAction) {
      ...
      return {
        ...
        compact,
        route: {
          routeUsed: 'dimensions-list-related-action',
          routeNote: 'Opened Dimensions first, selected the dimension row, then used Dimension > Dimensionswerte read-only.'
        }
      };
    }
  }
...
async function probePage(page: Page, probe: Probe, index: number): Promise<ProbeResult> {
  ...
  const textFile = `pws-ff-005-${String(index).padStart(3, '0')}-${probe.id}.txt`;
  await writeTextEvidence(evidencePath(PROJECT, EVIDENCE_ID, textFile), text || 'No compact page text captured.');
  const shot = await screenshotWithMetadata(page, `pws-ff-005-${String(index).padStart(3, '0')}-${probe.id}.png`, {
    page: probe.label,
    pageId: probe.pageId,
    filter: probe.filter ?? '',
    step: 'Read-only Foundation dimension context proof',
    status,
    ...
    internallyProves:
      status === 'observed'
        ...
        : `The route did not produce accepted page/context signals for ${probe.label}.`,
    doesNotProve: [
      ...probe.doesNotProve,
    ...
    screenshotQaRule:
      'Accept only if page identity, company context and visible dimension/dimension-value/setup signals are clear. Reject Role Center, search overlay or generic navigation text as page proof.',
...
test('PWS-FF-005 captures Universaarl dimensions read-first', async ({ page }) => {
  await fs.mkdir(EVIDENCE_DIR, { recursive: true });
  ...
  const evidenceRefs = [
    `${EVIDENCE_DIR_REL}/PWS-FF-005-result.json`,
  ...
  const result = {
    ...
    caseId: CASE_ID,
    source: 'playwright-readonly-foundation-dimensions',
    resultStatus,
    startedAt,
    ...
    actionsTaken: [
      ...
      'Opened Dimension Values page 537 read-only with PRODUCTLINE, COSTCENTER and CHANNEL filters.',
      'Opened General Ledger Setup page 118 read-only for dimension setup context.',
      'Captured screenshot QA metadata and compact page text for every probe.'
    ],
    actionsNotTaken: [
    ...
    proved: [
      ...
      ...(observed.some((entry) => entry.id === 'general-ledger-dimension-context')
        ? ['General Ledger Setup dimension context is visible read-only.']
        : []),
    ...
    nextStepDecision: {
      ...
      lastEvidenceSummary:
        'PWS-FF-002C parked General Posting Setup as partial-not-posting-ready and selected Dimensions as the next independent no-write Foundation lane.',
      isPlannedNextCaseStillSensible: true,
      reason:
        'Dimensions support reporting, UAT and training and can be understood read-only without adding more setup risk.',
      lookaheadReviewed: [
        {
          caseId: 'FOUNDATION-READINESS-DECISION-REFRESH-AFTER-DIMENSIONS',
          status: blocked.length > 0 ? 'ready-after-current' : 'ready-next',
          reason: 'Foundation Readiness should consume the Dimensions read-first outcome before Master Data, but blocked Dimension Values need route recovery first.'
        },
        {
        ...
        {
          caseId: 'PWS-FF-005B-GLOBAL-DIMENSION-WRITE-GATE',
          status: 'needs-source-check-first',
          reason: 'Changing global dimensions is a setup write and is not allowed by this read-first case.'
        }
      ],
      queueChangesMade: [],
      selectedNextCase,
      whySelectedNextCaseIsBest:
        blocked.length > 0
          ? 'Dimensions list is visible, but Dimension Values and General Ledger Setup routes are not yet accepted; recover the related-action route before claiming Foundation readiness.'
  ...
  await writeJsonEvidence(evidencePath(PROJECT, EVIDENCE_ID, 'PWS-FF-005-result.json'), result);
  await writeTextEvidence(
    ...
    [
      ...
      '## Ergebnis',
      '',
      `${observed.length}/${results.length} Seitenkontexte wurden als read-only beobachtet.`,
      '',
      '## UI-Learning',
      '',
  ...
  expect(unsafe, 'PWS-FF-005 must not continue with wrong instance/company.').toEqual([]);
});
