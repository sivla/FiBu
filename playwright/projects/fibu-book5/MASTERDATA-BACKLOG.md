...
| Bankkonto | 20 | `BANK-RM-01` | `PAYMENTS-002` oeffnet Bank Accounts und zeigt vorhandene CRONUS-Bankkonten `CHECKING`/`SAVINGS`; Zielbankkonto `BANK-RM-01` ist nicht sichtbar | ja | ja, `payments-002` | `missing-setup` | P4 | `PAYMENTS-003`: `BANK-RM-01` idempotent anlegen/fitten oder ein vorhandenes CRONUS-Bankkonto bewusst als Laborersatz freigeben. |
...
| Journale | 19, 20, 21, 24 | Zahlungs-, Fibu-, Anlagen- und Abschlussjournale | Item Journal ist fuer Inventory belegt; `PAYMENTS-002` oeffnet Cash Receipt Journal und Payment Journal read-only und zeigt Felder/Post-Aktionen, erstellt aber keine Zahlungszeile | teilweise | ja, `inventory-005` bis `inventory-008`, `payments-002` | `partial` | P4 | Nach Bankkonto-Fit eine einzelne Zahlungsjournal-Readiness-Zeile kontrolliert vorbereiten; noch nicht buchen. |
...
| Payments/OP-Ausgleich Evidence Pack | 19, 20 | offene Debitoren-/Kreditorenposten, Zahlung, Ausgleich, Bank-/Journalwirkung | `PAYMENTS-001` belegt offene Posten; `PAYMENTS-002` und `PAYMENTS-READINESS.md` belegen Bank-Accounts-Einstieg, Cash Receipt Journal, Payment Journal und Apply Entries als Governance-Gate; `BANK-RM-01` fehlt; Zahlung, Ausgleich und Bankwirkung fehlen | ja als Prozessfall | ja, `payments-001`, `payments-002` | `partial` | P4 | `PAYMENTS-003`: Bankkonto-Fit/Entscheidung; erst danach ggf. eine einzelne Laborzahlung vorbereiten. |
...
2. P2P-Laborbuchung ist erledigt: `106049` -> `108219`. Nicht erneut buchen; `PAYMENTS-001` hat offene Posten als Startpunkt belegt und `PAYMENTS-002` hat Journal-/Apply-Readiness belegt. Naechster P2P-/O2C-naher Schritt ist Bankkonto-Fit/Entscheidung fuer `BANK-RM-01`.
...
Lies CURRENT-STATE.md, LAB-FIT-STATUS.md, BOOK-CLICK-GUIDE-COVERAGE.md, evidence/payments-001/README.md, evidence/payments-002/README.md und evidence/payments-002/PAYMENTS-002-result.json.
Starte `PAYMENTS-003` als idempotenten Bankkonto-Fit-/Entscheidungslauf: pruefe, ob `BANK-RM-01` angelegt werden soll oder ob `CHECKING`/`SAVINGS` bewusst als CRONUS-Laborbankkonto genutzt werden. Wenn ein Fit gesetzt wird, vorher/nachher dokumentieren. Keine Zahlung, kein Ausgleich, keine Bankabstimmung, keine neue O2C-/P2P-/Inventory-Buchung und kein deutscher Finalnachweis.
```
