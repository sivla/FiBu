...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort; `FRA-ZL` + `RESALE` wurde mit `Inventory Account = 14140` als CRONUS-Laborfit eingerichtet; `INVENTORY-001` fuehrt O2C und P2P ueber Item Ledger Entries, Value Entries, G/L Entries, Artikelkarten und Location read-only zusammen | `masterdata-004`, `masterdata-005`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001`, `p2p-001`, `evidence/inventory-001/INVENTORY-LAB-TRACE.md`, `playwright/projects/fibu-book5/img/inventory-001-*` | Inventory-Posting-Setup-Luecke ist im Labor geschlossen; O2C-Artikelposten `792` zeigt `RM-M100`, `FRA-ZL`, Menge `-1`; P2P-Artikelposten `793` zeigt `RAW-STEEL`, `FRA-ZL`, Menge `10`; P2P-Sachposten zeigen `22100`/`14140`; alle gefilterten Tabellenbilder nutzen die breite Layoutansicht; Inventory Valuation ist nur als Tell-Me-Einstieg belegt | Artikelposten, Wertposten, Sachposten und Lagerbewertung in deutscher Zielcompany mit deutschen Konten und Steuerlogik | Inventory Valuation/Lagerbewertung mit Datum, Artikel- und Lagerortfilter read-only pruefen |
...
2. Inventory Valuation/Lagerbewertung read-only oeffnen und mit Datum, Artikel `RM-M100`/`RAW-STEEL` und Lagerort `FRA-ZL` als Zahlenbericht pruefen; keine Buchung.
