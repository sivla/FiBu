...
function compactPageText(text: string) {
  const interesting =
    /P2P019|Purchase Journals|Batch Name|Posting Date|Document Type|Document No\.|Account Type|Account No\.|Vendor|Amount|Gen\. Bus\. Posting Group|Gen\. Prod\. Posting Group|DOMESTIC|RETAIL|Journal Check|Preview Posting|Post|K10000|Invoice|2\.500|2500|issue|must have a value/i;
  ...
  return {
    ...
    issueText:
      text
        .split('\n')
        .map((line) => line.trim())
        .find((line) => /must have a value|muss.*Wert enthalten|does not exist|is missing|ist nicht vorhanden|error|fehler/i.test(line)) ?? ''
  };
}
...
test('p2p-019 sets Purchase Journal posting groups and gates preview by Journal Check', async ({ page }) => {
  const documentNo = `P2P019-${Date.now().toString().slice(-6)}`;
  ...
  await screenshot(page, 'p2p-019-020-posting-group-gate.png', {
  ...
  const cleanJournalCheck = journalCheck.zeroIssuesTotalVisible || journalCheck.zeroLinesWithIssuesVisible || journalCheck.currentLineNoIssuesVisible || (!journalCheck.issueText && journalCheck.journalCheckVisible);
  const oldBlockerStillPresent = /Gen\\. Bus\\. Posting Group/i.test(journalCheck.issueText);
  const accountNoHeaderMisreadAvoided = !/Account No\\./i.test(journalCheck.issueText);
  const journalCheckErrorFixed = genBusVisible && !oldBlockerStillPresent && accountNoHeaderMisreadAvoided;
  const highImpactExecute = journalCheckErrorFixed || preview.opened || cleanJournalCheck;
  const result = {
    schemaVersion: 1,
    caseId: 'P2P-019-PURCHASE-JOURNAL-CLEAN-PREVIEW-GATE',
    ...
    signals: [
      ...
      ...(cleanJournalCheck ? ['journal-check-error-fixed'] : []),
    ...
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/p2p-019/010-initial-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-019/020-after-posting-group-gate-page-text.txt',
      'playwright/projects/fibu-book5/evidence/p2p-019/020-after-posting-group-gate-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-019/030-cleanup-page-text.txt'
    ],
    screenshots: ['playwright/projects/fibu-book5/img/p2p-019-020-posting-group-gate.png'],
    journalCheck,
    preview,
    proves: [
      'Purchase Journal values were entered with stricter Journal Check parsing that ignores column-header Account No. text.',
      journalCheckErrorFixed
        ? cleanJournalCheck ? 'Journal Check appears clean under strict issue parsing.' : 'The previous Gen. Bus. Posting Group blocker is no longer the first visible issue.'
    ...
    nextStep: journalCheckErrorFixed
      ? 'P2P-020: if Preview opened, decide controlled posting/trace route; if not, continue blocker diagnosis.'
      : 'P2P-020: continue blocker diagnosis before any posting route.'
  };
  await writeJsonEvidence(p2pEvidencePath('P2P-019-result.json'), result);
  await writeTextEvidence(
    ...
    [
      ...
      `- Journal Check sauber nach strenger Auswertung: ${cleanJournalCheck ? 'ja' : 'nein'}`,
