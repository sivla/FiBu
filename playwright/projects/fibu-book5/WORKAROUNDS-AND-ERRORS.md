...
| Loesung | Nicht buchen und nicht mit `OK` im normalen Buchungsdialog weitergehen. `MASTERDATA-008` zeigt jetzt die Zielzeile `FRA-ZL` + `RESALE` auf Page `5826`; das `Inventory Account` ist leer. Naechster Schritt ist eine fachliche Kontenentscheidung. Danach `Preview Posting` erneut ausfuehren. |
| Pruefung nach Korrektur | Nach gesetztem, begruendetem Bestandskonto `npm run fibu:uat:o2c` erneut ausfuehren. Erwartung: `Preview Posting` zeigt echte Postenvorschau oder den naechsten fachlichen Setup-Fehler. |
...
| Status | teilweise geloest: Ursache praktisch diagnostiziert, Kontenfix und erneuter Preview-Posting-Nachweis offen |
