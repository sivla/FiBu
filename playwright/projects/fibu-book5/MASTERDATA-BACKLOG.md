...
4. Inventory einfach vertiefen: `INVENTORY-002` ist als read-only Zahlenbericht erledigt, `INVENTORY-003` erklaert den negativen `RM-M100`-Wert, `INVENTORY-004` legt den Zielbestandsplan fest und `INVENTORY-005` bestaetigt `Item Journals` als Einstieg; naechster Schritt ist die kontrollierte positive `RM-M100`-Bestandsbewegung als eigener Lauf.
