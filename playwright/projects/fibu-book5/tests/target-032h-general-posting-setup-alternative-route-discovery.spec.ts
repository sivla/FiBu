...
const CASE_ID = 'TARGET-032H-GENERAL-POSTING-SETUP-ALTERNATIVE-ROUTE-DISCOVERY';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'target-032h-general-posting-setup-alternative-route-discovery';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const IMG_DIR = path.resolve('playwright/projects/fibu-book5/img');
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-032H-result.json');
...
async function openPage(page: Page, pageId: number, expected: RegExp) {
  await page.goto(buildTargetUrl(pageId).toString(), { waitUntil: 'domcontentloaded', timeout: 120_000 });
...
test('TARGET-032H General Posting Setup alternative route discovery', async ({ page }) => {
  await fs.mkdir(EVIDENCE_DIR, { recursive: true });
  await fs.mkdir(IMG_DIR, { recursive: true });
  ...
  const before = await capturePageState(page, 'target-032h-010-page314-start', 'Page 314 start before alternative route discovery.', {
  ...
  await capturePageState(page, 'target-032h-020-action-tooltip-qa', 'After action hover and tooltip probe.', {
    hoverResults
  });
  ...
  const moreOptionsState = await capturePageState(page, 'target-032h-030-more-options-state', 'After opening More options if available.', {
    moreOptionsClicked
  });
  ...
  const inspectionState = await capturePageState(page, 'target-032h-040-page-inspection-context', 'After Ctrl+Alt+F1 Page Inspection probe.', {
    inspectionOpened
  });
  ...
  await openPage(page, 314, /Buchungsmatrix|General Posting Setup/i);
  const finalPage314State = await capturePageState(
    page,
    'target-032h-050-page314-final-reopen-proof',
    'Final Page 314 reopen proof after tooltip, More-options and Page-Inspection probes.',
    { targetAlreadyVisible }
  );
  actionsTaken.push('Reopened Page 314 as a final read-only proof and did not repeat the rejected Page 315 route.');

  const routeSignals = {
    sourceMentionsOwnCard:
      'Microsoft Learn describes General Posting Setup as a setup matrix for business and product posting groups. This case only checks whether Page 314 exposes a safer UI route than inline matrix editing.',
    actionInventoryHasCardSignal: inventory.actions.some((action) => /Karte|Card|Details|Offnen|Open/i.test(labelOf(action))),
    ...
    finalReopenStayedOnPage314: new URL(page.url()).searchParams.get('page') === '314',
    finalReopenHasDangerousDialog: Boolean(finalPage314State.hasDangerousDialog),
    skippedRejectedPage315Probe: true
  };

  const discoveredRoute = routeSignals.actionInventoryHasCardSignal || routeSignals.moreOptionsHasCardSignal;
  if (!discoveredRoute) {
    blockedBy.push('No materially different Page 314 route candidate was visible in action inventory, tooltip or More-options evidence.');
  }
  warnings.push('Page 315 was intentionally not repeated because prior evidence already rejected it as a VAT Entries route, not a General Posting Setup card route.');
  ...
  const resultStatus = discoveredRoute ? 'observed-weak-route-signal-no-write' : 'blocked';
  const nextCase = discoveredRoute
    ? 'TARGET-032I-GENERAL-POSTING-SETUP-ROUTE-CANDIDATE-DECISION'
    : 'TARGET-032I-GENERAL-POSTING-SETUP-SOURCE-OR-CONFIGURATION-ROUTE-DECISION';
  ...
  const result = {
    ...
    hoverResults,
    screenshots: [
      'playwright/projects/fibu-book5/img/target-032h-010-page314-start.png',
      'playwright/projects/fibu-book5/img/target-032h-020-action-tooltip-qa.png',
      'playwright/projects/fibu-book5/img/target-032h-030-more-options-state.png',
      'playwright/projects/fibu-book5/img/target-032h-040-page-inspection-context.png',
      'playwright/projects/fibu-book5/img/target-032h-050-page314-final-reopen-proof.png'
    ],
    proved: [
      ...
      'Page 314 was reopened after the probes without a dangerous dialog.',
      'No master data, document draft, Preview Posting, Posting, Payment or API shortcut occurred.',
      ...(discoveredRoute ? ['A weak route signal exists and needs classification before any write.'] : [])
    ],
    notProved: [
    ...
    nextStepDecision: {
      ...
      lastEvidenceSummary:
        'TARGET-032D and follow-ups showed that Page 314 inline/list editing is fragile; TARGET-032E rejected direct Page 315 as a VAT Entries route.',
      ...
      reason: discoveredRoute ? 'A weak route signal was captured without writing values.' : 'No route candidate was good enough.'
        },
        {
          caseId: nextCase,
          status: 'ready-next',
          reason: discoveredRoute
            ...
            : 'A source/configuration route decision is needed to avoid repeating weak UI routes.'
        },
        {
      ...
      whySelectedNextCaseIsBest: discoveredRoute
        ...
        : 'It prevents another blind UI retry and forces a source/configuration route decision.',
      risksBeforeNextCase: [
      ...
      requiredPreparation: [
        'Review 032H action inventory, tooltip screenshots, More-options evidence and Page Inspection context.'
      ]
    },
    nextCase,
    changedFiles: [
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-032H-result.json`,
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/*.txt`,
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/*.json`,
      'playwright/projects/fibu-book5/img/target-032h-*.png'
    ],
    evidenceRefs: [
      `playwright/projects/fibu-book5/evidence/${EVIDENCE_ID}/TARGET-032H-result.json`,
      'playwright/projects/fibu-book5/img/target-032h-010-page314-start.png',
    ...
    statePatch: {},
    reason: discoveredRoute
        ? 'A weak route signal was captured without writing setup values.'
  ...
  await fs.writeFile(
    ...
    [
      '# TARGET-032H General Posting Setup Alternative Route Discovery',
      '',
      `Status: ${resultStatus}`,
      ...
      `- Page 315 repeated: ${routeSignals.skippedRejectedPage315Probe ? 'no, prior rejected route was not repeated' : 'yes'}`,
      '',
      '## Grenzen',
      '',
