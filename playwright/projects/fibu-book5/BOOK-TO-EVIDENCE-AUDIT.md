...
| Financial Reports Seite | 10, 25 | read-only geoeffnet, Liste sichtbar; weitere Reportingpfade `REPORTING-002` bis `REPORTING-011` liefern keine belastbare Summenwirkung nach `PRODUCTLINE`/`CHANNEL`; `REPORTING-011` zeigt `Analysis Views`, legt `RM-PLCH` aber nicht an; `GOVERNANCE-007` gibt `REPORTING-013` genau einmal frei | ja, `reporting-001` bis `reporting-012`, `governance-007` | ja | ja | partial-negativ / gate-rejected / next-gate-approved | `REPORTING-013`: Feldmapping zuerst, Setup nur bei sicherer UI-Zuordnung |
...
| `FiBu_Buch_BC_Standardprozesse_DE_Master_Blueprint.md` | Kapitel 10/25 Reporting | Financial Reports nach `PRODUCTLINE`, `CHANNEL`, `DEPARTMENT` filtern | Buchziel ist noch nicht durch Evidence belegt; `REPORTING-001` bis `REPORTING-012` zeigen Einstieg, Artikelposten-Dimension, mehrere Negativpfade und den abgelehnten Analysis-View-Fit; `GOVERNANCE-007` gibt den naechsten Feldmapping-Lauf frei | Abschnitt als Ziel-/Laborgrenze markieren; naechster Nachweis ist `REPORTING-013`, Setup nur bei sicherer UI-Feldzuordnung | `reporting-001` bis `reporting-012`, `governance-007` |
...
| Reporting | Financial Reports Seite offen; `REPORTING-002` bis `REPORTING-011` zeigen Artikelposten-Dimension, vorhandene Analysis-View-Grenze, negative/teilweise Sachposten-/Analysepfade und den abgelehnten Fit `RM-PLCH`; `GOVERNANCE-007` gibt `REPORTING-013` frei; Filter/Summenwirkung bleibt offen | `reporting-001` bis `reporting-012`, `governance-007` | `REPORTING-013` als genau ein Feldmapping-/Setup-Lauf; bei unsicherer Zuordnung ohne Setup abbrechen |
...
| Financial Reports | Seite offen; mehrere read-only Pfade und `REPORTING-011` ohne Ziel-Summenwirkung; `GOVERNANCE-007` gibt den naechsten Feldmapping-Versuch frei | `reporting-001` bis `reporting-012`, `governance-007` | Reportingfilter/Summenwirkung offen; `RM-PLCH` nicht angelegt | `REPORTING-013` ausfuehren und Gate danach schliessen |
...
`GOVERNANCE-005`, `PAYMENTS-011`, `PAYMENTS-013`, `PAYMENTS-014`, `GOVERNANCE-006`, `REPORTING-011`, `REPORTING-012` und `GOVERNANCE-007` sind erledigt. Starte als naechsten Lauf `REPORTING-013-ANALYSIS-VIEW-FIELDMAPPING-SETUP`: Feldmapping fuer Analysis Views zuerst dokumentieren, nur bei sicherer UI-Zuordnung `RM-PLCH` idempotent anlegen/aendern, danach `Analysis by Dimensions` pruefen; keine Zahlung, keine Bankabstimmung und keine Buchung.
```
