...
| Aktiver Case | `FOUNDATION-SETUP-PACKAGE-ROUTE-RECOVERY-READFIRST` |
...
| Foundation setup route decisions | active-work: Feldkarte erstellt; Paket-/Tabellen-Discovery lief read-first, blieb aber im Rollencenter. Metadaten-Write ist abgelehnt, bis eine echte read-only Seitenroute bewiesen ist. |
...
7. Naechster Schritt ist kein O2C/P2P-Beleg, kein Repeat von `VAT-POSTING-SETUP-READFIRST` und kein Setup-Write. `FOUNDATION-SETUP-PACKAGE-METADATA-WRITE-GATE-DECISION` ist abgeschlossen: kein Paket anlegen, bevor die Seite `Konfigurationspakete` read-only belastbar sichtbar ist. Jetzt `FOUNDATION-SETUP-PACKAGE-ROUTE-RECOVERY-READFIRST` ausfuehren: materiell andere Route, Bildkette, Stop vor `Neu`, `Tabellen abrufen`, Import, Export, Validate, Apply und Edit in Excel.
