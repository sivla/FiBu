...
`MANUFACTURING-001` ist als read-only Readiness erledigt: Manufacturing-Einstiege sind sichtbar, `Assembly Orders` nicht belastbar; `RM-M100`/`RAW-STEEL` sind vorhanden, `COMP-CTRL`/`KIT-MAINT` fehlen. Ohne Gate-Freigabe keine Production BOMs/Routings/Assembly-Strukturen anlegen, keine Fertigungs-/Montageauftraege erzeugen und keinen Verbrauch/Output buchen. Naechster sicherer Schritt ohne Gate ist `MANUFACTURING-002` als Kapitel-14-Buch-/Evidence-Sync.
```
