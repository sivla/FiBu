...
Als naechstes gezielt entweder den Payments-Lernblock fortsetzen oder Reporting read-only weiterklaeren. `PAYMENTS-010` hat den Buchungsdialog-/Freigabecheck vor einer moeglichen Laborzahlung erledigt: `Post` fuehrt in einen `Ja`/`Nein`-Dialog; `Nein` bricht ab; keine Zahlung wurde gebucht. Sinnvollster Payments-Lauf ist `PAYMENTS-011` nur nach ausdruecklicher Freigabe als kontrollierte Laborzahlung mit Postenspur. Ohne diese Freigabe ist Reporting aktuell der risikoarme Nutzenblock: Nach `REPORTING-006` ist der G/L-Entries-Data-Analysis-Versuch ebenfalls negativ; naechster Reporting-Hebel ist ein gezielter UI-Einstieg ueber `Analysis by Dimensions` oder ein freigegebener Analysis-View-Fit fuer `PRODUCTLINE`/`CHANNEL`.
