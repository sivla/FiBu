...
4. Inventory einfach vertiefen: `INVENTORY-002` ist als read-only Zahlenbericht erledigt, `INVENTORY-003` erklaert den negativen `RM-M100`-Wert, `INVENTORY-004` legt den Zielbestandsplan fest, `INVENTORY-005` bestaetigt `Item Journals` als Einstieg und `INVENTORY-006` bestaetigt den Ziel-Draft mit Dimension; naechster Schritt ist Journal Check/Preview-Alternative vor einer kontrollierten positiven `RM-M100`-Bestandsbewegung.
...
Fuehre `INVENTORY-006` als kontrollierten Journal-Lauf aus: Page `40`/`Item Journals` ist als Einstieg belegt. Bereite genau eine Journalzeile fuer `RM-M100 +2` in `FRA-ZL` vor, klaere Dimension `PRODUCTLINE=MACHINE` und Preview-/Kontrollmoeglichkeit. Nur wenn Zielwerte und Kontrolle passen, genau einmal buchen und danach Artikelposten, Wertposten, Sachposten sowie `Inventory Valuation` dokumentieren. Keine neue O2C-/P2P-Buchung, keine Warehouse-Aktivierung.
```
