...
The active auth refresh gate targets `playthru / UNIVERSAARL-DE`, but the Playwright auth profile is still not logged in to a Business Central shell. The latest bounded verification handoff ran `npm run auth:bc:open-login` with `BC_AUTH_OPEN_LOGIN_TIMEOUT_MS=30000`. The browser stayed on Microsoft sign-in, no Business Central shell signal was detected and no storage state was saved. The handoff command is intentionally bounded by default so failed attended login attempts finish with a result instead of leaving a long-running Playwright process behind.
