...
Ohne ausdrueckliche Freigabe ist der naechste sinnvolle Schritt `FIXEDASSETS-008` als Buch-/Checklisten-Sync: Kapitel 21 mit der Readiness-Kette `FIXEDASSETS-005` bis `FIXEDASSETS-007` abgleichen. `FIXEDASSETS-006` hat vorhandene CRONUS-Konten in `FA Posting Groups` gelesen; `FIXEDASSETS-007` hat `Depreciation Books` und `FA Classes` gelesen. Beide Schritte sollen nicht wiederholt werden, solange kein neuer Hebel vorliegt.
