...
function compactPageText(text: string) {
  const interesting =
    /P2P030|EXT-P2P030|Purchase Journals|Posting Preview|Preview Posting|G\/L Entries|VAT Entries|Journal Check|Issues Total|No issues found|Post|Document No\.|External Document No\.|Account Type|Account No\.|Vendor|Amount|Bal\. Account|K10000|82000|-2\.500|Invoice|error|fehler/i;
...
test('p2p-030 adds External Document No and retries scoped Purchase Journal Preview Posting', async ({ page }) => {
  const suffix = Date.now().toString().slice(-6);
  const documentNo = `P2P030-${suffix}`;
  const externalDocumentNo = `EXT-P2P030-${suffix}`;
  ...
  const result = {
    schemaVersion: 1,
    caseId: 'P2P-030-PURCHASE-JOURNAL-EXTERNAL-DOCUMENT-PREVIEW-GATE',
    parentCaseId: 'P2P-029-PURCHASE-JOURNAL-PREVIEW-ACTION-ROUTE',
    source: 'playwright-ui-labor-purchase-journal-external-document-preview-gate',
    ...
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/p2p-030/010-after-clean-journal-check-controls.json',
      'playwright/projects/fibu-book5/evidence/p2p-030/020-before-post-menu-actions.json',
      'playwright/projects/fibu-book5/evidence/p2p-030/030-related-post-split-button-candidates.json',
      'playwright/projects/fibu-book5/evidence/p2p-030/040-after-post-menu-actions.json',
      'playwright/projects/fibu-book5/evidence/p2p-030/050-preview-menuitem-candidates.json',
      'playwright/projects/fibu-book5/evidence/p2p-030/060-after-preview-click-page-text.txt',
      'playwright/projects/fibu-book5/evidence/p2p-030/070-cleanup-page-text.txt'
    ],
    screenshots: [],
    proves: [
      cleanJournalCheck ? 'P2P-028 values plus External Document No. produced a clean Purchase Journal Check.' : 'Clean Journal Check was not reached in this run.',
    ...
    nextStep: previewOpened
      ? 'P2P-031: review Preview Posting entries and decide whether a separate no-post or posting trace case is justified.'
      : 'P2P-031: classify remaining Purchase Journal Preview blocker after External Document No.; do not post.'
  };

  await writeJsonEvidence(p2pEvidencePath('P2P-030-result.json'), result);
  await writeTextEvidence(
    p2pEvidencePath('README.md'),
    [
      '# P2P-030 Purchase Journal External Document No Preview Gate',
      '',
