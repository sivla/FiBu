...
| Anlagen/Projekte/Service/Manufacturing | 14-16, 21 | Fixed Assets/Warehouse/Manufacturing/Service sind als Readiness bzw. Buch-Sync teilweise belegt; Fixed Assets hat `HGB` und `MACHINES` als Labor-Setup-Prerequisites sichtbar; Projects ist mit `PROJECTS-001` als Readiness gestartet und mit `PROJECTS-002` im Buch synchronisiert | teilweise | teilweise | teilweise | partial-readiness / fixedassets-hgb-machines-labor | Kapitel 17 als Dropshipping/Sonderverkauf ohne Shopify-Connector-Scope read-only; Setup/Buchung nur mit Gate |
...
`FIXEDASSETS-016-MACHINES-FA-POSTING-GROUP-FIT` ist erledigt: `MACHINES` ist in `RM-DEMO` sichtbar und das Nachherbild zeigt `12210`/`82000`. Fuehre als naechsten No-Approval-Schritt `FIXEDASSETS-017-FA-CNC-01-SETUP-READINESS` aus: nur entscheiden und vorbereiten, welche naechste Anlagen-Schicht UI-first tragfaehig ist. Keine Einkaufsrechnung, kein Zugang, keine AfA, keine Buchung und keine deutsche Finalbehauptung.
```
