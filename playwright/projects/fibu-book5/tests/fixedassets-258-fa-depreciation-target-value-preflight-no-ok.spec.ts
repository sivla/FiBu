...
const TEST_ID = 'fixedassets-258';
const CASE_ID = 'FIXEDASSETS-258-FA-DEPRECIATION-TARGET-VALUE-PREFLIGHT-NO-OK';
...
async function visibleDanger(page: Page) {
  const text = clean(await pageText(page));
  const dangerousConfirm = /\b(Post|Post and Print|Preview Posting|Ship|Invoice|Ship and Invoice|Delete|Yes|Ja)\b/i.test(text);
  return {
    dangerousConfirm,
    okVisible: /\bOK\b/.test(text),
...
test('FIXEDASSETS-258 fills Calculate Depreciation target values without OK', async ({ page }) => {
  ...
  if (frame && blocker.length === 0) {
    for (const key of ['depreciationBook', 'postingDate', 'documentNo', 'fixedAssetNo'] as FieldKey[]) {
      const dangerBefore = await visibleDanger(page);
      if (dangerBefore.dangerousConfirm) {
        blocker.push(`dangerous-confirm-visible-before-${key}`);
        break;
      }
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-target-value-preflight-no-ok',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 30.06.2026, FADEP-258-NO-OK and FA-CNC-01 were visibly proven on the request page without OK.']
        : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-258-fa-depreciation-target-value-preflight-no-ok.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-258/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-258/FIXEDASSETS-258-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-258/FIXEDASSETS-258-TARGET-VALUE-PREFLIGHT-NO-OK.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-258/010-target-value-preflight.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-258/010-target-value-preflight-text.txt',
    ],
    statePatch: {
      current: {
        activeCase: allValuesProved
          ? 'FIXEDASSETS-259-FA-DEPRECIATION-OK-EXECUTION-GATE-REVIEW'
          : 'FIXEDASSETS-259-FA-DEPRECIATION-TARGET-VALUE-PREFLIGHT-BLOCKER-REVIEW',
        active_case_file: allValuesProved
          ? '.agent/state/cases/fixedassets-259-fa-depreciation-ok-execution-gate-review.json'
          : '.agent/state/cases/fixedassets-259-fa-depreciation-target-value-preflight-blocker-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-258-fa-depreciation-target-value-preflight-no-ok.json',
        nextStep: allValuesProved
          ? 'FIXEDASSETS-259: locally review whether a controlled OK execution is justified; do not execute OK yet.'
      ...
      coverage: {
        ...
        depreciationReadiness: allValuesProved
          ? 'FA-258 proved HGB, 30.06.2026, FADEP-258-NO-OK and FA-CNC-01 on the Calculate Depreciation request page without OK. OK remains locked pending local review.'
          : 'FA-258 could not prove all target request-page values without OK. OK remains locked pending blocker review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
    ...
    reason: allValuesProved
      ? 'Target request-page values were proven without OK. Local review required before any execution.'
      : 'At least one target request-page value was not proven. OK remains locked.',
    observed: {
      startedAt,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-258-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-258-TARGET-VALUE-PREFLIGHT-NO-OK.md'),
    [
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-258 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-258-result.json` | JSON-Ergebnis | Zielwert-Preflight ohne OK | keine AfA-Ausfuehrung | labor |',
      '| `FIXEDASSETS-258-TARGET-VALUE-PREFLIGHT-NO-OK.md` | Lernzusammenfassung | Zielwerte, Grenzen und naechsten Gate-Schritt | keine Journalzeile | no-ok |',
