...
const CASE_ID = 'TARGET-026J-SKR04-ACCOUNT-CLEANUP-AND-SETUP-GATE';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'target-026j-skr04-account-cleanup-and-setup-gate';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-026J-result.json');
...
const account = {
  ...
  purpose: 'SKR04-Bankkonto fuer spaetere Bank-/Zahlungsstrecken'
};
...
async function correctIncomeBalanceOnAccountCard(page: Page, row: Locator) {
  ...
  const { frame, bodyText } = await findBcFrame(page, /G\/L Account|Sachkonto|Bank Saarland|GuV\/Bilanz|Income\/Balance|Bilanz|GuV/i);
  steps.push({ step: 'card-open-attempt', bodyText: bodyText.slice(0, 1000), url: sanitizeEvidenceUrl(page.url()) });
  if (!/Bank Saarland|1800/i.test(bodyText)) {
  ...
  const text = await safeText(page);
  return /1800[\s\S]{0,500}(Bilanz|Balance Sheet)/i.test(text)
...
async function setIncomeBalanceViaNameFieldTab(page: Page, frame: Frame) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const text = await safeText(page);
    if (/1800[\s\S]{0,1500}(Bilanz|Balance Sheet)/i.test(text)) {
...
async function captureState(page: Page, filePrefix: string, step: string, extra: Record<string, unknown> = {}) {
  const compact = await compactPageText(page, {
    include: [
      /Chart of Accounts|Kontenplan|G\/L Account|Sachkonto|No\.|Nr\.|Name|Balance Sheet|Bilanz|Income Statement|GuV|1200|1800|Bank Saarland|Neu|New|Liste bearbeiten|Edit List/i
    ],
    maxLines: 160,
    maxLineLength: 220
  });
  ...
  const snapshot = {
    ...
    visible: {
      ...
      accountName: /1800[\s\S]{0,500}Bank Saarland|Bank Saarland[\s\S]{0,500}1800/i.test(text),
      balanceSheet: /1800[\s\S]{0,500}(Bilanz|Balance Sheet)/i.test(text),
      incomeStatement: /1800[\s\S]{0,500}(GuV|Income Statement)/i.test(text),
...
async function ensureAccount(page: Page): Promise<{ status: StepStatus; reason: string; steps: Array<Record<string, unknown>> }> {
  ...
  if (literalPattern(account.no).test(afterText) && literalPattern(account.name).test(afterText) && /1800[\s\S]{0,500}Bilanz/i.test(afterText)) {
...
test('TARGET-026J classifies 1200 and creates or confirms 1800 SKR04 bank account', async ({ page }) => {
  await page.setViewportSize({ width: 2200, height: 1300 });
  await fs.mkdir(EVIDENCE_DIR, { recursive: true });

  await openChartOfAccounts(page);
  const before = await captureState(page, 'target-026j-010-before-chart-of-accounts', 'Before controlled setup attempt.', {
    targetAccount: account
  });
  const beforeText = await safeText(page);
  const account1200Classification = {
    no: '1200',
    observedAs: /1200[\s\S]{0,500}Bank Saarland/i.test(beforeText) ? '1200 Bank Saarland' : 'not-visible-in-before-snapshot',
    status: /1200[\s\S]{0,500}Bank Saarland/i.test(beforeText) ? 'wrong-bank-path-classified' : 'not-reclassified',
    decision:
      '1200 is not used as bank in this run. It remains blocked for bank/payment/VAT/posting until a dedicated safe rename or receivables reclassification case exists.'
  };

  const accountResult = await ensureAccount(page);
  const afterAttempt = await captureState(page, 'target-026j-020-after-account-attempt', 'After controlled account setup attempt.', {
    targetAccount: account,
    accountResult
  });
  await openChartOfAccounts(page);
  const afterReopen = await captureState(page, 'target-026j-030-after-reopen-proof', 'After reopening Chart of Accounts for persistence proof.', {
    targetAccount: account,
    accountResult
  });
  ...
  const nextStepDecisionCard = {
    ...
    lookaheadReviewed: [
      {
        caseId: 'TARGET-026K-SKR04-REMAINING-STARTER-ACCOUNTS-FIT',
        status: accountVisible ? 'ready-next' : 'needs-ui-discovery-first',
        reason: accountVisible
          ? 'The 1800 bank-account route has reopen proof and can be reused carefully for remaining mapped starter accounts.'
    ...
    selectedNextCase: accountVisible
      ? 'TARGET-026K-SKR04-REMAINING-STARTER-ACCOUNTS-FIT'
      : 'TARGET-026K-SKR04-ACCOUNT-ROUTE-RECOVERY',
    whySelectedNextCaseIsBest: accountVisible
      ? 'The fastest useful follow-up is to complete the remaining mapped SKR04 starter accounts without touching VAT or posting groups yet.'
  ...
  const result = {
    ...
    changedFiles: [
      'playwright/projects/fibu-book5/evidence/target-026j-skr04-account-cleanup-and-setup-gate/'
    ],
    evidenceRefs: [
      'target-026j-010-before-chart-of-accounts.png',
      'target-026j-020-after-account-attempt.png',
      'target-026j-030-after-reopen-proof.png',
      'TARGET-026J-result.json'
    ],
    warnings: [],
    blockedBy,
    ...
    statePatch:
      resultStatus === 'observed'
        ? {
            current: {
              activeCase: 'TARGET-026K-SKR04-REMAINING-STARTER-ACCOUNTS-FIT',
              nextCase: 'TARGET-026K-SKR04-REMAINING-STARTER-ACCOUNTS-FIT',
    ...
    afterReopen,
    accountResult,
    account1200Classification,
    nextStepDecisionCard,
    reason:
      resultStatus === 'observed'
        ? '1800 Bank Saarland is visible after controlled setup and reopen proof; 1200 is classified as wrong bank path and remains blocked for bank usage.'
  ...
  await writeText(
    ...
    [
      ...
      `- 1200: ${account1200Classification.status} - ${account1200Classification.decision}`,
      '',
      '## Grenzen',
      '',
      '- Kein vollstaendiger Kontenplan.',
      '- Screenshot-QA hat geprueft, dass das Bankkonto nicht als GuV-Konto stehen bleiben darf.',
