...
| Bank-/Journal-Readiness | `PAYMENTS-002` prueft Bankkonten, Cash Receipt Journal, Payment Journal und Apply-Entries-Pfade kontrolliert ohne Buchung. `PAYMENTS-003` legt `BANK-RM-01` als Laborbankkonto an und zeigt es in Bank Accounts; dieser API-Fit ist Laborhistorie, keine Buch-Klickanleitung. `PAYMENTS-004` bestaetigt live `BANK-RM-01` und zeigt im Cash Receipt Journal Pflichtfelder und Journal Check. `PAYMENTS-005` bereitet eine Zahlungsjournal-Entwurfszeile vollstaendig ueber die UI vor und loescht sie wieder. Der Entwurf zeigt Debitor, Betrag, Gegenkonto und Rechnungsbezug, aber Journal Check meldet noch `1 Issue` zur `Amount`-Validierung. Keine Zahlung, kein Ausgleich, keine Bankabstimmung |
...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. Sinnvollster naechster Lauf ist `PAYMENTS-006` als UI-only Korrekturlauf fuer den Cash-Receipt-Journal-Draft: denselben Entwurf ueber die UI vorbereiten, aber die `Amount`-Validierung so klaeren, dass Journal Check `0 Issues` oder eine fachlich erklaerte Restgrenze zeigt. Weiterhin keine Zahlung buchen, keinen Ausgleich setzen und keine Bankabstimmung starten. Begruendung: `PAYMENTS-005` hat den UI-Klickpfad bewiesen, aber noch keinen zahlungsreifen Entwurf.
...
8. `PAYMENTS-001` beweist offene Debitoren-/Kreditorenposten als Startpunkt fuer OP-Ausgleich. `PAYMENTS-002` beweist Bankkonten-, Cash-Receipt-Journal-, Payment-Journal- und Apply-Entries-Readiness. `PAYMENTS-003` beweist `BANK-RM-01` als Laborbankkonto, aber nicht als Buch-Klickpfad. `PAYMENTS-004` beweist Cash Receipt Journal Pflichtfelder, Gegenkonto-/Ausgleichshinweise und Journal Check read-only. `PAYMENTS-005` beweist die UI-Draft-Anlage und UI-Cleanup, aber Journal Check meldet noch ein Amount-Issue. Offen bleiben zahlungsreifer Entwurf, Ausgleich, Bank Account Posting Group/Sachkonto-Fit, Zahlung und Bankabstimmung.
