...
6. Manufacturing/Assembly ist als Readiness plus Kapitel-Sync erledigt; ohne Gate danach Service-Readiness, dann Projects-Readiness.
...
`FIXEDASSETS-008` ist als Buch-/Checklisten-Sync erledigt: Kapitel 21 und die Anlagen-Setup-Reihenfolge wurden mit `FIXEDASSETS-005` bis `FIXEDASSETS-007` abgeglichen. Ohne Gate-Freigabe keine Anlagenbuchungsgruppe anlegen oder aendern, kein AfA-Buch anlegen oder aendern, keine Anlagenkarte, keine Einkaufsrechnung, keine Anlagenbuchung.
`MANUFACTURING-001` und `MANUFACTURING-002` sind erledigt: Manufacturing-Einstiege sind sichtbar, `Assembly Orders` nicht belastbar; `RM-M100`/`RAW-STEEL` sind vorhanden, `COMP-CTRL`/`KIT-MAINT` fehlen; Kapitel 14 ist mit dieser Readiness synchronisiert. Ohne Gate-Freigabe keine Production BOMs/Routings/Assembly-Strukturen anlegen, keine Fertigungs-/Montageauftraege erzeugen und keinen Verbrauch/Output buchen. Naechster sicherer Schritt ohne Gate ist `SERVICE-001-READINESS` als read-only Start fuer Kapitel 15.
```
