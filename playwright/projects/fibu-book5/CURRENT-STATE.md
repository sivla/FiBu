...
Als naechstes gezielt den Payments-Lernblock fortsetzen, ohne erneut O2C/P2P/Inventory zu buchen. Sinnvollster naechster Lauf ist `PAYMENTS-008` als UI-only Amount-/Journal-Check-Diagnose nach geloestem Bankkonto-Fit: denselben Cash-Receipt-Draft fuer `D10000`/`PS-INV103297` mit `BANK-RM-01` erneut vorbereiten, Amount-Feld und Amount-LCY-Anzeige sauber unterscheiden, Journal Check lesen und wieder bereinigen. Keine Zahlung buchen, keinen Ausgleich setzen und keine Bankabstimmung starten. Begruendung: `PAYMENTS-007` hat `BANK-RM-01` auf `CHECKING` gefittet und den alten Bank-Posting-Group-Fehler entfernt, aber der Amount-Preflight ist wieder der aktuelle Blocker.
