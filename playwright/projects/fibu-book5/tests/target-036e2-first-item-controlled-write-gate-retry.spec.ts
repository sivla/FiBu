...
const CASE_ID = 'TARGET-036E2-FIRST-ITEM-CONTROLLED-WRITE-GATE-RETRY';
const EXPECTED_INSTANCE = 'playthru';
const TARGET_COMPANY = 'UNIVERSAARL-DE';
const PROJECT = 'fibu-book5';
const EVIDENCE_ID = 'target-036e2-first-item-controlled-write-gate-retry';
const EVIDENCE_DIR = path.resolve('playwright/projects/fibu-book5/evidence', EVIDENCE_ID);
const IMG_DIR = path.resolve('playwright/projects/fibu-book5/img');
const RESULT_PATH = path.join(EVIDENCE_DIR, 'TARGET-036E2-result.json');
...
async function fillLabeledBox(page: Page, frame: Frame, names: RegExp, value: string) {
  for (const role of ['textbox', 'combobox'] as const) {
    const box = await firstVisible(frame.getByRole(role, { name: names }), 600);
    if (box && (await box.isEditable({ timeout: 500 }).catch(() => false))) {
...
async function fillItemFields(page: Page, frame: Frame, steps: Step[]) {
  const { inspected } = await editableInputs(frame);
  ...
  const changedNo = await fillLabeledBox(page, frame, /^Nr\.?$|^No\.?$/i, targetItem.itemNo);
  if (changedNo) fieldsChanged.push(`No=${targetItem.itemNo}`);

  const changedDescription = await fillLabeledBox(page, frame, /^Beschreibung$|^Description$/i, targetItem.description);
  if (changedDescription) fieldsChanged.push(`Description=${targetItem.description}`);

  const changedType = await fillLabeledBox(page, frame, /^Art$|^Type$/i, targetItem.type);
  if (changedType) fieldsChanged.push(`Type=${targetItem.type}`);

  const changedBaseUnit = await fillLabeledBox(
    page,
    frame,
    /^(Basiseinheit|Base Unit of Measure|Base Unit|Einheitencode|Unit of Measure Code)$/i,
    'STK'
  );
  if (changedBaseUnit) fieldsChanged.push('Base Unit=STK');

  return {
    changed: changedNo && changedDescription && changedBaseUnit,
    changedNo,
    changedDescription,
    changedType,
    changedBaseUnit,
    ...
    fieldsNotTouched,
    reason:
      changedNo && changedDescription && changedBaseUnit
        ? 'Filled labelled No., Description and Base Unit fields; Type was changed only if a trusted labelled field was editable.'
        : 'No trusted labelled field route for all required item fields was visible.'
  };
}
...
async function inspectUnits(page: Page) {
  ...
  const text = await safeText(page);
  await captureState(page, 'target-036e2-005-units-before-item-write', 'Units of Measure before first item write retry.', {
  ...
  return {
    ...
    availableCandidates,
    rawSignals: clean(text).split(/\n| {2,}/).filter((line) => /Code|Beschreibung|STK|Unit|Einheit/i.test(line)).slice(0, 30)
  };
}
...
test('TARGET-036E2 creates or blocks first Universaarl item with Base Unit STK', async ({ page }) => {
  ...
  const steps: Step[] = [
    {
      step: 'smart-decision',
      whyNow:
        'TARGET-041 proved STK visible on Units of Measure after reopen. TARGET-036E2 retries the first item with only No., Description, Type and Base Unit STK unlocked.',
      evidenceBasis: [
        'TARGET-036E: first item was blocked because no Base Unit was proven.',
        'TARGET-041: STK is visible after reopen on Page 209 Units of Measure.'
      ],
      fieldsChangedOnlyIfSafe: [
        `Item.No=${targetItem.itemNo}`,
        `Item.Description=${targetItem.description}`,
        `Item.Type=${targetItem.type}`,
        'Item.Base Unit=STK if visibly available'
      ],
      fieldsNotTouched: [
      ...
      fallback: 'If STK is not available or the card route is ambiguous, do not save an item; document a field-helper recovery case.'
    }
  ];
  ...
  const before = await captureState(page, 'target-036e2-010-items-before', 'Before first Item write retry gate.', {
    targetItem,
    unitReadiness
  });
  ...
  if (literalPattern(targetItem.itemNo).test(beforeText) || literalPattern(targetItem.description).test(beforeText)) {
    ...
    writeAttempt = {
      ...
      status: 'blocked',
      reason:
        'Units of Measure page did not show STK. The first item was not saved because Base Unit would be unproven.',
      unitReadiness
    };
  } else {
    ...
    if (!newClicked) {
      ...
      if (containsHardForbiddenText(afterNewText)) {
        ...
        const fill = await fillItemFields(page, cardFrame, steps);
        steps.push({ step: 'fill-item-approved-fields-only', fill });
        writeAttempt = fill.changed
          ? {
              changed: true,
              status: 'created-or-edited',
              reason: fill.reason,
              fill
            }
          : {
  ...
  await captureState(page, 'target-036e2-020-after-item-attempt', 'After Item create-or-block attempt.', {
    targetItem,
    unitReadiness,
    writeAttempt,
    steps
  ...
  const afterReopen = await captureState(page, 'target-036e2-030-after-reopen-filtered-proof', 'After reopening Items filtered to U-ITEM-HW100.', {
    targetItem,
    unitReadiness,
    writeAttempt,
    steps,
  ...
  const nextCase =
    resultStatus === 'observed'
      ...
      : unitReadiness.availableCandidates.length === 0
        ...
        : 'TARGET-036E3-FIRST-ITEM-FIELD-HELPER-RECOVERY';
  ...
  const result = {
    ...
    notProved: [
      ...
      ...(resultStatus === 'observed' ? [] : ['No Base Unit is selected on a saved item.']),
    ...
    warnings: [
      ...
      ...(resultStatus === 'observed' ? [] : ['A missing STK Base Unit surface blocks a trustworthy item save.']),
    ...
    nextStepDecision: {
      ...
      lastEvidenceSummary:
        'TARGET-036E blocked item creation because no Base Unit was proven; TARGET-041 proved STK visible after reopen.',
      isPlannedNextCaseStillSensible: true,
      reason:
        resultStatus === 'observed'
          ? 'The item write retry was the right next step because STK was available and U-ITEM-HW100 is now visible after reopen.'
          : 'The item write retry was the right next step, but it stopped because the field route was not safe enough.',
      lookaheadReviewed: [
        {
        ...
        {
          caseId: 'TARGET-036E3-FIRST-ITEM-FIELD-HELPER-RECOVERY',
      ...
      risksBeforeNextCase: [
        'The saved item is still not posting-ready.',
        'Later item updates must not set posting groups casually.',
        'No document, Preview Posting or Posting is allowed from this foundation gate.'
      ],
      requiredPreparation: [
        'Use only Base Unit STK.',
