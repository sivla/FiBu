...
Screenshot-QA zu TARGET-027D/D2/D3/D4: Der Lauf hat eine echte Setup-Wirkung erzeugt, aber keinen fachlichen Erfolg. Die Seite 472 wurde korrekt erreicht und eine partielle `INLAND`/`VAT19`-Zeile ist nach Reopen sichtbar. Gleichzeitig zeigen die Bilder, dass die Werte durch die Tab-Route verrutscht sind: `19` steht als Beschreibung, `MwSt. %` bleibt `0`, und die Konten `3806`/`1406` sind leer. TARGET-027D4 macht daraus endlich technische Feldwahrheit: Page Inspection zeigt Page `VAT Posting Setup (472, List)`, Tabelle `VAT Posting Setup (325)` und die relevanten Felder. Diese Evidence ist Blocker- und Korrekturmaterial, kein Buchbeweis fuer deutsche Umsatzsteuer. Der naechste Screenshot muss entweder die korrekt vervollstaendigte Zeile oder eine sauber bereinigte Zeile zeigen.
