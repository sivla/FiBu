...
function foundationDecisionStatus() {
  ...
  if (!existsSync(foundationDecisionPath)) {
    return {
      ...
      nextStep: 'Write FOUNDATION-READINESS-DECISION.md before customer preflight.'
    };
  }
  ...
  if (/template\/no-evidence|pending-target075-evidence/i.test(text)) {
    return {
      ready: false,
      blocker: missingBlocker,
      nextStep: 'Finalize FOUNDATION-READINESS-DECISION.md from TARGET-075 evidence before customer preflight.'
    };
  }
  if (!text.includes('PWS-MD-004-CUSTOMER-CARD-TEMPLATE-REQUIRED-FIELDS-PREFLIGHT')) {
    return {
      ready: false,
      blocker: 'foundation-readiness-decision-missing-pws-md-004',
      nextStep: 'Add an explicit PWS-MD-004 handoff before customer card/template preflight.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When live gate is open, run PWS-MD-004 only with --live-approved as read-first/no-save.'
  };
}

if (help) {
  console.log(`PWS-MD-004 guarded runner

Usage:
  node scripts/agent/run-pws-md-004-customer-card-template-required-fields-preflight.mjs --check
  node scripts/agent/run-pws-md-004-customer-card-template-required-fields-preflight.mjs --list
  node scripts/agent/run-pws-md-004-customer-card-template-required-fields-preflight.mjs --live-approved

This runner inspects the customer New/template/required-field boundary without saving customer data. Live execution requires:
- FOUNDATION-READINESS-DECISION.md with explicit PWS-MD-004 handoff
- usable stored auth
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-004-customer-card-template-required-fields-preflight-check',
        caseId: 'PWS-MD-004-CUSTOMER-CARD-TEMPLATE-REQUIRED-FIELDS-PREFLIGHT',
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-md-004-customer-card-template-required-fields-preflight-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep:
          'Do not run PWS-MD-004 live before Foundation Readiness Decision, live gate, usable auth and --live-approved.'
      },
      null,
      2
    )
