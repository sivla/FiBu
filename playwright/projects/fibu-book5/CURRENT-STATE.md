...
Als naechstes gezielt `AUTOPILOT-STATE.json` und `POSTING-AND-SETUP-GATES.md` beachten. `FIXEDASSETS-005` bis `FIXEDASSETS-007` haben die Anlagen-Readiness fuer FA Posting Groups, CRONUS-Konten, AfA-Buecher und Anlagenklassen gelesen; `FIXEDASSETS-008` hat Kapitel 21 mit dieser Evidence-Kette synchronisiert. `WAREHOUSE-001` hat `FRA-ZL` und Warehouse-Einstiege read-only geprueft; `WAREHOUSE-002` hat Kapitel 13 mit diesem Befund synchronisiert. Ohne ausdrueckliche Gate-Freigabe bleibt Fixed Assets fuer Setup und Buchung gesperrt, und Warehouse-Aktivierung bleibt gesperrt: keine Bins, keine Lagerort-Aktivierungsfelder, keine Warehouse Receipts/Put-aways/Picks/Shipments erzeugen oder buchen. Naechster sicherer Schritt ohne Freigabe ist `MANUFACTURING-001-READINESS` als read-only Vorbereitung fuer Assembly/Manufacturing-Einstiege und Voraussetzungen. Payments nur nach Payment-Gate, Reporting nur nach Analysis-View-Gate, Steuer nur nach DE-VAT-Gate, Warehouse-Aktivierung/Manufacturing/Service/Projects nur nach eigenem Gate. Shopify/Online Store ist kein naechster Prozessblock mehr; Kapitel 17 wird spaeter nur als Dropshipping/Sonderverkauf ohne Connector-Scope bearbeitet. `PAYMENTS-010` und `PAYMENTS-EVIDENCE-PACK-SYNC.md` haben den Zahlungsweg bis zur letzten Sicherheitsgrenze didaktisch geschlossen; `POSTING-TRACE-001` und `POSTING-TRACE-002` haben die gebuchten O2C-/P2P-/Inventory-Postenspuren als Anfaenger-Lernatlas und konkrete O2C-Buchstelle verbunden. `REPORTING-009` hat den letzten einfachen Sachposten-Dimensionsversuch read-only abgeschlossen: Shortcut-Spalten ja, `PRODUCTLINE`/`CHANNEL` nein. `TAX-001` hat die deutsche USt-Grenze als Readiness geklaert: ohne Freigabe kein `VAT19`-Setup und keine neue 19-%-Buchung.
