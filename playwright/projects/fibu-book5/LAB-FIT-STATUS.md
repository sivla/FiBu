...
| Procure-to-Pay | labor | Readiness fuer `K10000`, `RAW-STEEL`, `FRA-ZL` geprueft; Vendor Template angewendet; RAW-STEEL mit CRONUS-Technikfit `RETAIL`/`RESALE`/`FURNITURE`; temporaerer Purchase Order Draft mit Zeile `10 x 2500` angelegt und geloescht | `evidence/p2p-001/P2P-READINESS.json`, `evidence/p2p-001/P2P-READINESS.md`, `playwright/projects/fibu-book5/img/p2p-001-*` | P2P ist bereit fuer den naechsten Preview-Posting-Lauf, aber noch nicht gebucht; Laborentwurf laeuft mit `USD` und `Tax Percent = 0` | deutscher Einkaufsprozess mit Kreditor, Artikel/Anlage, EUR, 19-%-Vorsteuer und Posten | P2P-Preview-Posting erzeugen, Postenarten/Blocker sichern, Entwurf bereinigen |
...
2. P2P als Preview-Posting-Lauf weiterfuehren: `K10000`/`RAW-STEEL` sind readiness-geprueft; naechster Schritt ist Preview Posting ohne Buchung und mit sauberem Cleanup.
