...
const CASE_ID = 'FIXEDASSETS-177-FA-GL-JOURNAL-REFINED-CANDIDATE-READONLY';
const NEXT_CASE_ID = 'FIXEDASSETS-178-FA-GL-JOURNAL-REFINED-CANDIDATE-READONLY-REVIEW';
const TEST_ID = 'fixedassets-177';
...
function statePatch(status: 'observed' | 'blocked', summary: string) {
  return {
    current: {
      ...
      active_case_file: '.agent/state/cases/fixedassets-178-fa-gl-journal-refined-candidate-readonly-review.json',
      lastReferenceCase: CASE_ID,
      lastReferenceCaseFile: '.agent/state/cases/fixedassets-177-fa-gl-journal-refined-candidate-readonly.json',
    ...
    lastRunSummary: {
      ...
      runId: CASE_ID,
      date: '2026-06-21',
      workType: 'playwright-readonly-helper-probe',
      ...
      resultStatus: status,
      summary,
      nextStep: 'Run FIXEDASSETS-178 local review before any value entry.',
    },
    activeCase: {
      status: status === 'observed' ? 'observed-readonly-helper-probe' : 'blocked-readonly-helper-probe',
      lastResult: {
        status,
        resultFile: 'playwright/projects/fibu-book5/evidence/fixedassets-177/FIXEDASSETS-177-result.json',
        summary,
      },
      nextSafeAction: 'Run FIXEDASSETS-178 local review; do not enter values, Preview Posting or Post.',
    },
    coverage: {
      areas: {
        fixedassets: {
          currentBlock: 'fa-gl-journal-refined-candidate-readonly-review',
...
test('FIXEDASSETS-177 applies refined journal-grid candidate helper read-only', async ({ page }) => {
  ...
  try {
    ...
    if (!context.environmentInUrl || (!context.companyInUrl && !context.companyInText) || context.wrongEnvironmentVisible) {
      ...
      await screenshot(page, 'fixedassets-177-010-refined-candidate-readonly.png', {
        projectName: project.name,
        testId: TEST_ID,
        status: helperResult.success ? 'labor' : 'rejected',
        bookUse: helperResult.success ? 'debug-candidate' : 'do-not-use',
        purpose:
          'FA-177 Read-only-Probe: prueft, ob der verfeinerte Journal-Grid-Helper einen row-anchored Bal.-Account-No.-Kandidaten findet.',
  ...
  const summary =
    status === 'observed'
      ? `FA-177 found refined helper status ${helperResult?.status}; no values were entered.`
      : `FA-177 read-only helper probe stayed safe but did not unlock value entry: ${blockedBy.join(' | ')}`;
  const patch = statePatch(status, summary);

  await writeJsonEvidence(faEvidencePath('010-refined-candidate-readonly.json'), {
    schemaVersion: 1,
    purpose: 'fixedassets-177-refined-journal-grid-candidate-readonly',
    caseId: CASE_ID,
    context,
    frameUrl,
    target: TARGET,
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixedassets-177-refined-journal-grid-candidate-readonly-result',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(helperResult ? [`Refined helper status was ${helperResult.status}.`] : []),
    ...
    notProved: [
      ...(helperResult?.success ? [] : ['No safe row-anchored Bal. Account No. candidate was accepted.']),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-177-fa-gl-journal-refined-candidate-readonly.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-177/',
      'playwright/projects/fibu-book5/img/fixedassets-177-010-refined-candidate-readonly.png',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-177/README.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-177/010-refined-candidate-readonly.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-177/FIXEDASSETS-177-learning.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-177/FIXEDASSETS-177-result.json',
      'playwright/projects/fibu-book5/img/fixedassets-177-010-refined-candidate-readonly.png',
    ],
    warnings: [
      'Read-only helper probe only.',
      'Do not enter values until FA-178 reviews this result.',
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-177-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-177-learning.md'),
    [
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-177 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `010-refined-candidate-readonly.json` | JSON | Read-only Grid-Snapshot und Ergebnis des verfeinerten Helpers | keine Werteingabe, keine Preview, keine Buchung | `labor`, `read-only` |',
      '| `FIXEDASSETS-177-result.json` | JSON | Ergebnis, Grenzen, Safety Flags und State-Patch-Plan | keinen deutschen Finalnachweis | `labor` |',
      '| `FIXEDASSETS-177-learning.md` | Markdown | Lernwert row-anchored Journalzellen | keine Postenspur | `labor` |',
      '| `fixedassets-177-010-refined-candidate-readonly.screenshot.json` | Screenshot-Metadaten | sichtbarer Journal-Kontext der Read-only-Probe | keine Werteingabe | `labor` oder `rejected` |',
      '',
