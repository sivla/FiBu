...
const specPath = 'playwright/projects/fibu-book5/tests/pws-ff-006-chart-of-accounts-starter-accounts-readonly.spec.ts';
const casePath = '.agent/state/cases/pws-ff-006-chart-of-accounts-starter-accounts-readfirst.json';
...
function localCaseStatus() {
  if (!existsSync(casePath)) {
    return {
      ready: false,
      blocker: 'pws-ff-006-case-file-missing',
      nextStep: 'Create the PWS-FF-006 case file before any live Chart of Accounts starter-account proof.'
    };
  }
  ...
  if (selected !== CASE_ID) {
    return {
      ready: false,
      blocker: 'pws-ff-006-not-selected-next-case',
  ...
  if (caseFile.instance !== EXPECTED_INSTANCE || caseFile.company !== TARGET_COMPANY) {
    return {
      ready: false,
      blocker: 'pws-ff-006-case-target-mismatch',
  ...
  if (caseFile.effectiveBcActionsAllowed !== false) {
    return {
      ready: false,
      blocker: 'pws-ff-006-must-stay-readonly',
      nextStep: 'PWS-FF-006 must remain read-first/no-write.'
    };
  }
  return {
    ready: true,
    blocker: '',
    nextStep: 'When auth and live gate are ready, run PWS-FF-006 with --live-approved as read-first/no-write.'
  };
}

if (help) {
  console.log(`PWS-FF-006 guarded runner

Usage:
  node scripts/agent/run-pws-ff-006-chart-of-accounts-starter-accounts-readonly.mjs --check
  node scripts/agent/run-pws-ff-006-chart-of-accounts-starter-accounts-readonly.mjs --check --overnight
  node scripts/agent/run-pws-ff-006-chart-of-accounts-starter-accounts-readonly.mjs --list
  node scripts/agent/run-pws-ff-006-chart-of-accounts-starter-accounts-readonly.mjs --live-approved

This runner opens only the Chart of Accounts read-first. It must not create, edit, import, preview, post or switch company.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-006-chart-of-accounts-starter-accounts-readonly-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'pws-ff-006-chart-of-accounts-starter-accounts-readonly-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
        ...
        nextStep: 'Do not run PWS-FF-006 live before case selection, fresh auth, live gate and --live-approved.'
      },
      null,
      2
    )
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      PWS_FF_006_RUNNER_GUARD_CHECKED: '1',
      PWS_FF_006_LIVE_APPROVED: '1',
      PWS_FF_006_BC_TARGET_URL: targetUrl
    }
  })
);
