...
|---|---|---|---|---|---|
| `FiBu_Buch_BC_Standardprozesse_DE_Master_Blueprint.md` | `Bebilderte Klickanleitungen: aktueller Foundation-Stand` | `MASTERDATA-008` geprueft als Labor-Diagnose; Kontoentscheidung offen | Der Evidence-Stand ist weiter: `MASTERDATA-009` hat `14140` gesetzt und Preview danach bestaetigt | `MASTERDATA-009` als eigene Zeile aufnehmen; `MASTERDATA-008` historisch als Diagnose markieren | `evidence/masterdata-009/010-inventory-posting-setup-fit.json`, Screenshot `masterdata-009-*` |
| `FiBu_Buch_BC_Standardprozesse_DE_Master_Blueprint.md` | gleiche Tabelle, O2C-Zeile | O2C nur Kopf/Zeile, Auftrag wird danach bereinigt | Inzwischen gab es Preview, Laborbuchung und Postenspur; nur Preview-Auftrag wurde bereinigt | O2C-Zeile splitten: Screenshot-/Preview-Lauf vs. bewusste Laborbuchung `PS-INV103297` | `080-posting-result.json`, `082-posting-entry-trace.json` |
| `FiBu_Buch_BC_Standardprozesse_DE_Master_Blueprint.md` | O2C-Zielmodell / Buchungsspur | deutsche USt `19 %`, Brutto `80.920`, USt-Posten als Erwartung | fachlich als Ziel korrekt, aber nicht als RM-DEMO-Laborergebnis belegt | Tabelle um Spalte `Ziel DE` vs. `aktueller CRONUS-Laborstand` ergaenzen | `045-target-vs-labor-delta.*`, `080-posting-result.json` |
