...
| Compliance / E-Rechnung | `COMPLIANCE-001` prueft read-only Kapitel 22: Tell-Me-/Role-Center-Kontexte fuer `E-Rechnungen`, `VAT Entries`, `VAT Posting Setup`, `Document Sending Profiles`, `Change Log Entries` und `Change Log Setup` sind sichtbar/kontextuell sichtbar. Der Lauf nutzt breite Layoutansicht, erzeugt sechs Screenshot-Kandidaten und bucht/aendert nichts. Das ist Navigationsevidence fuer Anfaenger, aber kein E-Rechnungsprozess, kein Peppol-/Providerstatus, kein Archivnachweis, keine Change-Log-Aktivierung, kein deutsches VAT19-Setup und kein GoBD-/DE-Finalnachweis |
...
- `COMPLIANCE-001` zeigt nur Readiness-/Navigationskontexte. E-Rechnungsversand, VAT19, Change-Log-Aktivierung und GoBD-nahe Nachweise bleiben Gate- und DE-Finalthemen.
