...
const TEST_ID = 'fixedassets-265';
const CASE_ID = 'FIXEDASSETS-265-FA-DEPRECIATION-VALUE-PREFLIGHT-RETRY-AFTER-MAPPING-FIX';
...
test('FIXEDASSETS-265 fills Calculate Depreciation target values after mapping fix without OK', async ({ page }) => {
  ...
  const result = {
    schemaVersion: 1,
    purpose: 'fixed-assets-depreciation-value-preflight-after-mapping-fix-no-ok',
    caseId: CASE_ID,
    ...
    proved: [
      ...
      ...(allValuesProved
        ? ['HGB, 30.06.2026, FADEP-265-NO-OK and FA-CNC-01 were visibly proven on the request page without OK.']
        : []),
    ...
    changedFiles: [
      'package.json',
      'playwright/projects/fibu-book5/tests/fixedassets-265-fa-depreciation-value-preflight-retry-after-mapping-fix.spec.ts',
      'playwright/projects/fibu-book5/evidence/fixedassets-265/',
    ],
    evidenceRefs: [
      'playwright/projects/fibu-book5/evidence/fixedassets-265/FIXEDASSETS-265-result.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-265/FIXEDASSETS-265-VALUE-PREFLIGHT-RETRY-AFTER-MAPPING-FIX.md',
      'playwright/projects/fibu-book5/evidence/fixedassets-265/010-target-value-preflight.json',
      'playwright/projects/fibu-book5/evidence/fixedassets-265/010-target-value-preflight-text.txt',
    ],
    statePatch: {
      current: {
        activeCase: allValuesProved
          ? 'FIXEDASSETS-266-FA-DEPRECIATION-OK-EXECUTION-GATE-REVIEW'
          : 'FIXEDASSETS-266-FA-DEPRECIATION-VALUE-PREFLIGHT-AFTER-MAPPING-FIX-REVIEW',
        active_case_file: allValuesProved
          ? '.agent/state/cases/fixedassets-266-fa-depreciation-ok-execution-gate-review.json'
          : '.agent/state/cases/fixedassets-266-fa-depreciation-value-preflight-after-mapping-fix-review.json',
        lastReferenceCase: CASE_ID,
        lastReferenceCaseFile: '.agent/state/cases/fixedassets-265-fa-depreciation-value-preflight-retry-after-mapping-fix.json',
        nextStep: allValuesProved
          ...
          : 'FIXEDASSETS-266: locally review the no-OK value-preflight result after the mapping fix before any further retry.',
      },
      coverage: {
        ...
        depreciationReadiness: allValuesProved
          ? 'FA-265 proved HGB, 30.06.2026, FADEP-265-NO-OK and FA-CNC-01 on the Calculate Depreciation request page without OK after the mapping fix. OK remains locked pending local review.'
          : 'FA-265 could not prove all target request-page values without OK after the mapping fix. OK remains locked pending local review.',
      },
    },
    blockedBy: blocker,
    requiresReview: true,
    ...
    reason: allValuesProved
      ? 'Target request-page values were proven without OK after the mapping fix. Local review required before any execution.'
      : 'At least one target request-page value was not proven after the mapping fix. OK remains locked.',
    observed: {
      startedAt,
  ...
  await writeJsonEvidence(faEvidencePath('FIXEDASSETS-265-result.json'), result);
  await writeTextEvidence(
    faEvidencePath('FIXEDASSETS-265-VALUE-PREFLIGHT-RETRY-AFTER-MAPPING-FIX.md'),
    [
  ...
  await writeTextEvidence(
    ...
    [
      '# FIXEDASSETS-265 Evidence Index',
      '',
      '| Datei | Typ | Beweist | Beweist nicht | Status |',
      '|---|---|---|---|---|',
      '| `FIXEDASSETS-265-result.json` | JSON-Ergebnis | Zielwert-Preflight nach Mapping-Fix ohne OK | keine AfA-Ausfuehrung | labor |',
      '| `FIXEDASSETS-265-VALUE-PREFLIGHT-RETRY-AFTER-MAPPING-FIX.md` | Lernzusammenfassung | Zielwerte, Grenzen und naechsten Gate-Schritt | keine Journalzeile | no-ok |',
