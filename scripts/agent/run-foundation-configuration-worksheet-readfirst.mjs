...
const specPath = 'playwright/projects/fibu-book5/tests/foundation-configuration-worksheet-readfirst.spec.ts';
const currentPath = '.agent/state/current.json';
const casePath = '.agent/state/cases/foundation-configuration-worksheet-readfirst.json';
...
function localCaseStatus() {
  ...
  if (activeCase !== CASE_ID) {
    return {
      ready: false,
      blocker: 'foundation-configuration-worksheet-readfirst-not-active-case',
...
if (help) {
  console.log(`FOUNDATION configuration worksheet read-first runner

Usage:
  node scripts/agent/run-foundation-configuration-worksheet-readfirst.mjs --check
  node scripts/agent/run-foundation-configuration-worksheet-readfirst.mjs --list
  node scripts/agent/run-foundation-configuration-worksheet-readfirst.mjs --live-approved

This runner may only open Configuration Worksheet and Configuration Packages read-only in playthru / UNIVERSAARL-DE.
It must not create packages, get tables, import, export, validate, apply, edit in Excel or write setup values.`);
  process.exit(0);
}
...
if (checkOnly) {
  console.log(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-configuration-worksheet-readfirst-check',
        caseId: CASE_ID,
...
if (!liveApproved || blockedBy.length > 0) {
  console.error(
    JSON.stringify(
      {
        schemaVersion: 1,
        purpose: 'foundation-configuration-worksheet-readfirst-guard',
        canRun: false,
        liveApproved,
        expectedInstance: EXPECTED_INSTANCE,
        expectedCompany: TARGET_COMPANY,
...
exitWith(
  run('npx', ['playwright', 'test', specPath], {
    stdio: 'inherit',
    env: {
      FOUNDATION_CONFIGURATION_WORKSHEET_READFIRST_RUNNER_GUARD_CHECKED: '1',
      FOUNDATION_CONFIGURATION_WORKSHEET_READFIRST_LIVE_APPROVED: '1'
    }
  })
);
