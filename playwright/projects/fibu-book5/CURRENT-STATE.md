...
- `DROPSHIPPING-001` als read-only Evidence fuer Kapitel 17: BC-Standard-Einstiege fuer Sales Orders, Purchase Orders, Requisition Worksheets und Purchasing Codes sind sichtbar; `Drop Shipments` ist kein stabiler Treffer; Zielobjekte `D11000`, `K20000`, `SP-PUMP-01` fehlen; Shopify bleibt out of scope
...
- `DROPSHIPPING-001` ist als read-only Evidence fuer Kapitel 17 praktisch gelaufen: `Sales Orders`, `Purchase Orders`, `Requisition Worksheets` und `Purchasing Codes` sind als Einstiegspfade sichtbar; `Drop Shipments` wurde nicht belastbar sichtbar; `D11000`, `K20000` und `SP-PUMP-01` sind als konkrete Zielnummern in `RM-DEMO` nicht sichtbar. Keine Einrichtung, kein Auftrag, keine Bestellung, keine Requisition-Worksheet-Aktion, kein Shopify-Setup und keine Buchung.
