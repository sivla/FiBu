...
| Posting Groups | labor | Debitor-/Artikel-Posting-Fit fuer CRONUS-Techniklauf hergestellt; Inventory Posting Setup fuer `FRA-ZL` + `RESALE` auf `Inventory Account = 14140` gesetzt | `masterdata-006`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001`, `MICROSOFT-DOC-VALIDATION.md` | konkrete Inventory-Posting-Setup-Luecke ist im Labor geschlossen; O2C-Preview nach dem Fix noch nicht erneut geprueft | deutsche Posting Groups, General Posting Setup und Inventory Posting Setup | `UAT-O2C-001` erneut mit Preview Posting laufen lassen |
...
| Order-to-Cash | labor | Sales Orders gesucht, Auftrag per API erzeugt, Kopf/Zeile fotografiert, Dimensionen geprueft, Preview-Posting-Fehlerbild und Cleanup nachgewiesen | `playwright/projects/fibu-book5/img/uat-o2c-001-*`, `evidence/uat-o2c-001/` | Klickpfad, EUR, Menge, Lagerort, Preis und Dimension funktionieren; der fruehere Inventory-Posting-Setup-Blocker ist durch `MASTERDATA-009` im Labor vorbereitet; Steuer bleibt 0 % | deutscher O2C mit 19 % USt, Buchungsvorschau, Buchung und Postenspur | Preview Posting erneut laufen lassen und neues Ergebnis als Lernfall dokumentieren |
...
| Inventory | labor | Artikel und Lagerort vorhanden; Verkaufszeile nutzt Lagerort; `FRA-ZL` + `RESALE` wurde mit `Inventory Account = 14140` als CRONUS-Laborfit eingerichtet | `masterdata-004`, `masterdata-005`, `masterdata-008`, `masterdata-009`, `UAT-O2C-001` | Inventory-Posting-Setup-Luecke ist im Labor geschlossen; Artikelposten/Wertposten sind noch nicht nachgewiesen | Artikelposten und Wertposten nach Lieferung/Fakturierung | O2C-Preview erneut pruefen; bei Erfolg Postenarten nur als Labor-Evidence bewerten |
...
1. `Preview Posting` fuer `UAT-O2C-001` nach dem `MASTERDATA-009`-Laborfit erneut ausfuehren.
2. Wenn die Vorschau Postenarten zeigt, diese nur als CRONUS-Labor-Evidence bewerten und die deutsche USt-Grenze weiter offen halten.
3. Wenn ein neuer Setup-Fehler erscheint, ihn als naechsten Lernfall dokumentieren.
