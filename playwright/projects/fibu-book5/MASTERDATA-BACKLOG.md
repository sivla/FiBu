...
| Anlage | 7, 21 | `FA-CNC-01` | nur geplant; `FIXEDASSETS-001` hat nur Tell-Me-Einstiege read-only geprueft (`Fixed Assets` und `FA Ledger Entries` sichtbar, `Depreciation Books`/`FA Posting Groups` mit englischen Suchbegriffen nicht belastbar sichtbar) | ja | ja, `fixedassets-001` als Einstiegsevidence | `planned-only / readiness` | P2 | Vor Anlagenprozess Zielbetrag harmonisieren (`120.000 EUR` im Buch vs. `250.000` in `resources-assets-projects.json`), Nummernserie, Anlagenkarte, FA Posting Group, AfA-Buch und Einkaufs-/Aktivierungsweg per UI pruefen. |
