...
| Aktiver Case | `CUSTOMER-SETUP-VALUE-WRITE-GATE` |
...
| Master Data | customer-setup-value-write-gate-next | Debitor `U-CUST-100 / Saarland Maschinenbau AG` ist auf echter `playthru`-Debitorenkarte sichtbar. `CUSTOMER-PAYMENT-TERMS-WRITE-GATE` hat `NET30 / 30 Tage netto / 30T` auf der echten Zahlungsbedingungen-Seite geschrieben und per Reopen-Screenshot bewiesen; ein fehlerhafter Teildatensatz aus dem vorherigen Blocker wurde kontrolliert geloescht. Naechster Schritt ist nur Customer Posting Group `INLAND`, Gen. Business Posting Group `INLAND` und Payment Terms Code `NET30` auf `U-CUST-100`; noch kein Beleg, kein Preview, kein Posting. |
...
6. `CUSTOMER-PAYMENT-TERMS-WRITE-GATE` ist beobachtet: `NET30 / 30 Tage netto / 30T` ist in `playthru / UNIVERSAARL-DE` auf der echten Zahlungsbedingungen-Seite sichtbar und nach Reopen bewiesen; der vorherige falsche Teildatensatz `30 TAGE NE` wurde geloescht. Naechster Schritt ist `CUSTOMER-SETUP-VALUE-WRITE-GATE`: nur `U-CUST-100` mit Customer Posting Group `INLAND`, Gen. Business Posting Group `INLAND` und Payment Terms Code `NET30`. Kein Beleg, kein Preview, kein Posting.
